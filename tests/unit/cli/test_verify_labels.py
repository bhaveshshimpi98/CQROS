"""Unit tests for CQROS label-verification CLI."""
...
from cqros.cli.verify_labels import (
    DiscoveredWorkItem,
    VerifyLabelsOptions,
    VerifyLabelsSummary,
    build_options,
...
from cqros.core.constants import DEFAULT_STORAGE_ROOT, STORAGE_DIR_LABELS
from cqros.core.exceptions import ValidationError
from cqros.labels.verification import LabelVerifier, VerificationReport
from cqros.labels.verification.exceptions import ERROR_SCHEMA_MISMATCH
...
def test_build_parser_accepts_all_flags() -> None:
    """Parser accepts every documented label-verification flag."""
    args = build_parser().parse_args(
        [
            "--symbols",
            "BTCUSDT",
...
def test_build_options_maps_filters() -> None:
    """Explicit CLI flags map onto VerifyLabelsOptions."""
...
def test_discover_work_finds_partitions(tmp_path: Path) -> None:
    """Discovery walks label partitions without hardcoding symbols."""
...
def test_run_verification_empty_work_passes(tmp_path: Path) -> None:
    ...
    repository = MagicMock(spec=LabelRepository)
    verifier = MagicMock(spec=LabelVerifier)
    summary = _run(
        run_verification(
...
def test_run_verification_fail_on_task_failure(
    ...
    verifier.verify.side_effect = ValidationError(
        "merged label schema dtype mismatch",
    ...
    assert "Dataset: Labels" in captured
    assert "Verifier: LabelVerifier" in captured
...
def test_run_verification_respects_worker_count(tmp_path: Path) -> None:
    ...
    with patch("cqros.cli.verify_labels.asyncio.create_task", wraps=asyncio.create_task) as create:
        summary = _run(
            run_verification(
                repository=repository,
                verifier=verifier,
    ...
    worker_names = [
        ...
        if call.kwargs.get("name", "").startswith("verify-labels-worker-")
    ]
...
def test_format_partition_failure() -> None:
    ...
    text = format_partition_failure(
        ...
        exception_type="LabelValidationError",
        message="merged label schema dtype mismatch",
        code=ERROR_SCHEMA_MISMATCH,
    )
    assert text.startswith("FAILED\n")
    assert "Dataset: Labels" in text
    assert "Verifier: LabelVerifier" in text
...
def test_main_exit_code_pass(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_labels.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_labels.discover_work", return_value=()),
    ):
...
def test_main_exit_code_fail(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_labels.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_labels.discover_work", return_value=work),
        patch("cqros.cli.verify_labels.LabelRepository") as repo_cls,
        patch("cqros.cli.verify_labels.LabelVerifier") as verifier_cls,
    ):
        repository = MagicMock(spec=LabelRepository)
        repository.load.return_value = pl.DataFrame({"open_time": [1]})
