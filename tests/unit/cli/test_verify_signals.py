"""Unit tests for CQROS signal-dataset verification CLI."""
...
import cqros.cli.verify_signals as verify_signals_module
from cqros.cli.verify_signals import (
    DiscoveredWorkItem,
    VerifySignalOptions,
    VerifySignalSummary,
    build_options,
...
from cqros.core.constants import DEFAULT_STORAGE_ROOT, STORAGE_DIR_SIGNALS
from cqros.core.exceptions import ValidationError
from cqros.signals.verification import SignalVerifier, VerificationReport
from cqros.signals.verification.exceptions import ERROR_SCHEMA_MISMATCH
from cqros.storage import ParquetStore, SignalRepository, StorageLayout
...
def _failed_report(*, invalid_signal_rows: int = 1) -> VerificationReport:
    ...
    return VerificationReport(
        ...
        nan_rows=0,
        invalid_timestamp_rows=0,
        invalid_numeric_rows=invalid_signal_rows,
...
def test_package_exports() -> None:
    ...
    assert expected.issubset(set(verify_signals_module.__all__))
    assert verify_signals_module.build_parser is build_parser
    assert verify_signals_module.main is main
...
def test_build_parser_accepts_all_flags() -> None:
    """Parser accepts every documented signal-verification flag."""
    args = build_parser().parse_args(
        [
            "--model",
            _MODEL,
...
def test_build_options_maps_filters() -> None:
    """Explicit CLI flags map onto VerifySignalOptions."""
...
def test_run_verification_empty_work_passes(tmp_path: Path) -> None:
    ...
    repository = MagicMock(spec=SignalRepository)
    verifier = MagicMock(spec=SignalVerifier)
    summary = _run(
        run_verification(
...
def test_run_verification_fail_on_counters(tmp_path: Path) -> None:
    ...
    verifier = MagicMock(spec=SignalVerifier)
    verifier.verify.return_value = _failed_report(invalid_signal_rows=2)
...
def test_run_verification_fail_on_task_failure(
    ...
    assert "Dataset: Signals" in captured
    assert "Verifier: SignalVerifier" in captured
...
def test_run_verification_model_version_filters(tmp_path: Path) -> None:
    """--model / --version restrict rows before SignalVerifier runs."""
    work = (DiscoveredWorkItem(symbol="BTCUSDT", timeframe="1h", years=(2024,)),)
    repository = MagicMock(spec=SignalRepository)
    repository.load.return_value = pl.DataFrame(
        {
...
def test_run_verification_model_filter_skips_non_matching(tmp_path: Path) -> None:
    """Partitions with no matching model/version rows are excluded."""
    work = (DiscoveredWorkItem(symbol="BTCUSDT", timeframe="1h", years=(2024,)),)
    repository = MagicMock(spec=SignalRepository)
    repository.load.return_value = pl.DataFrame(
        {
...
def test_run_verification_respects_worker_count(tmp_path: Path) -> None:
    ...
    with patch("cqros.cli.verify_signals.asyncio.create_task", wraps=asyncio.create_task) as create:
        summary = _run(
            run_verification(
                repository=repository,
                verifier=verifier,
    ...
    worker_names = [
        ...
        if call.kwargs.get("name", "").startswith("verify-signals-worker-")
    ]
    assert len(worker_names) == 2


def test_format_summary_pass_and_fail() -> None:
    """Summary rendering mirrors the Prediction CLI PASS/FAIL style."""
...
def test_format_partition_failure() -> None:
    ...
    text = format_partition_failure(
        ...
        exception_type="SignalValidationError",
        message="merged signal schema dtype mismatch",
        code=ERROR_SCHEMA_MISMATCH,
    )
    assert text.startswith("FAILED\n")
    assert "Dataset: Signals" in text
    assert "Verifier: SignalVerifier" in text
...
def test_main_exit_code_pass(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_signals.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_signals.discover_work", return_value=()),
    ):
...
def test_main_exit_code_fail(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_signals.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_signals.discover_work", return_value=work),
        patch("cqros.cli.verify_signals.SignalRepository") as repo_cls,
        patch("cqros.cli.verify_signals.SignalVerifier") as verifier_cls,
    ):
        repository = MagicMock(spec=SignalRepository)
        repository.load.return_value = pl.DataFrame({"open_time": [1]})
