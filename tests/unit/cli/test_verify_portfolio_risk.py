"""Unit tests for CQROS portfolio-risk verification CLI."""
...
import cqros.cli.verify_portfolio_risk as verify_portfolio_risk_module
from cqros.cli.verify_portfolio_risk import (
    DiscoveredWorkItem,
...
def _portfolio_risk_frame(*, symbol: str = _SYMBOL) -> pl.DataFrame:
    """Return a canonical passing portfolio-risk frame."""
...
def test_build_parser_and_options(tmp_path: Path) -> None:
    ...
    with patch.object(verify_portfolio_risk_module, "DEFAULT_STORAGE_ROOT", tmp_path):
    ...
    with pytest.raises(ValidationError) as exc_info:
        build_options(args)
    assert exc_info.value.error_code == "CLI-VERIFY-PORTFOLIO-RISK-001"
...
def test_format_summary_and_partition_failure() -> None:
    ...
    summary = VerifyPortfolioRiskSummary(
        symbols_verified=1,
        datasets_verified=1,
    ...
    failure = format_partition_failure(
        dataset="Portfolio Risk",
        symbol=_SYMBOL,
        timeframe=_TIMEFRAME,
        partition="2026.parquet",
        verifier="PortfolioRiskVerifier",
        exception_type="PortfolioRiskValidationError",
        message="missing required columns",
        code="PRISK-VERIFICATION-001",
    )
    assert "FAILED" in failure
    assert _SYMBOL in failure
    assert "PortfolioRiskVerifier" in failure
    assert "PRISK-VERIFICATION-001" in failure
...
def test_run_verification_fail_on_invalid_frame(tmp_path: Path) -> None:
    ...
    repository = PortfolioRiskRepository(StorageLayout(tmp_path), ParquetStore())
    invalid = _portfolio_risk_frame().drop("equity")
    repository.save(
        invalid,
        manager=_MANAGER,
        exchange=EXCHANGE_BINANCE,
