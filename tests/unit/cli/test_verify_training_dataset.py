"""Unit tests for CQROS training-dataset verification CLI."""
...
from cqros.cli.verify_training_dataset import (
    DiscoveredWorkItem,
...
from cqros.core.constants import DEFAULT_STORAGE_ROOT, STORAGE_DIR_TRAINING
...
from cqros.training.verification.exceptions import ERROR_SCHEMA_MISMATCH
...
def test_build_parser_accepts_all_flags() -> None:
    """Parser accepts every documented training-verification flag."""
    args = build_parser().parse_args(
        [
            "--symbols",
            "BTCUSDT",
...
def test_build_options_maps_filters() -> None:
    """Explicit CLI flags map onto VerifyTrainingOptions."""
...
def test_discover_work_finds_partitions(tmp_path: Path) -> None:
    """Discovery walks training partitions without hardcoding symbols."""
    _touch_training(tmp_path, symbol="BTCUSDT", timeframe="1h", year=2024)
    _touch_training(tmp_path, symbol="ETHUSDT", timeframe="1h", year=2023)
    _touch_training(tmp_path, symbol="ETHUSDT", timeframe="1h", year=2024)
...
def test_discover_work_filters(tmp_path: Path) -> None:
    ...
    _touch_training(tmp_path, symbol="BTCUSDT", timeframe="1h", year=2024)
    _touch_training(tmp_path, symbol="BTCUSDT", timeframe="4h", year=2024)
    _touch_training(tmp_path, symbol="ETHUSDT", timeframe="1h", year=2024)
    _touch_training(tmp_path, symbol="BTCUSDT", timeframe="1h", year=2025)
...
def test_run_verification_empty_work_passes(tmp_path: Path) -> None:
    ...
    repository = MagicMock(spec=TrainingRepository)
    verifier = MagicMock(spec=TrainingVerifier)
    summary = _run(
        run_verification(
...
def test_run_verification_fail_on_task_failure(
    ...
    assert "Dataset: Training" in captured
    assert "Verifier: TrainingVerifier" in captured
...
def test_run_verification_respects_worker_count(tmp_path: Path) -> None:
    ...
    with patch(
        "cqros.cli.verify_training_dataset.asyncio.create_task", wraps=asyncio.create_task
    ) as create:
        summary = _run(
            run_verification(
                repository=repository,
    ...
    worker_names = [
        ...
        if call.kwargs.get("name", "").startswith("verify-training-worker-")
    ]
    assert len(worker_names) == 2


def test_format_summary_pass_and_fail() -> None:
    """Summary rendering mirrors the Feature CLI PASS/FAIL style."""
...
def test_format_partition_failure() -> None:
    ...
    text = format_partition_failure(
        ...
        exception_type="TrainingValidationError",
        message="merged training schema dtype mismatch",
        code=ERROR_SCHEMA_MISMATCH,
    )
    assert text.startswith("FAILED\n")
    assert "Dataset: Training" in text
    assert "Verifier: TrainingVerifier" in text
...
def test_main_exit_code_pass(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_training_dataset.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_training_dataset.discover_work", return_value=()),
    ):
...
def test_main_exit_code_fail(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_training_dataset.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_training_dataset.discover_work", return_value=work),
        patch("cqros.cli.verify_training_dataset.TrainingRepository") as repo_cls,
        patch("cqros.cli.verify_training_dataset.TrainingVerifier") as verifier_cls,
    ):
        repository = MagicMock(spec=TrainingRepository)
        repository.load.return_value = pl.DataFrame({"open_time": [1]})
