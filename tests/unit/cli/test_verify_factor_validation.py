"""Unit tests for CQROS factor validation verification CLI."""
...
import cqros.cli.verify_factor_validation as verify_factor_validation_module
from cqros.cli.verify_factor_validation import (
    DiscoveredWorkItem,
...
def _options(
    *,
    ...
    """Build VerifyFactorValidationOptions against a temporary storage root."""
...
def _canonical_frame(
    *,
    ...
    """Return a canonical passing factor validation frame for CLI tests."""
    return pl.DataFrame(
        {
...
def test_build_parser_and_options(tmp_path: Path) -> None:
    ...
    assert exc_info.value.error_code == "CLI-VERIFY-FACTOR-VALIDATION-001"
...
def test_build_options_rejects_blank_manager() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        build_options(args)
    assert exc_info.value.error_code == "CLI-VERIFY-FACTOR-VALIDATION-004"
...
def test_format_summary_shows_pass_status() -> None:
    ...
    summary = VerifyFactorValidationSummary(
        panels_verified=1,
        datasets_verified=1,
...
def test_format_partition_failure_renders_expected_fields() -> None:
    ...
    failure = format_partition_failure(
        ...
        code="FVAL-VERIFICATION-001",
    )
    assert "FAILED" in failure
    assert f"Timeframe: {_TIMEFRAME}" in failure
    assert f"Year: {_YEAR}" in failure
    assert "FactorValidationVerifier" in failure
    assert "FVAL-VERIFICATION-001" in failure
    assert "Factor Validation" in failure
...
def test_run_verification_fail_on_invalid_frame(tmp_path: Path) -> None:
    ...
    repository = FactorValidationRepository(layout, datastore)
    options = _options(storage_root=tmp_path)
    work = discover_work(repository, options)
    summary = _run(
        run_verification(
...
def test_main_returns_success_for_empty_universe(tmp_path: Path) -> None:
    ...
    with patch.object(verify_factor_validation_module, "DEFAULT_STORAGE_ROOT", tmp_path):
