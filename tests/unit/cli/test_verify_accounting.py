"""Unit tests for CQROS accounting verification CLI."""
...
import cqros.cli.verify_accounting as verify_accounting_module
...
from cqros.cli.verify_accounting import (
    DiscoveredWorkItem,
...
def _accounting_frame() -> pl.DataFrame:
    """Return a canonical passing accounting frame."""
...
def test_build_parser_and_options(tmp_path: Path) -> None:
    ...
    with patch.object(verify_accounting_module, "DEFAULT_STORAGE_ROOT", tmp_path):
    ...
    with pytest.raises(ValidationError) as exc_info:
        build_options(args)
    assert exc_info.value.error_code == "CLI-VERIFY-ACCOUNTING-001"
...
def test_format_summary_and_partition_failure() -> None:
    ...
    summary = VerifyAccountingSummary(
        symbols_verified=1,
        datasets_verified=1,
    ...
    failure = format_partition_failure(
        ...
        verifier="AccountingVerifier",
        exception_type="AccountingValidationError",
        message="missing required columns",
        code="ACC-VERIFICATION-001",
    )
    assert "FAILED" in failure
    assert _SYMBOL in failure
    assert "AccountingVerifier" in failure
    assert "ACC-VERIFICATION-001" in failure
