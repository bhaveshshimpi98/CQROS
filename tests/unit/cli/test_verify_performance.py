"""Unit tests for CQROS performance verification CLI."""
...
import cqros.cli.verify_performance as verify_performance_module
from cqros.cli.verify_performance import (
    DiscoveredWorkItem,
...
from cqros.performance import PerformanceRepository, PerformanceStatus, PerformanceVerifier
...
def _options(
    *,
    ...
    """Build VerifyPerformanceOptions against a temporary storage root."""
...
def _canonical_frame(
    *,
    ...
    """Return a canonical passing performance frame for CLI verification tests."""
    return pl.DataFrame(
        {
            "symbol": [symbol],
...
def test_build_parser_and_options(tmp_path: Path) -> None:
    ...
    assert exc_info.value.error_code == "CLI-VERIFY-PERFORMANCE-001"
...
def test_build_options_rejects_blank_manager() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        build_options(args)
    assert exc_info.value.error_code == "CLI-VERIFY-PERFORMANCE-004"
...
def test_format_summary_shows_pass_status() -> None:
    ...
    summary = VerifyPerformanceSummary(
        symbols_verified=1,
        datasets_verified=1,
...
def test_format_partition_failure_renders_expected_fields() -> None:
    ...
    failure = format_partition_failure(
        ...
        verifier="PerformanceVerifier",
        exception_type="PerformanceValidationError",
        message="missing required columns",
        code="PERF-VERIFICATION-001",
    )
    assert "FAILED" in failure
    assert _SYMBOL in failure
    assert "PerformanceVerifier" in failure
    assert "PERF-VERIFICATION-001" in failure
    assert "Performance" in failure
...
def test_run_verification_fail_on_invalid_frame(tmp_path: Path) -> None:
    ...
    repository = PerformanceRepository(layout, datastore)
    options = _options(storage_root=tmp_path)
    work = discover_work(repository, options)
    summary = _run(
        run_verification(
...
def test_main_returns_success_for_empty_universe(tmp_path: Path) -> None:
    ...
    with patch.object(verify_performance_module, "DEFAULT_STORAGE_ROOT", tmp_path):
