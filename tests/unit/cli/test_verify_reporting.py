"""Unit tests for CQROS reporting verification CLI."""
...
import cqros.cli.verify_reporting as verify_reporting_module
from cqros.cli.verify_reporting import (
    DiscoveredWorkItem,
...
from cqros.reporting import ReportingRepository, ReportingStatus, ReportingVerifier
...
def _options(
    *,
    ...
    """Build VerifyReportingOptions against a temporary storage root."""
...
def _canonical_frame(
    *,
    ...
    """Return a canonical passing reporting frame for CLI verification tests."""
    return pl.DataFrame(
        {
            "symbol": [symbol],
...
def test_build_parser_and_options(tmp_path: Path) -> None:
    ...
    assert exc_info.value.error_code == "CLI-VERIFY-REPORTING-001"
...
def test_build_options_rejects_blank_manager() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        build_options(args)
    assert exc_info.value.error_code == "CLI-VERIFY-REPORTING-004"
...
def test_format_summary_shows_pass_status() -> None:
    ...
    summary = VerifyReportingSummary(
        symbols_verified=1,
        datasets_verified=1,
...
def test_format_partition_failure_renders_expected_fields() -> None:
    ...
    failure = format_partition_failure(
        ...
        verifier="ReportingVerifier",
        exception_type="ReportingValidationError",
        message="missing required columns",
        code="REP-VERIFICATION-001",
    )
    assert "FAILED" in failure
    assert _SYMBOL in failure
    assert "ReportingVerifier" in failure
    assert "REP-VERIFICATION-001" in failure
    assert "Reporting" in failure
...
def test_run_verification_fail_on_invalid_frame(tmp_path: Path) -> None:
    ...
    repository = ReportingRepository(layout, datastore)
    options = _options(storage_root=tmp_path)
    work = discover_work(repository, options)
    summary = _run(
        run_verification(
...
def test_main_returns_success_for_empty_universe(tmp_path: Path) -> None:
    ...
    with patch.object(verify_reporting_module, "DEFAULT_STORAGE_ROOT", tmp_path):
