"""Unit tests for CQROS prediction-dataset verification CLI."""
...
from cqros.cli.verify_predictions import (
    DiscoveredWorkItem,
...
from cqros.predictions.verification.exceptions import ERROR_SCHEMA_MISMATCH
...
def test_build_parser_accepts_all_flags() -> None:
    """Parser accepts every documented prediction-verification flag."""
    args = build_parser().parse_args(
        [
            "--symbols",
            "BTCUSDT",
...
def test_build_options_maps_filters() -> None:
    """Explicit CLI flags map onto VerifyPredictionOptions."""
...
def test_discover_work_finds_partitions(tmp_path: Path) -> None:
    ...
    assert work[0].framework == _FRAMEWORK
    assert work[0].model_name == _MODEL
...
def test_run_verification_empty_work_passes(tmp_path: Path) -> None:
    ...
    verifier = MagicMock(spec=PredictionVerifier)
    summary = _run(
        run_verification(
...
def test_run_verification_fail_on_task_failure(
    ...
    verifier.verify.side_effect = ValidationError(
        "merged prediction schema dtype mismatch",
    ...
    assert "Dataset: Predictions" in captured
    assert "Verifier: PredictionVerifier" in captured
...
def test_run_verification_respects_worker_count(tmp_path: Path) -> None:
    ...
    with patch(
        "cqros.cli.verify_predictions.asyncio.create_task", wraps=asyncio.create_task
    ) as create:
        summary = _run(
            run_verification(
                repository=repository,
    ...
    worker_names = [
        ...
        if call.kwargs.get("name", "").startswith("verify-predictions-worker-")
    ]
...
def test_format_partition_failure() -> None:
    ...
    text = format_partition_failure(
        ...
        exception_type="PredictionValidationError",
        message="merged prediction schema dtype mismatch",
        code=ERROR_SCHEMA_MISMATCH,
    )
    assert text.startswith("FAILED\n")
    assert "Dataset: Predictions" in text
    assert "Verifier: PredictionVerifier" in text
...
def test_main_exit_code_pass(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_predictions.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_predictions.discover_work", return_value=()),
    ):
...
def test_main_exit_code_fail(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        patch("cqros.cli.verify_predictions.DEFAULT_STORAGE_ROOT", str(tmp_path)),
        patch("cqros.cli.verify_predictions.discover_work", return_value=work),
        patch("cqros.cli.verify_predictions.PredictionRepository") as repo_cls,
        patch("cqros.cli.verify_predictions.PredictionVerifier") as verifier_cls,
    ):
        repository = MagicMock(spec=PredictionRepository)
        repository.load.return_value = pl.DataFrame({"open_time": [1]})
