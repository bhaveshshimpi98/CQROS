"""Unit tests for CQROS backtesting verification CLI."""
...
import cqros.cli.verify_backtesting as verify_backtesting_module
...
from cqros.cli.verify_backtesting import (
    DiscoveredWorkItem,
...
def _options(
    *,
    ...
    """Build VerifyBacktestingOptions against a temporary storage root."""
...
def _canonical_frame(
    *,
    ...
    """Return a canonical passing backtesting frame for CLI verification tests."""
    return pl.DataFrame(
        {
            "symbol": [symbol],
...
def test_build_parser_and_options(tmp_path: Path) -> None:
    ...
    assert exc_info.value.error_code == "CLI-VERIFY-BACKTESTING-001"
...
def test_build_options_rejects_blank_manager() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        build_options(args)
    assert exc_info.value.error_code == "CLI-VERIFY-BACKTESTING-006"
...
def test_format_summary_shows_pass_status() -> None:
    ...
    summary = VerifyBacktestingSummary(
        symbols_verified=1,
        datasets_verified=1,
...
def test_format_partition_failure_renders_expected_fields() -> None:
    ...
    failure = format_partition_failure(
        ...
        verifier="BacktestingVerifier",
        exception_type="BacktestingValidationError",
        message="missing required columns",
        code="BT-VERIFICATION-001",
    )
    assert "FAILED" in failure
    assert _SYMBOL in failure
    assert "BacktestingVerifier" in failure
    assert "BT-VERIFICATION-001" in failure
    assert "Backtesting" in failure
...
def test_run_verification_fail_on_invalid_frame(tmp_path: Path) -> None:
    ...
    repository = BacktestingRepository(StorageLayout(tmp_path), ParquetStore())
    invalid = _canonical_frame().drop("status")
    repository.save(
        invalid,
        manager=_MANAGER,
        exchange=EXCHANGE_BINANCE,
...
def test_main_returns_success_for_empty_universe(tmp_path: Path) -> None:
    ...
    with patch.object(verify_backtesting_module, "DEFAULT_STORAGE_ROOT", tmp_path):
