"""Unit tests for CQROS ``AnalyticsEngineRegistry``."""
...
def _registry_with(*names: str) -> AnalyticsEngineRegistry:
    """Build a registry pre-populated with SimpleAnalyticsEngine under each name."""
    registry = AnalyticsEngineRegistry()
    for name in names:
        registry.register(name, SimpleAnalyticsEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise ANA_REG_NAME_BLANK."""
    registry = AnalyticsEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(AnalyticsValidationError) as exc_info:
            registry.register(blank, SimpleAnalyticsEngine())
        assert exc_info.value.error_code == "ANA_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement AnalyticsEngine raise ANA_REG_NOT_ENGINE."""
    registry = AnalyticsEngineRegistry()
    with pytest.raises(AnalyticsValidationError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "ANA_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises ANA_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(AnalyticsValidationError) as exc_info:
        registry.register("simple", SimpleAnalyticsEngine())
    assert exc_info.value.error_code == "ANA_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = AnalyticsEngineRegistry()
    engines = {
        "engine-a": SimpleAnalyticsEngine(),
        "engine-b": SimpleAnalyticsEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimpleAnalyticsEngine]]:
            return [("dup", SimpleAnalyticsEngine()), ("dup", SimpleAnalyticsEngine())]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(AnalyticsValidationError) as exc_info:
        registry.register_many({"existing": SimpleAnalyticsEngine()})
    assert exc_info.value.error_code == "ANA_REG_DUPLICATE"
...
def test_get_rejects_unknown_name() -> None:
    """get() raises ANA_REG_UNKNOWN for unregistered names."""
    registry = AnalyticsEngineRegistry()
    with pytest.raises(AnalyticsValidationError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimpleAnalyticsEngine()
