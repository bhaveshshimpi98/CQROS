"""Unit tests for CQROS ``PerformanceEngineRegistry``."""
...
def _registry_with(*names: str) -> PerformanceEngineRegistry:
    """Build a registry pre-populated with SimplePerformanceEngine under each name."""
    registry = PerformanceEngineRegistry()
    for name in names:
        registry.register(name, SimplePerformanceEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise PERF_REG_NAME_BLANK."""
    registry = PerformanceEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(PerformanceValidationError) as exc_info:
            registry.register(blank, SimplePerformanceEngine())
        assert exc_info.value.error_code == "PERF_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement PerformanceEngine raise PERF_REG_NOT_ENGINE."""
    registry = PerformanceEngineRegistry()
    with pytest.raises(PerformanceValidationError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "PERF_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises PERF_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(PerformanceValidationError) as exc_info:
        registry.register("simple", SimplePerformanceEngine())
    assert exc_info.value.error_code == "PERF_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = PerformanceEngineRegistry()
    engines = {
        "engine-a": SimplePerformanceEngine(),
        "engine-b": SimplePerformanceEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimplePerformanceEngine]]:
            return [("dup", SimplePerformanceEngine()), ("dup", SimplePerformanceEngine())]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(PerformanceValidationError) as exc_info:
        registry.register_many({"existing": SimplePerformanceEngine()})
    assert exc_info.value.error_code == "PERF_REG_DUPLICATE"
...
def test_get_rejects_unknown_name() -> None:
    """get() raises PERF_REG_UNKNOWN for unregistered names."""
    registry = PerformanceEngineRegistry()
    with pytest.raises(PerformanceValidationError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimplePerformanceEngine()
