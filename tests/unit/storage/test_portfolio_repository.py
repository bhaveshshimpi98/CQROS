"""Unit tests for CQROS portfolio dataset repository."""
...
from cqros.storage.portfolio_repository import (
    PortfolioRepository as PortfolioRepositoryDirect,
)
...
def sample_frame() -> pl.DataFrame:
    """Return a deterministic canonical sample portfolio DataFrame."""
    return pl.DataFrame(
        {
...
def repository(
    ...
    """Return a portfolio repository wired to the test layout and datastore."""
    return PortfolioRepository(layout, datastore)


def _portfolio_path(layout: StorageLayout) -> Path:
    """Compose the canonical sample portfolio partition path."""
    return layout.portfolio_path(
        _EXCHANGE,
        _MARKET,
        _SYMBOL,
        _TIMEFRAME,
...
def test_portfolio_repository_is_exported_from_package() -> None:
    """Package export matches the portfolio repository module class."""
    assert PortfolioRepository is PortfolioRepositoryDirect


def test_portfolio_partition_ref_is_frozen_dataclass() -> None:
    """PortfolioPartitionRef is an immutable slotted dataclass."""
    ref = PortfolioPartitionRef(
        exchange=_EXCHANGE,
        market=_MARKET,
        symbol=_SYMBOL,
    ...
    assert ref == PortfolioPartitionRef(
        exchange=_EXCHANGE,
        market=_MARKET,
        symbol=_SYMBOL,
        timeframe=_TIMEFRAME,
...
def test_save_and_load_uses_portfolio_layout_path(
    repository: PortfolioRepository,
    layout: StorageLayout,
    datastore: _InMemoryDataStore,
    sample_frame: pl.DataFrame,
) -> None:
    """Portfolio save/load uses StorageLayout.portfolio_path."""
    expected = _portfolio_path(layout)
    assert STORAGE_DIR_PORTFOLIOS in expected.parts
...
def test_canonical_schema_round_trip(
    ...
    """Persisted frames retain the canonical Portfolio column set."""
    repository.save(
        sample_frame,
        exchange=_EXCHANGE,
        market=_MARKET,
...
def test_dtype_preservation(
    ...
    """Repository preserves provided Portfolio column dtypes on round-trip."""
    repository.save(
        sample_frame,
        exchange=_EXCHANGE,
        market=_MARKET,
...
def test_portfolio_path_partitioning_matches_layout_contract(
    layout: StorageLayout,
) -> None:
    """Portfolio partitions follow exchange/market/symbol/timeframe/year.parquet."""
    path = _portfolio_path(layout)
    ...
    assert path.parent.parent.parent.parent.parent.name == STORAGE_DIR_PORTFOLIOS
    assert STORAGE_DIR_PORTFOLIOS in path.parts
...
def test_exists_false_when_missing(
    ...
    assert datastore.exists_paths == [_portfolio_path(layout)]
...
def test_exists_true_when_partition_saved(
    ...
    assert datastore.read_paths == []
    assert _portfolio_path(layout) in datastore.exists_paths
...
def test_discover_partitions_finds_year_files(tmp_path: Path) -> None:
    """Discovery walks portfolio trees without returning filesystem paths."""
    path = (
        tmp_path
        / STORAGE_DIR_PORTFOLIOS
        / EXCHANGE_BINANCE
...
def test_round_trip_with_parquet_store(
    ...
    """Portfolio repository round-trips through a real ``ParquetStore``."""
    ...
    for column, dtype in _CANONICAL_DTYPES.items():
        assert loaded.schema[column] == dtype
    assert _portfolio_path(layout).is_file()


def test_portfolio_paths_differ_from_other_dataset_paths(layout: StorageLayout) -> None:
    """Portfolio partitions resolve to a location distinct from other tiers."""
    portfolio = _portfolio_path(layout)
    ...
    assert portfolio != signal
    assert portfolio != training
    assert portfolio != feature
