"""Unit tests for CQROS label dataset repository."""
...
from cqros.storage.label_repository import LabelRepository as LabelRepositoryDirect
...
def sample_frame() -> pl.DataFrame:
    """Return a deterministic sample label DataFrame."""
    return pl.DataFrame(
        {
...
def repository(
    ...
    """Return a label repository wired to the test layout and datastore."""
    return LabelRepository(layout, datastore)


def test_label_repository_is_exported_from_package() -> None:
    """Package export matches the label repository module class."""
    assert LabelRepository is LabelRepositoryDirect


def test_label_partition_ref_is_frozen_dataclass() -> None:
    """LabelPartitionRef is an immutable slotted dataclass."""
    ref = LabelPartitionRef(symbol=_SYMBOL, timeframe=_TIMEFRAME, year=_YEAR)
    assert is_dataclass(ref)
    assert ref.symbol == _SYMBOL
    assert ref.timeframe == _TIMEFRAME
    assert ref.year == _YEAR
    assert ref == LabelPartitionRef(symbol=_SYMBOL, timeframe=_TIMEFRAME, year=_YEAR)
    with pytest.raises(FrozenInstanceError):
        ref.year = 2025  # type: ignore[misc]


def test_save_and_load_uses_label_layout_path(
    repository: LabelRepository,
    layout: StorageLayout,
    datastore: _InMemoryDataStore,
    sample_frame: pl.DataFrame,
) -> None:
    """Label save/load uses StorageLayout.label_path."""
    expected = layout.label_path(
        _EXCHANGE,
        _MARKET,
        _SYMBOL,
        _TIMEFRAME,
    ...
    assert STORAGE_DIR_LABELS in expected.parts
...
def test_label_path_partitioning_matches_layout_contract(layout: StorageLayout) -> None:
    """Label partitions follow exchange/market/symbol/timeframe/year.parquet."""
    path = layout.label_path(
        _EXCHANGE,
        _MARKET,
        _SYMBOL,
        _TIMEFRAME,
    ...
    assert path.parent.parent.parent.parent.parent.name == STORAGE_DIR_LABELS
    assert STORAGE_DIR_LABELS in path.parts
...
def test_exists_false_when_missing(
    ...
    assert datastore.exists_paths == [
        layout.label_path(_EXCHANGE, _MARKET, _SYMBOL, _TIMEFRAME, _YEAR)
    ]
...
def test_list_years_returns_sorted_partition_years(tmp_path: Path) -> None:
    ...
    base = (
        tmp_path / STORAGE_DIR_LABELS / EXCHANGE_BINANCE / MARKET_USDT_PERPETUAL / "BTCUSDT" / "1h"
    )
    ...
    repository = LabelRepository(StorageLayout(tmp_path), ParquetStore())
...
def test_discover_partitions_finds_year_files(tmp_path: Path) -> None:
    """Discovery walks label trees without returning filesystem paths."""
    path = (
        tmp_path
        / STORAGE_DIR_LABELS
        / EXCHANGE_BINANCE
    ...
    assert partitions == (
        LabelPartitionRef(symbol="BTCUSDT", timeframe="1h", year=2024),
        LabelPartitionRef(symbol="ETHUSDT", timeframe="1h", year=2023),
    )
...
def test_discover_partitions_applies_filters(tmp_path: Path) -> None:
    ...
    assert partitions == (LabelPartitionRef(symbol="BTCUSDT", timeframe="1h", year=2024),)
...
def test_round_trip_with_parquet_store(
    ...
    """Label repository round-trips through a real ``ParquetStore``."""
...
def test_label_paths_differ_from_feature_paths(layout: StorageLayout) -> None:
    """Label and feature partitions resolve to different locations."""
