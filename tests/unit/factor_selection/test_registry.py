"""Unit tests for CQROS ``FactorSelectionEngineRegistry``."""
...
def _registry_with(*names: str) -> FactorSelectionEngineRegistry:
    """Build a registry pre-populated with SimpleFactorSelectionEngine under each name."""
    registry = FactorSelectionEngineRegistry()
    for name in names:
        registry.register(name, SimpleFactorSelectionEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise FSEL_REG_NAME_BLANK."""
    registry = FactorSelectionEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(FactorSelectionError) as exc_info:
            registry.register(blank, SimpleFactorSelectionEngine())
        assert exc_info.value.error_code == "FSEL_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement FactorSelectionEngine raise FSEL_REG_NOT_ENGINE."""
    registry = FactorSelectionEngineRegistry()
    with pytest.raises(FactorSelectionError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "FSEL_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises FSEL_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(FactorSelectionError) as exc_info:
        registry.register("simple", SimpleFactorSelectionEngine())
    assert exc_info.value.error_code == "FSEL_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = FactorSelectionEngineRegistry()
    engines = {
        "engine-a": SimpleFactorSelectionEngine(),
        "engine-b": SimpleFactorSelectionEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimpleFactorSelectionEngine]]:
            return [
                ("dup", SimpleFactorSelectionEngine()),
                ("dup", SimpleFactorSelectionEngine()),
            ]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(FactorSelectionError) as exc_info:
        registry.register_many({"existing": SimpleFactorSelectionEngine()})
    assert exc_info.value.error_code == "FSEL_REG_DUPLICATE"
...
def test_register_many_is_atomic_on_bad_engine() -> None:
    ...
    with pytest.raises(FactorSelectionError):
        registry.register_many(
            {
                "good": SimpleFactorSelectionEngine(),
...
def test_get_rejects_unknown_name() -> None:
    """get() raises FSEL_REG_UNKNOWN for unregistered names."""
    registry = FactorSelectionEngineRegistry()
    with pytest.raises(FactorSelectionError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimpleFactorSelectionEngine()
