"""Unit tests for CQROS ``ReportingEngineRegistry``."""
...
def _registry_with(*names: str) -> ReportingEngineRegistry:
    """Build a registry pre-populated with SimpleReportingEngine under each name."""
    registry = ReportingEngineRegistry()
    for name in names:
        registry.register(name, SimpleReportingEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise REP_REG_NAME_BLANK."""
    registry = ReportingEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(ReportingValidationError) as exc_info:
            registry.register(blank, SimpleReportingEngine())
        assert exc_info.value.error_code == "REP_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement ReportingEngine raise REP_REG_NOT_ENGINE."""
    registry = ReportingEngineRegistry()
    with pytest.raises(ReportingValidationError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "REP_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises REP_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(ReportingValidationError) as exc_info:
        registry.register("simple", SimpleReportingEngine())
    assert exc_info.value.error_code == "REP_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = ReportingEngineRegistry()
    engines = {
        "engine-a": SimpleReportingEngine(),
        "engine-b": SimpleReportingEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimpleReportingEngine]]:
            return [("dup", SimpleReportingEngine()), ("dup", SimpleReportingEngine())]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(ReportingValidationError) as exc_info:
        registry.register_many({"existing": SimpleReportingEngine()})
    assert exc_info.value.error_code == "REP_REG_DUPLICATE"
...
def test_get_rejects_unknown_name() -> None:
    """get() raises REP_REG_UNKNOWN for unregistered names."""
    registry = ReportingEngineRegistry()
    with pytest.raises(ReportingValidationError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimpleReportingEngine()
