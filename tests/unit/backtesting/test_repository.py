"""Unit tests for CQROS backtesting dataset repository."""
...
def _canonical_frame(
    *,
    ...
    """Build a canonical backtesting frame for repository tests."""
    return pl.DataFrame(
        {
            "symbol": [symbol],
...
# ---------------------------------------------------------------------------
# BacktestingPartitionRef dataclass
# ---------------------------------------------------------------------------


def test_partition_ref_is_frozen_dataclass() -> None:
    """BacktestingPartitionRef is a frozen immutable dataclass."""
    ref = BacktestingPartitionRef(
        manager=_MANAGER,
        exchange=_EXCHANGE,
        market=_MARKET,
        symbol=_SYMBOL,
...
def test_partition_ref_equality() -> None:
    """Two BacktestingPartitionRef instances with identical fields are equal."""
    ref1 = BacktestingPartitionRef(
        manager=_MANAGER,
        exchange=_EXCHANGE,
        market=_MARKET,
        symbol=_SYMBOL,
...
def test_save_exists_load_round_trip() -> None:
    ...
    repository = BacktestingRepository(StorageLayout(Path("/data")), store)
...
def test_parquet_store_path_contains_backtesting_directory(tmp_path: Path) -> None:
    """Saved partitions reside under the backtesting storage directory."""
    layout = StorageLayout(tmp_path)
    repository = BacktestingRepository(layout, ParquetStore())
    repository.save(
        _canonical_frame(),
    ...
    assert (tmp_path / STORAGE_DIR_BACKTESTING).is_dir()
...
def test_discover_managers_returns_empty_when_no_partitions(tmp_path: Path) -> None:
    """discover_managers returns empty tuple when the backtesting tier is absent."""
    repository = BacktestingRepository(StorageLayout(tmp_path), ParquetStore())
