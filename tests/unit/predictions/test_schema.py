"""Unit tests for the CQROS merged prediction dataset schema."""
...
from cqros.predictions.schema import COLUMN_DTYPES as COLUMN_DTYPES_DIRECT
from cqros.predictions.schema import (
    MERGED_PREDICTION_SCHEMA as MERGED_PREDICTION_SCHEMA_DIRECT,
)
from cqros.predictions.schema import METADATA_COLUMNS as METADATA_COLUMNS_DIRECT
from cqros.predictions.schema import PREDICTION_COLUMNS as PREDICTION_COLUMNS_DIRECT
from cqros.predictions.schema import (
    PRIMARY_KEY_COLUMNS as PRIMARY_KEY_COLUMNS_DIRECT,
)
from cqros.predictions.schema import REQUIRED_COLUMNS as REQUIRED_COLUMNS_DIRECT


def test_merged_prediction_schema_is_exported_from_package() -> None:
    ...
    assert MERGED_PREDICTION_SCHEMA is MERGED_PREDICTION_SCHEMA_DIRECT
    assert METADATA_COLUMNS is METADATA_COLUMNS_DIRECT
    assert PREDICTION_COLUMNS is PREDICTION_COLUMNS_DIRECT
...
def test_metadata_and_prediction_columns() -> None:
    """Metadata and prediction partitions expose the expected columns."""
    assert METADATA_COLUMNS == ("model_name", "model_version")
    assert PREDICTION_COLUMNS == ("prediction",)


def test_canonical_column_order() -> None:
    """Canonical order is primary key, metadata, then prediction."""
    assert CANONICAL_COLUMN_ORDER[:3] == PRIMARY_KEY_COLUMNS
    assert CANONICAL_COLUMN_ORDER[3:5] == METADATA_COLUMNS
    assert CANONICAL_COLUMN_ORDER[5:] == PREDICTION_COLUMNS
...
def test_required_columns_are_complete() -> None:
    """Required columns include the primary key and every prediction column."""
    assert set(PRIMARY_KEY_COLUMNS).issubset(set(REQUIRED_COLUMNS))
    assert set(METADATA_COLUMNS).issubset(set(REQUIRED_COLUMNS))
    assert set(PREDICTION_COLUMNS).issubset(set(REQUIRED_COLUMNS))
    assert len(REQUIRED_COLUMNS) == (
        len(PRIMARY_KEY_COLUMNS) + len(METADATA_COLUMNS) + len(PREDICTION_COLUMNS)
    )
...
def test_column_names_are_unique() -> None:
    ...
    assert set(PRIMARY_KEY_COLUMNS).isdisjoint(set(PREDICTION_COLUMNS))
    assert set(METADATA_COLUMNS).isdisjoint(set(PREDICTION_COLUMNS))
...
def test_column_dtypes_cover_canonical_order() -> None:
    ...
    assert COLUMN_DTYPES["model_name"] == pl.String
    assert COLUMN_DTYPES["model_version"] == pl.String
    assert COLUMN_DTYPES["prediction"] == pl.Float64


def test_merged_prediction_schema_matches_canonical_order_and_dtypes() -> None:
    """Polars schema preserves canonical order and expected dtypes."""
    assert MERGED_PREDICTION_SCHEMA.names() == list(CANONICAL_COLUMN_ORDER)
    for column in CANONICAL_COLUMN_ORDER:
        assert MERGED_PREDICTION_SCHEMA[column] == COLUMN_DTYPES[column]


def test_schema_identity() -> None:
    """Merged schema identity is stable across package and module imports."""
    assert MERGED_PREDICTION_SCHEMA is MERGED_PREDICTION_SCHEMA_DIRECT
    assert MERGED_PREDICTION_SCHEMA.names() == [
        "symbol",
        "timeframe",
        "open_time",
        "model_name",
...
def test_primary_key_precedes_prediction_columns() -> None:
    """Primary key columns occupy the leading positions of the schema."""
    assert MERGED_PREDICTION_SCHEMA.names()[:3] == list(PRIMARY_KEY_COLUMNS)
...
def test_exported_collections_are_immutable() -> None:
    ...
    assert isinstance(PREDICTION_COLUMNS, tuple)
