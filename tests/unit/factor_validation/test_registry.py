"""Unit tests for CQROS ``FactorValidationEngineRegistry``."""
...
def _registry_with(*names: str) -> FactorValidationEngineRegistry:
    """Build a registry pre-populated with SimpleFactorValidationEngine under each name."""
    registry = FactorValidationEngineRegistry()
    for name in names:
        registry.register(name, SimpleFactorValidationEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise FVAL_REG_NAME_BLANK."""
    registry = FactorValidationEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(FactorValidationError) as exc_info:
            registry.register(blank, SimpleFactorValidationEngine())
        assert exc_info.value.error_code == "FVAL_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement FactorValidationEngine raise FVAL_REG_NOT_ENGINE."""
    registry = FactorValidationEngineRegistry()
    with pytest.raises(FactorValidationError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "FVAL_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises FVAL_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(FactorValidationError) as exc_info:
        registry.register("simple", SimpleFactorValidationEngine())
    assert exc_info.value.error_code == "FVAL_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = FactorValidationEngineRegistry()
    engines = {
        "engine-a": SimpleFactorValidationEngine(),
        "engine-b": SimpleFactorValidationEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimpleFactorValidationEngine]]:
            return [
                ("dup", SimpleFactorValidationEngine()),
                ("dup", SimpleFactorValidationEngine()),
            ]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(FactorValidationError) as exc_info:
        registry.register_many({"existing": SimpleFactorValidationEngine()})
    assert exc_info.value.error_code == "FVAL_REG_DUPLICATE"
...
def test_register_many_is_atomic_on_bad_engine() -> None:
    ...
    with pytest.raises(FactorValidationError):
        registry.register_many(
            {
                "good": SimpleFactorValidationEngine(),
...
def test_get_rejects_unknown_name() -> None:
    """get() raises FVAL_REG_UNKNOWN for unregistered names."""
    registry = FactorValidationEngineRegistry()
    with pytest.raises(FactorValidationError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimpleFactorValidationEngine()
