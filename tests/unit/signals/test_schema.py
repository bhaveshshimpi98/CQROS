"""Unit tests for the CQROS merged signal dataset schema."""
...
from cqros.signals.schema import COLUMN_DTYPES as COLUMN_DTYPES_DIRECT
from cqros.signals.schema import (
    MERGED_SIGNAL_SCHEMA as MERGED_SIGNAL_SCHEMA_DIRECT,
)
from cqros.signals.schema import METADATA_COLUMNS as METADATA_COLUMNS_DIRECT
from cqros.signals.schema import (
    PRIMARY_KEY_COLUMNS as PRIMARY_KEY_COLUMNS_DIRECT,
)
from cqros.signals.schema import REQUIRED_COLUMNS as REQUIRED_COLUMNS_DIRECT
from cqros.signals.schema import SIGNAL_COLUMNS as SIGNAL_COLUMNS_DIRECT


def test_merged_signal_schema_is_exported_from_package() -> None:
    ...
    assert MERGED_SIGNAL_SCHEMA is MERGED_SIGNAL_SCHEMA_DIRECT
    ...
    assert SIGNAL_COLUMNS is SIGNAL_COLUMNS_DIRECT
...
def test_metadata_and_signal_columns() -> None:
    """Metadata and signal partitions expose the expected columns."""
    assert METADATA_COLUMNS == ("model_name", "model_version")
    assert SIGNAL_COLUMNS == ("signal",)


def test_canonical_column_order() -> None:
    """Canonical order is primary key, metadata, then signal."""
    assert CANONICAL_COLUMN_ORDER[:3] == PRIMARY_KEY_COLUMNS
    assert CANONICAL_COLUMN_ORDER[3:5] == METADATA_COLUMNS
    assert CANONICAL_COLUMN_ORDER[5:] == SIGNAL_COLUMNS
...
def test_required_columns_are_complete() -> None:
    """Required columns include the primary key and every signal column."""
    assert set(PRIMARY_KEY_COLUMNS).issubset(set(REQUIRED_COLUMNS))
    assert set(METADATA_COLUMNS).issubset(set(REQUIRED_COLUMNS))
    assert set(SIGNAL_COLUMNS).issubset(set(REQUIRED_COLUMNS))
    assert len(REQUIRED_COLUMNS) == (
        len(PRIMARY_KEY_COLUMNS) + len(METADATA_COLUMNS) + len(SIGNAL_COLUMNS)
    )
...
def test_column_names_are_unique() -> None:
    ...
    assert set(PRIMARY_KEY_COLUMNS).isdisjoint(set(SIGNAL_COLUMNS))
    assert set(METADATA_COLUMNS).isdisjoint(set(SIGNAL_COLUMNS))
...
def test_column_dtypes_cover_canonical_order() -> None:
    ...
    assert COLUMN_DTYPES["signal"] == pl.String


def test_merged_signal_schema_matches_canonical_order_and_dtypes() -> None:
    """Polars schema preserves canonical order and expected dtypes."""
    assert MERGED_SIGNAL_SCHEMA.names() == list(CANONICAL_COLUMN_ORDER)
    for column in CANONICAL_COLUMN_ORDER:
        assert MERGED_SIGNAL_SCHEMA[column] == COLUMN_DTYPES[column]


def test_schema_identity() -> None:
    """Merged schema identity is stable across package and module imports."""
    assert MERGED_SIGNAL_SCHEMA is MERGED_SIGNAL_SCHEMA_DIRECT
    assert MERGED_SIGNAL_SCHEMA.names() == [
        "symbol",
        "timeframe",
        "open_time",
        "model_name",
...
def test_primary_key_precedes_signal_columns() -> None:
    """Primary key columns occupy the leading positions of the schema."""
    assert MERGED_SIGNAL_SCHEMA.names()[:3] == list(PRIMARY_KEY_COLUMNS)
