"""Unit tests for CQROS ``MonitoringEngineRegistry``."""
...
def _registry_with(*names: str) -> MonitoringEngineRegistry:
    """Build a registry pre-populated with SimpleMonitoringEngine under each name."""
    registry = MonitoringEngineRegistry()
    for name in names:
        registry.register(name, SimpleMonitoringEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise MON_REG_NAME_BLANK."""
    registry = MonitoringEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(MonitoringValidationError) as exc_info:
            registry.register(blank, SimpleMonitoringEngine())
        assert exc_info.value.error_code == "MON_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement MonitoringEngine raise MON_REG_NOT_ENGINE."""
    registry = MonitoringEngineRegistry()
    with pytest.raises(MonitoringValidationError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "MON_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises MON_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(MonitoringValidationError) as exc_info:
        registry.register("simple", SimpleMonitoringEngine())
    assert exc_info.value.error_code == "MON_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = MonitoringEngineRegistry()
    engines = {
        "engine-a": SimpleMonitoringEngine(),
        "engine-b": SimpleMonitoringEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimpleMonitoringEngine]]:
            return [("dup", SimpleMonitoringEngine()), ("dup", SimpleMonitoringEngine())]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(MonitoringValidationError) as exc_info:
        registry.register_many({"existing": SimpleMonitoringEngine()})
    assert exc_info.value.error_code == "MON_REG_DUPLICATE"
...
def test_get_rejects_unknown_name() -> None:
    """get() raises MON_REG_UNKNOWN for unregistered names."""
    registry = MonitoringEngineRegistry()
    with pytest.raises(MonitoringValidationError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimpleMonitoringEngine()
