"""Unit tests for CQROS monitoring dataset repository."""
...
from cqros.monitoring import MonitoringRepository, MonitoringStatus, MonitoringValidationError
from cqros.monitoring.repository import MonitoringPartitionRef
from cqros.monitoring.schema import CANONICAL_COLUMN_ORDER, COLUMN_DTYPES, MONITORING_SCHEMA
...
def _canonical_frame(
    *,
    ...
    """Build a canonical monitoring frame for repository tests."""
    return pl.DataFrame(
        {
            "symbol": [symbol],
...
# ---------------------------------------------------------------------------
# MonitoringPartitionRef dataclass
# ---------------------------------------------------------------------------


def test_partition_ref_is_frozen_dataclass() -> None:
    """MonitoringPartitionRef is a frozen immutable dataclass."""
    ref = MonitoringPartitionRef(
        manager=_MANAGER,
        exchange=_EXCHANGE,
        market=_MARKET,
        symbol=_SYMBOL,
...
def test_partition_ref_equality() -> None:
    """Two MonitoringPartitionRef instances with identical fields are equal."""
    ref1 = MonitoringPartitionRef(
        manager=_MANAGER,
        exchange=_EXCHANGE,
        market=_MARKET,
        symbol=_SYMBOL,
...
def test_save_exists_load_round_trip() -> None:
    ...
    repository = MonitoringRepository(StorageLayout(Path("/data")), store)
    ...
    assert loaded.schema == MONITORING_SCHEMA
...
def test_save_overwrites_existing_partition() -> None:
    ...
    frame_v1 = _canonical_frame(monitor_name="report_monitor")
    repository.save(frame_v1, **kwargs)  # type: ignore[arg-type]

    frame_v2 = _canonical_frame(monitor_name="risk_monitor")
    repository.save(frame_v2, **kwargs)  # type: ignore[arg-type]

    loaded = repository.load(**kwargs)  # type: ignore[arg-type]
    assert loaded["monitor_name"].to_list() == ["risk_monitor"]


def test_save_rejects_non_dataframe() -> None:
    """save() rejects non-DataFrame inputs with MON_REPO_FRAME_TYPE."""
    repository = MonitoringRepository(StorageLayout(Path("/data")), _InMemoryDataStore())
    ...
    assert exc_info.value.error_code == "MON_REPO_FRAME_TYPE"
...
def test_save_rejects_missing_required_columns() -> None:
    ...
    assert exc_info.value.error_code == "MON_REPO_MISSING_COLUMNS"


def test_save_rejects_schema_cast_failure() -> None:
    """save() rejects frames that cannot cast to MONITORING_SCHEMA."""
    repository = MonitoringRepository(StorageLayout(Path("/data")), _InMemoryDataStore())
    frame = _canonical_frame().with_columns(pl.lit("not-a-float").alias("metric_value"))
    with pytest.raises(MonitoringValidationError) as exc_info:
        repository.save(frame, **_partition_kwargs())  # type: ignore[arg-type]
    assert exc_info.value.error_code == "MON_REPO_SCHEMA_CAST"


def test_load_casts_to_monitoring_schema() -> None:
    """load() returns frames cast to MONITORING_SCHEMA."""
    store = _InMemoryDataStore()
    repository = MonitoringRepository(StorageLayout(Path("/data")), store)
    kwargs = _partition_kwargs()
...
def test_parquet_store_path_contains_monitoring_directory(tmp_path: Path) -> None:
    """Saved partitions reside under the monitoring storage directory."""
    layout = StorageLayout(tmp_path)
    repository = MonitoringRepository(layout, ParquetStore())
    repository.save(_canonical_frame(), **_partition_kwargs())  # type: ignore[arg-type]
    assert (tmp_path / STORAGE_DIR_MONITORING).is_dir()
...
def test_discover_managers_returns_empty_when_no_partitions(tmp_path: Path) -> None:
    """discover_managers returns empty tuple when the monitoring tier is absent."""
