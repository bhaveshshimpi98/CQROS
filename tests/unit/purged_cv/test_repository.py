"""Unit tests for CQROS purged-CV dataset repository."""
...
from cqros.purged_cv import PurgedCVError, PurgedCVRepository, PurgedCVStatus
...
def _canonical_frame(
    *,
    ...
    """Build a canonical purged-CV frame for repository tests."""
    return pl.DataFrame(
        {
...
# ---------------------------------------------------------------------------
# PurgedCVPartitionRef dataclass
# ---------------------------------------------------------------------------


def test_partition_ref_is_frozen_dataclass() -> None:
    """PurgedCVPartitionRef is a frozen immutable dataclass."""
    ref = PurgedCVPartitionRef(
        manager=_MANAGER,
        exchange=_EXCHANGE,
        market=_MARKET,
...
def test_partition_ref_equality() -> None:
    """Two PurgedCVPartitionRef instances with identical fields are equal."""
    ref1 = PurgedCVPartitionRef(
        manager=_MANAGER,
        exchange=_EXCHANGE,
        market=_MARKET,
        timeframe=_TIMEFRAME,
...
def test_save_exists_load_round_trip() -> None:
    ...
    repository = PurgedCVRepository(StorageLayout(Path("/data")), store)
    ...
    assert loaded.schema == PURGED_CV_SCHEMA
...
def test_save_rejects_non_dataframe() -> None:
    """save() rejects non-DataFrame inputs with PCV_REPO_FRAME_TYPE."""
    repository = PurgedCVRepository(StorageLayout(Path("/data")), _InMemoryDataStore())
    with pytest.raises(PurgedCVError) as exc_info:
        repository.save(
            "not-a-frame",  # type: ignore[arg-type]
            **_partition_kwargs(),  # type: ignore[arg-type]
        )
    assert exc_info.value.error_code == "PCV_REPO_FRAME_TYPE"
...
def test_save_rejects_missing_required_columns() -> None:
    ...
    assert exc_info.value.error_code == "PCV_REPO_MISSING_COLUMNS"


def test_save_rejects_schema_cast_failure() -> None:
    """save() rejects frames that cannot cast to PURGED_CV_SCHEMA."""
    repository = PurgedCVRepository(StorageLayout(Path("/data")), _InMemoryDataStore())
    ...
    assert exc_info.value.error_code == "PCV_REPO_SCHEMA_CAST"


def test_load_casts_to_purged_cv_schema() -> None:
    """load() returns frames cast to PURGED_CV_SCHEMA."""
    store = _InMemoryDataStore()
    repository = PurgedCVRepository(StorageLayout(Path("/data")), store)
    kwargs = _partition_kwargs()
...
def test_parquet_store_path_contains_purged_cv_directory(tmp_path: Path) -> None:
    """Saved partitions reside under the purged-CV storage directory."""
    layout = StorageLayout(tmp_path)
    repository = PurgedCVRepository(layout, ParquetStore())
    repository.save(_canonical_frame(), **_partition_kwargs())  # type: ignore[arg-type]
    assert (tmp_path / STORAGE_DIR_PURGED_CV).is_dir()
...
def test_discover_managers_returns_empty_when_no_partitions(tmp_path: Path) -> None:
    """discover_managers returns empty tuple when the purged-CV tier is absent."""
