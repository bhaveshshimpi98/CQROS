"""Unit tests for CQROS ``PurgedCVEngineRegistry``."""
...
def _registry_with(*names: str) -> PurgedCVEngineRegistry:
    """Build a registry pre-populated with SimplePurgedCVEngine under each name."""
    registry = PurgedCVEngineRegistry()
    for name in names:
        registry.register(name, SimplePurgedCVEngine())
    return registry
...
def test_register_rejects_blank_name() -> None:
    """Blank or whitespace-only names raise PCV_REG_NAME_BLANK."""
    registry = PurgedCVEngineRegistry()
    for blank in ("", "   ", "\t"):
        with pytest.raises(PurgedCVError) as exc_info:
            registry.register(blank, SimplePurgedCVEngine())
        assert exc_info.value.error_code == "PCV_REG_NAME_BLANK"


def test_register_rejects_non_engine_object() -> None:
    """Objects that do not implement PurgedCVEngine raise PCV_REG_NOT_ENGINE."""
    registry = PurgedCVEngineRegistry()
    with pytest.raises(PurgedCVError) as exc_info:
        registry.register("bad", "not-an-engine")  # type: ignore[arg-type]
    assert exc_info.value.error_code == "PCV_REG_NOT_ENGINE"


def test_register_rejects_duplicate_name() -> None:
    """Re-registering an existing name raises PCV_REG_DUPLICATE."""
    registry = _registry_with("simple")
    with pytest.raises(PurgedCVError) as exc_info:
        registry.register("simple", SimplePurgedCVEngine())
    assert exc_info.value.error_code == "PCV_REG_DUPLICATE"
...
def test_register_many_adds_all_engines() -> None:
    ...
    registry = PurgedCVEngineRegistry()
    engines = {
        "engine-a": SimplePurgedCVEngine(),
        "engine-b": SimplePurgedCVEngine(),
    }
...
def test_register_many_rejects_duplicate_within_mapping() -> None:
    ...
    class _DuplicateMapping:
        ...
        def items(self) -> list[tuple[str, SimplePurgedCVEngine]]:
            return [
                ("dup", SimplePurgedCVEngine()),
                ("dup", SimplePurgedCVEngine()),
            ]
...
def test_register_many_rejects_already_registered_name() -> None:
    ...
    with pytest.raises(PurgedCVError) as exc_info:
        registry.register_many({"existing": SimplePurgedCVEngine()})
    assert exc_info.value.error_code == "PCV_REG_DUPLICATE"
...
def test_register_many_is_atomic_on_bad_engine() -> None:
    ...
    registry = PurgedCVEngineRegistry()
    with pytest.raises(PurgedCVError):
        registry.register_many(
            {
                "good": SimplePurgedCVEngine(),
...
def test_get_rejects_unknown_name() -> None:
    """get() raises PCV_REG_UNKNOWN for unregistered names."""
    registry = PurgedCVEngineRegistry()
    with pytest.raises(PurgedCVError) as exc_info:
...
def test_clear_allows_re_registration() -> None:
    ...
    registry = _registry_with("simple")
    registry.clear()
    new_engine = SimplePurgedCVEngine()
