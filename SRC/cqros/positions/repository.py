"""CQROS position dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    position year partitions produced by CQROS Position Engine accounting.

Responsibilities:
    - Resolve storage locations for position year partitions via
      ``StorageLayout.position_path``
    - Persist, load, check existence, and delete position partitions
    ...
    - Remain free of accounting, schema validation, and trading logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``PositionPartitionRef``, ``PositionRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical position
    schema validation belongs to ``PositionPipeline`` and
    ``PositionVerifier``. This module must not import position engines.
"""
...
class PositionPartitionRef:
    """Identity of one discovered position year partition.
...
class PositionRepository:
    ...
    def discover_managers(self) -> tuple[str, ...]:
        """Return sorted manager identifiers with at least one position partition.

        Returns:
            Deterministically ordered manager identifiers discovered on disk.
        """
        base = self._positions_root()
        if not base.is_dir():
            return ()
    ...
    def discover_symbols(
        ...
        *,
        ...
        """Return sorted symbols with at least one position partition for ``manager``.
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as position parquet partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[PositionPartitionRef, ...]:
        """Discover position year partitions matching optional filters.

        Missing position trees are skipped. Only year partitions that exist
        as ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the positions tier.
        ...
        items: list[PositionPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a position year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.position_path``. Parquet contents
        ...
        Returns:
            ``True`` when the yearly position partition exists; otherwise
            ``False``.
        """
        path = self._layout.position_path(
            manager,
            exchange,
            market,
            symbol,
        ...
        self._logger.debug(
            "Position dataset exists" if present else "Position dataset does not exist",
            extra=_position_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def save(
        ...
        *,
        ...
        """Persist a position year partition.

        Args:
            dataframe: Position frame to store.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
    ...
    def load(
        ...
        *,
        ...
        """Load a position year partition.
        ...
        Returns:
            Eagerly loaded position DataFrame.
        """
        path = self._layout.position_path(
            manager,
            exchange,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a position year partition.
    ...
    def _positions_root(self) -> Path:
        """Return the positions directory under the storage root."""
        return self._layout.root / STORAGE_DIR_POSITIONS
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the positions directory for manager/exchange/market."""
        return self._positions_root() / manager / exchange / market
...
def _position_log_extra(
    *,
    ...
    """Build structured log fields for a position dataset operation."""
