"""CQROS factor combination metrics dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    factor combination year partitions produced by the CQROS Factor
    Combination Engine.

Responsibilities:
    - Resolve storage locations for factor combination year partitions via
      ``StorageLayout.factor_combination_path``
    - Persist, load, check existence, and discover factor combination
      partitions
    - Validate frames against ``FACTOR_COMBINATION_SCHEMA`` before save
    - Cast loaded frames to ``FACTOR_COMBINATION_SCHEMA`` after read
    - Discover managers, timeframes, and year partitions under the
      factor combination tier
    ...
    - Remain free of combination math and trading logic

Dependencies:
    ``polars``, ``cqros.core``, ``cqros.factor_combination.exceptions``,
    ``cqros.factor_combination.schema``, and ``cqros.storage``
    layout/interfaces.

Public API:
    ``FactorCombinationPartitionRef``, ``FactorCombinationRepository``

Notes:
    Schema validation and casting keep the on-disk contract aligned with
    ``FACTOR_COMBINATION_SCHEMA``. Combination computation belongs to the
    engine and pipeline. This module must not import factor combination
...
_ERROR_FRAME_TYPE: Final[str] = "FCOMB_REPO_FRAME_TYPE"
_ERROR_MISSING_COLUMNS: Final[str] = "FCOMB_REPO_MISSING_COLUMNS"
_ERROR_SCHEMA_CAST: Final[str] = "FCOMB_REPO_SCHEMA_CAST"


@dataclass(frozen=True, slots=True)
class FactorCombinationPartitionRef:
    """Identity of one discovered factor combination year partition.
...
class FactorCombinationRepository:
    """Repository facade for canonical factor combination datasets.

    Callers identify partitions by manager, exchange, market, timeframe, and
    year. Paths are composed privately via
    ``StorageLayout.factor_combination_path`` and never returned. Persistence
    ...
    ``FACTOR_COMBINATION_SCHEMA`` on save and load.
    ...
    def discover_managers(self) -> tuple[str, ...]:
        ...
        """
        base = self._factor_combination_root()
        if not base.is_dir():
            return ()
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as factor combination partitions.
    ...
    def discover(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[FactorCombinationPartitionRef, ...]:
        """Discover factor combination year partitions matching optional filters.

        Missing factor combination trees are skipped. Only year partitions that
        exist as ``{year}.parquet`` files are included. Paths are never
        returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the factor combination tier.
    ...
    def discover_partitions(
        ...
        *,
        ...
        items: list[FactorCombinationPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a factor combination year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.factor_combination_path``. Parquet
        ...
        Returns:
            ``True`` when the yearly factor combination partition exists;
            otherwise ``False``.
        """
        path = self._layout.factor_combination_path(
            manager,
            exchange,
            market,
            timeframe,
        ...
        self._logger.debug(
            (
                "Factor combination dataset exists"
                if present
                else "Factor combination dataset does not exist"
            ),
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                timeframe=timeframe,
    ...
    def save(
        ...
        *,
        ...
        """Persist a factor combination year partition after schema validation.

        Args:
            dataframe: Factor combination frame to store. Must conform to
                ``FACTOR_COMBINATION_SCHEMA``.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
        ...
        Raises:
            FactorCombinationError: If ``dataframe`` is not a DataFrame,
                required columns are missing, or casting to
                ``FACTOR_COMBINATION_SCHEMA`` fails.
        """
        validated = _require_factor_combination_schema(dataframe)
        path = self._layout.factor_combination_path(
            manager,
            exchange,
            market,
        ...
        self._logger.debug(
            "Saving factor combination dataset",
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
        ...
        self._logger.info(
            "Saved factor combination dataset",
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
    ...
    def load(
        ...
        *,
        ...
        """Load a factor combination partition cast to ``FACTOR_COMBINATION_SCHEMA``.

        When the partition does not exist, returns an empty DataFrame with
        ``FACTOR_COMBINATION_SCHEMA``. Never returns ``None``.
        ...
        Returns:
            Eagerly loaded factor combination DataFrame cast to
            ``FACTOR_COMBINATION_SCHEMA``, or an empty schema-conformant
            frame when the partition is absent.

        Raises:
            FactorCombinationError: If the loaded frame fails schema
        ...
        self._logger.debug(
            "Loading factor combination dataset",
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
        ...
        if not self._datastore.exists(path):
            ...
            self._logger.info(
                "Factor combination dataset missing; returning empty frame",
                extra=_factor_combination_log_extra(
                    manager=manager,
                    exchange=exchange,
                    market=market,
        ...
        validated = _require_factor_combination_schema(frame)
        self._logger.info(
            "Loaded factor combination dataset",
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a factor combination year partition.
        ...
        self._logger.debug(
            "Deleting factor combination dataset",
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
        ...
        if not self._datastore.exists(path):
            self._logger.debug(
                "Factor combination dataset already absent",
                extra=_factor_combination_log_extra(
                    manager=manager,
                    exchange=exchange,
                    market=market,
        ...
        self._logger.info(
            "Deleted factor combination dataset",
            extra=_factor_combination_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
    ...
    def _factor_combination_root(self) -> Path:
        """Return the factor combination directory under the storage root."""
        return self._layout.root / STORAGE_DIR_FACTOR_COMBINATION
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the factor combination directory for manager/exchange/market."""
        return self._factor_combination_root() / manager / exchange / market
...
def _require_factor_combination_schema(frame: object) -> pl.DataFrame:
    """Validate and cast ``frame`` to ``FACTOR_COMBINATION_SCHEMA``."""
    ...
    if missing:
        raise FactorCombinationError(
            "factor combination schema is missing required columns",
    ...
    try:
        return frame.select(list(FACTOR_COMBINATION_COLUMNS)).cast(FACTOR_COMBINATION_SCHEMA)
    except (pl.exceptions.PolarsError, TypeError, ValueError) as exc:
        raise FactorCombinationError(
            "factor combination frame failed FACTOR_COMBINATION_SCHEMA cast",
            error_code=_ERROR_SCHEMA_CAST,
            details={"reason": str(exc)},
        ) from exc


def _factor_combination_log_extra(
    *,
    manager: str,
    exchange: Exchange,
    market: Market,
    ...
    """Build structured log fields for a factor combination dataset operation."""
    payload: dict[str, object] = {
        "tier": "factor_combination",
        "manager": manager,
        "exchange": exchange,
        "market": market,
        "timeframe": timeframe,
