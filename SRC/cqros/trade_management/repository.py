"""CQROS trade management decision dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    trade-management year partitions produced by CQROS Trade Management
    Engine evaluation.

Responsibilities:
    - Resolve storage locations for trade-management year partitions via
      ``StorageLayout.trade_management_path``
    - Persist, load, check existence, and delete trade-management partitions
    - Discover managers, symbols, timeframes, and year partitions under the
      trade-management tier
    ...
    - Remain free of trade-management math, schema validation, and trading
      logic
...
Public API:
    ``TradeManagementPartitionRef``, ``TradeManagementRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical
    trade-management schema validation belongs to ``TradeManagementPipeline``
    and ``TradeManagementVerifier``. This module must not import
    trade-management managers. Writes overwrite existing partitions when the
    datastore permits.
"""
...
class TradeManagementPartitionRef:
    """Identity of one discovered trade-management year partition.
...
class TradeManagementRepository:
    """Repository facade for canonical trade-management datasets.

    Callers identify partitions by manager, exchange, market, symbol,
    timeframe, and year. Paths are composed privately via
    ``StorageLayout.trade_management_path`` and never returned. Persistence
    ...
    def discover_managers(self) -> tuple[str, ...]:
        ...
        """
        base = self._trade_management_root()
        if not base.is_dir():
            return ()
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as trade-management partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[TradeManagementPartitionRef, ...]:
        """Discover trade-management year partitions matching optional filters.

        Missing trade-management trees are skipped. Only year partitions that
        exist as ``{year}.parquet`` files are included. Paths are never
        returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the trade-management tier.
        ...
        items: list[TradeManagementPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a trade-management year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.trade_management_path``. Parquet
        ...
        Returns:
            ``True`` when the yearly trade-management partition exists;
            otherwise ``False``.
        """
        path = self._layout.trade_management_path(
            manager,
            exchange,
            market,
            symbol,
        ...
        self._logger.debug(
            (
                "Trade-management dataset exists"
                if present
                else "Trade-management dataset does not exist"
            ),
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def save(
        ...
        *,
        ...
        """Persist a trade-management year partition, overwriting when present.

        Args:
            dataframe: Trade-management frame to store.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
        ...
        self._logger.debug(
            "Saving trade-management dataset",
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
        ...
        self._logger.info(
            "Saved trade-management dataset",
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
    ...
    def load(
        ...
        *,
        ...
        """Load a trade-management year partition.
        ...
        Returns:
            Eagerly loaded trade-management DataFrame.
        """
        path = self._layout.trade_management_path(
            manager,
            exchange,
        ...
        self._logger.debug(
            "Loading trade-management dataset",
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
        ...
        self._logger.info(
            "Loaded trade-management dataset",
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a trade-management year partition.
        ...
        self._logger.debug(
            "Deleting trade-management dataset",
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
        ...
        self._logger.info(
            "Deleted trade-management dataset",
            extra=_trade_management_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
    ...
    def _trade_management_root(self) -> Path:
        """Return the trade-management directory under the storage root."""
        return self._layout.root / STORAGE_DIR_TRADE_MANAGEMENT
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the trade-management directory for manager/exchange/market."""
        return self._trade_management_root() / manager / exchange / market
...
def _trade_management_log_extra(
    *,
    manager: str,
    exchange: Exchange,
    market: Market,
    ...
    """Build structured log fields for a trade-management dataset operation."""
    payload: dict[str, object] = {
        "tier": "trade_management",
        "manager": manager,
        "exchange": exchange,
        "market": market,
        "symbol": symbol,
