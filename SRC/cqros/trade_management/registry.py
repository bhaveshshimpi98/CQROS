"""CQROS Trade Management Engine registry.

Purpose:
    Provide the authoritative in-memory catalog of available trade management
    manager implementations for registration and lookup.

Responsibilities:
    - Register ``TradeManagementManager`` instances by unique name
    - Provide deterministic lookup, existence checks, and ordered listing
    - Reject duplicates, blank names, and objects that do not implement
      ``TradeManagementManager``
    - Remain free of trade-management math, persistence, and trading

Dependencies:
    ``cqros.trade_management.exceptions`` and ``cqros.trade_management.manager``.

Public API:
    ``TradeManagementManagerRegistry``

Notes:
    This registry is not thread-safe. Callers that share one instance across
    threads must provide their own synchronization.
"""

from __future__ import annotations

from collections.abc import Mapping
from typing import Final

from cqros.trade_management.exceptions import TradeManagementValidationError
from cqros.trade_management.manager import TradeManagementManager

__all__ = ["TradeManagementManagerRegistry"]

_ERROR_NOT_MANAGER: Final[str] = "TME_REG_NOT_MANAGER"
_ERROR_NAME_BLANK: Final[str] = "TME_REG_NAME_BLANK"
_ERROR_DUPLICATE: Final[str] = "TME_REG_DUPLICATE"
_ERROR_UNKNOWN: Final[str] = "TME_REG_UNKNOWN"


class TradeManagementManagerRegistry:
    """Authoritative catalog of registered CQROS trade management managers.

    Managers are indexed by caller-supplied unique names. The registry stores
    references to the supplied ``TradeManagementManager`` instances and never
    mutates, instantiates, or invokes them. Returned name collections are new
    tuples and do not expose the internal mapping. Insertion order is
    preserved.

    Notes:
        This registry is not thread-safe. Concurrent mutation from multiple
        threads requires external synchronization.
    """

    __slots__ = ("_managers",)

    def __init__(self) -> None:
        """Initialize an empty trade management manager registry."""
        self._managers: dict[str, TradeManagementManager] = {}

    def register(self, name: str, manager: TradeManagementManager) -> None:
        """Register one trade management manager under ``name``.

        Args:
            name: Unique registry key for the manager.
            manager: Trade management manager instance to register. Must not be
                mutated by the registry after registration.

        Raises:
            TradeManagementValidationError: If ``name`` is blank, ``manager``
                does not implement ``TradeManagementManager``, or a name is
                already registered.
        """
        validated_name = _require_name(name)
        validated_manager = _require_manager(manager)
        if validated_name in self._managers:
            raise TradeManagementValidationError(
                f"manager already registered: {validated_name}",
                error_code=_ERROR_DUPLICATE,
                details={"name": validated_name},
            )
        self._managers[validated_name] = validated_manager

    def register_many(self, mapping: Mapping[str, TradeManagementManager]) -> None:
        """Register multiple trade management managers atomically.

        Either every entry in ``mapping`` is registered, or the registry
        remains unchanged when any registration would fail.

        Args:
            mapping: Mapping of unique names to trade management manager
                instances.

        Raises:
            TradeManagementValidationError: If any name is blank, already
                registered, duplicated within ``mapping``, or any value does
                not implement ``TradeManagementManager``.
        """
        pending: dict[str, TradeManagementManager] = {}
        for name, manager in mapping.items():
            validated_name = _require_name(name)
            validated_manager = _require_manager(manager)
            if validated_name in self._managers or validated_name in pending:
                raise TradeManagementValidationError(
                    f"manager already registered: {validated_name}",
                    error_code=_ERROR_DUPLICATE,
                    details={"name": validated_name},
                )
            pending[validated_name] = validated_manager
        self._managers.update(pending)

    def get(self, name: str) -> TradeManagementManager:
        """Return the registered trade management manager for ``name``.

        Args:
            name: Manager name to look up.

        Returns:
            The registered trade management manager instance.

        Raises:
            TradeManagementValidationError: If no manager is registered under
                ``name``.
        """
        manager = self._managers.get(name)
        if manager is None:
            raise TradeManagementValidationError(
                f"manager not registered: {name}",
                error_code=_ERROR_UNKNOWN,
                details={"name": name},
            )
        return manager

    def exists(self, name: str) -> bool:
        """Return whether a manager is registered under ``name``.

        Args:
            name: Manager name to check.

        Returns:
            ``True`` when the name is registered, otherwise ``False``.
        """
        return name in self._managers

    def list(self) -> tuple[str, ...]:
        """Return registered manager names in insertion order.

        Returns:
            A new tuple of registered names.
        """
        return tuple(self._managers)

    def clear(self) -> None:
        """Remove all registered managers."""
        self._managers.clear()


def _require_name(name: object) -> str:
    """Validate and return a non-blank manager registry name."""
    if not isinstance(name, str) or name.strip() == "":
        raise TradeManagementValidationError(
            "manager name must be a non-blank string",
            error_code=_ERROR_NAME_BLANK,
            details={"name": name},
        )
    return name


def _require_manager(manager: object) -> TradeManagementManager:
    """Validate that ``manager`` implements ``TradeManagementManager``."""
    if not isinstance(manager, TradeManagementManager):
        raise TradeManagementValidationError(
            "manager must implement the TradeManagementManager protocol",
            error_code=_ERROR_NOT_MANAGER,
            details={"value_type": type(manager).__name__},
        )
    return manager
