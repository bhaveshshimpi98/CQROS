"""CQROS Models Engine exception hierarchy.

Purpose:
    Provide model-specific exception types used by engines, pipelines,
    repositories, and verification workflows.

Responsibilities:
    - Define the package ``ModelException`` root under ``ResearchError``
    - Expose ``ModelError`` for input and contract validation
...
class ModelException(ResearchError):  # noqa: N818
    """Raised when model workflows or artifacts fail."""

    __slots__ = ()


class ModelError(ModelException):
    """Raised when model inputs, outputs, or contracts fail."""
