"""CQROS accounting dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers accounting
    partitions and executes ``AccountingVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for accounting dataset verification
    - Discover available accounting partitions through ``AccountingRepository``
    - Load partitions and verify them with ``AccountingVerifier``
...
Dependencies:
    ...
    ``cqros.accounting``, and ``cqros.storage``.
...
__all__ = [
    ...
    "DiscoveredWorkItem",
    "VerifyAccountingOptions",
    "VerifyAccountingSummary",
    "build_options",
    "build_parser",
    "discover_work",
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-ACCOUNTING-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-ACCOUNTING-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-ACCOUNTING-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-ACCOUNTING-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-ACCOUNTING-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-ACCOUNTING-006"

_VERIFIER_NAME: Final[str] = "AccountingVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Accounting"
...
class VerifyAccountingOptions:
    """Immutable CLI options for accounting dataset verification.

    Attributes:
        storage_root: Storage root containing ``accounting``.
...
class DiscoveredWorkItem:
    """One discovered accounting partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyAccountingSummary:
    """Immutable aggregate summary for an accounting verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always accounting tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the accounting dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-accounting",
        description=(
            "Verify CQROS accounting datasets across the discovered " "accounting universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the accounting tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded accounting rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded accounting rows. "
...
def build_options(args: argparse.Namespace) -> VerifyAccountingOptions:
    """Map parsed CLI arguments onto ``VerifyAccountingOptions``.
    ...
    return VerifyAccountingOptions(
...
def discover_work(
    ...
    options: VerifyAccountingOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover accounting partitions matching the CLI filters.

    Args:
        repository: Accounting repository providing discovery APIs.
...
def format_summary(summary: VerifyAccountingSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the accounting dataset verification CLI."""
    parser = build_parser()
...
async def run_verification(
    *,
    ...
    if len(work) == 0:
        return VerifyAccountingSummary(
            symbols_verified=0,
            datasets_verified=0,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-accounting-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> AccountingTaskResult | None:
    """Verify one accounting year partition synchronously."""
    try:
...
def _report_task_failure(result: AccountingTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed accounting partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
