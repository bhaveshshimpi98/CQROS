"""CQROS OMS order dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers order
    partitions and executes ``OrderVerifier`` across the universe with bounded
    symbol concurrency.

Responsibilities:
    - Parse CLI arguments for OMS order dataset verification
    - Discover available order partitions through ``OrderRepository``
    - Load partitions and verify them with ``OrderVerifier``
...
Dependencies:
    ...
    ``cqros.oms.verification``, and ``cqros.storage``.
...
Notes:
    ...
    logic, OMS business rules, lifecycle validation, or repository filesystem
    walks beyond calling repository discovery and load APIs. Optional
    ``--manager`` filters discovery by manager path segment. Optional
    ``--policy`` / ``--model`` / ``--version`` filters restrict verification to
    matching identity rows after load when those columns are present.
"""
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-ORDERS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-ORDERS-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-ORDERS-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-ORDERS-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-ORDERS-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-ORDERS-006"
_ERROR_POLICY: Final[str] = "CLI-VERIFY-ORDERS-007"

_VERIFIER_NAME: Final[str] = "OrderVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Orders"

_COL_MODEL_NAME: Final[str] = "model_name"
_COL_MODEL_VERSION: Final[str] = "model_version"
_COL_POLICY: Final[str] = "policy"
_COL_MANAGER: Final[str] = "manager"

# Attribution keys matching OrderVerifier warning text. VerificationReport
# exposes only the combined ``invalid_numeric_rows`` counter, so the CLI maps
# that union onto summary categories when the corresponding warning is present.
...
class VerifyOrderOptions:
    """Immutable CLI options for OMS order dataset verification.

    Attributes:
        storage_root: Storage root containing ``orders``.
        manager: Optional manager allowlist identity. ``None`` discovers all.
        policy: Optional policy-name allowlist identity. ``None`` keeps all.
...
class DiscoveredWorkItem:
    """One discovered order partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyOrderSummary:
    """Immutable aggregate summary for an order verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always orders tier).
        ...
        invalid_side_rows: Sum of attributed invalid-side counters.
        invalid_order_type_rows: Sum of attributed invalid order-type counters.
        invalid_status_rows: Sum of attributed invalid-status counters.
        invalid_quantity_rows: Sum of attributed invalid-quantity counters.
        invalid_filled_quantity_rows: Sum of attributed invalid filled-quantity
            counters.
        invalid_average_fill_price_rows: Sum of attributed invalid average-fill
            price counters.
        invalid_limit_price_rows: Sum of attributed invalid limit-price
            counters.
        invalid_stop_price_rows: Sum of attributed invalid stop-price counters.
...
def build_parser() -> argparse.ArgumentParser:
    """Create the OMS order dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-orders",
        description=("Verify CQROS OMS order datasets across the discovered " "orders universe."),
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the orders tier."
        ),
    )
    parser.add_argument(
        "--policy",
        ...
        metavar="NAME",
        help=(
            "Optional policy filter applied to loaded order rows. "
            "Omit to verify all policies present in each partition."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded order rows. "
            "Omit to verify all models present in each partition."
        ),
    )
    parser.add_argument(
        "--version",
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded order rows. "
            "Omit to verify all versions present in each partition."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyOrderOptions:
    """Map parsed CLI arguments onto ``VerifyOrderOptions``.
    ...
    policy = _normalize_optional_identity(
        args.policy,
        parameter="policy",
        error_code=_ERROR_POLICY,
    )
    model = _normalize_optional_identity(args.model, parameter="model", error_code=_ERROR_MODEL)
...
def discover_work(
    ...
    """Discover order partitions matching the CLI filters.

    Args:
        repository: Order repository providing discovery APIs.
...
def format_partition_failure(
    *,
    ...
    Args:
        ...
        code: Optional CQROS / OMS validation error code.
...
def format_summary(summary: VerifyOrderSummary) -> str:
    ...
    lines = [
        ...
        "",
        f"Invalid side rows: {summary.invalid_side_rows}",
        f"Invalid order type rows: {summary.invalid_order_type_rows}",
        f"Invalid status rows: {summary.invalid_status_rows}",
        "",
        f"Invalid quantity rows: {summary.invalid_quantity_rows}",
        f"Invalid filled quantity rows: {summary.invalid_filled_quantity_rows}",
        ("Invalid average fill price rows: " f"{summary.invalid_average_fill_price_rows}"),
        f"Invalid limit price rows: {summary.invalid_limit_price_rows}",
        f"Invalid stop price rows: {summary.invalid_stop_price_rows}",
        "",
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the OMS order dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Injected order repository.
        verifier: Injected order verifier.
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-orders-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> OrderTaskResult | None:
    """Verify one order year partition synchronously.

    Returns:
        A task result, or ``None`` when identity filters exclude every row in
        the partition.
    """
    try:
...
def _apply_identity_filters(
    ...
    *,
    ...
    """Restrict ``frame`` to optional manager/policy/model/version filters."""
    filtered = frame
    if manager is not None and _COL_MANAGER in filtered.columns:
        filtered = filtered.filter(pl.col(_COL_MANAGER) == manager)
    if policy is not None and _COL_POLICY in filtered.columns:
        filtered = filtered.filter(pl.col(_COL_POLICY) == policy)
    if model is not None and _COL_MODEL_NAME in filtered.columns:
        filtered = filtered.filter(pl.col(_COL_MODEL_NAME) == model)
    if version is not None and _COL_MODEL_VERSION in filtered.columns:
        filtered = filtered.filter(pl.col(_COL_MODEL_VERSION) == version)
    return filtered


def _report_task_failure(result: OrderTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed order partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _attribute_invalid_rows(
    report: VerificationReport,
) -> tuple[int, int, int, int, int, int, int, int]:
    """Attribute combined invalid-numeric rows onto summary categories.

    ``VerificationReport`` exposes a single ``invalid_numeric_rows`` union.
    OrderVerifier warnings identify which categories contributed, so the CLI
    attributes the union count to matching categories for summary display.
    """
    ...
    side_rows = (
        numeric
        if (_WARN_INVALID_SIDE in report.warnings or _WARN_EMPTY_SIDE in report.warnings)
        else 0
    )
    order_type_rows = (
        numeric
        if (
            _WARN_INVALID_ORDER_TYPE in report.warnings or _WARN_EMPTY_ORDER_TYPE in report.warnings
        )
        else 0
    )
    status_rows = (
        numeric
        if (_WARN_INVALID_STATUS in report.warnings or _WARN_EMPTY_STATUS in report.warnings)
        else 0
    )
    quantity_rows = numeric if _WARN_INVALID_QUANTITY in report.warnings else 0
    filled_quantity_rows = numeric if _WARN_INVALID_FILLED_QUANTITY in report.warnings else 0
    average_fill_price_rows = numeric if _WARN_INVALID_AVERAGE_FILL_PRICE in report.warnings else 0
    limit_price_rows = numeric if _WARN_INVALID_LIMIT_PRICE in report.warnings else 0
    stop_price_rows = numeric if _WARN_INVALID_STOP_PRICE in report.warnings else 0
    return (
        side_rows,
        order_type_rows,
        status_rows,
        quantity_rows,
        filled_quantity_rows,
        average_fill_price_rows,
        limit_price_rows,
        stop_price_rows,
    )
...
def _build_summary(
    *,
        ...
        if result.status == "succeeded":
            ...
            if report is not None:
                ...
                (
                    ...
                    limit_price_rows,
                    stop_price_rows,
                ) = _attribute_invalid_rows(report)
                invalid_side_rows += side_rows
                invalid_order_type_rows += order_type_rows
                invalid_status_rows += status_rows
                invalid_quantity_rows += quantity_rows
                invalid_filled_quantity_rows += filled_quantity_rows
                invalid_average_fill_price_rows += average_fill_price_rows
                invalid_limit_price_rows += limit_price_rows
                invalid_stop_price_rows += stop_price_rows
    ...
    repository_passed = (
        ...
        and invalid_order_type_rows == 0
        and invalid_status_rows == 0
        and invalid_quantity_rows == 0
        and invalid_filled_quantity_rows == 0
        and invalid_average_fill_price_rows == 0
        and invalid_limit_price_rows == 0
        and invalid_stop_price_rows == 0
        and warnings == 0
    )
