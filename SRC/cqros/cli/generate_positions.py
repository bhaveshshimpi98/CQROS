"""CQROS position-generation CLI.

Purpose:
    Provide an argparse-based production entry point that discovers executed-
    trade partitions and executes ``PositionPipeline`` across the universe with
    bounded symbol concurrency, persisting outputs through
    ``PositionRepository``.

Responsibilities:
    - Parse CLI arguments for position dataset generation
    - Discover available execution partitions through ``TradeRepository``
    - Filter loaded trade frames by optional ``--model`` / ``--version``
    - Resolve ``--engine`` through ``PositionEngineRegistry``
    - Execute ``PositionPipeline`` and persist via ``PositionRepository``
...
Dependencies:
    ...
    ``cqros.execution``, ``cqros.positions``, and ``cqros.storage``.

Public API:
    ``build_parser``, ``build_options``, ``build_default_engine``,
    ``build_engine_registry``, ``build_position_pipeline``,
...
Notes:
    ...
    exclusively to ``PositionPipeline``. Persistence remains in the CLI
    because ``PositionPipeline`` does not own a repository.
"""
...
from cqros.execution import ExecutionPartitionRef, TradeRepository
...
_ERROR_WORKERS: Final[str] = "CLI-GENERATE-POSITIONS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-GENERATE-POSITIONS-002"
_ERROR_YEAR: Final[str] = "CLI-GENERATE-POSITIONS-003"
_ERROR_MODEL: Final[str] = "CLI-GENERATE-POSITIONS-004"
_ERROR_VERSION: Final[str] = "CLI-GENERATE-POSITIONS-005"
_ERROR_MANAGER: Final[str] = "CLI-GENERATE-POSITIONS-006"
_ERROR_ENGINE: Final[str] = "CLI-GENERATE-POSITIONS-007"


@dataclass(frozen=True, slots=True)
class PositionGenerationOptions:
    """Immutable CLI options for position dataset generation.

    Attributes:
        storage_root: Storage root containing ``executions`` and ``positions``.
        manager: Order manager identity used for discovery and position lineage.
        engine: Registry key of the position engine to execute.
        model: Optional model identifier used to filter trade rows.
        version: Optional model version used to filter trade rows.
        ...
        overwrite: When ``True``, regenerate existing position partitions.
...
class DiscoveredWorkItem:
    """One discovered trade partition group ready for position generation.

    Attributes:
        manager: Order manager identifier of the source partitions.
        symbol: Tradeable symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing execution parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class PositionGenerationSummary:
    """Immutable aggregate summary for a position-generation run.

    Attributes:
        manager: Order manager identity used for generation.
        engine: Position engine registry key used for generation.
        version: Optional model version used for generation.
        symbols_discovered: Unique symbols discovered from execution storage.
        ...
        output_directory: Positions-tier output directory.
...
def build_parser() -> argparse.ArgumentParser:
    """Create the position-generation argument parser.

    Returns:
        Configured ``ArgumentParser`` for position-generation flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-generate-positions",
        description=(
            "Generate CQROS position datasets from discovered executed-trade "
            "partitions and an injected position engine."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        required=True,
        metavar="NAME",
        help="Order manager identity used for discovery and position lineage.",
    )
    parser.add_argument(
        "--engine",
        dest="engine",
        ...
        metavar="NAME",
        help=f"Position engine registry key (default: {_DEFAULT_ENGINE}).",
    )
    parser.add_argument(
        "--model",
        dest="model",
        default=None,
        metavar="NAME",
        help="Optional stable model identifier used to filter trade rows.",
    )
    parser.add_argument(
        "--version",
        dest="version",
        default=None,
        metavar="VERSION",
        help="Optional model version identifier used to filter trade rows.",
    )
    parser.add_argument(
        "--symbols",
        dest="symbols",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Regenerate position partitions that already exist.",
    )
    parser.add_argument(
        "--workers",
        dest="workers",
...
def build_options(args: argparse.Namespace) -> PositionGenerationOptions:
    """Map parsed CLI arguments onto ``PositionGenerationOptions``.
...
def build_default_engine() -> PositionEngine:
    """Compose the default production position engine for the CLI.

    Returns:
        ``AverageCostPositionEngine`` instance.
    """
    return AverageCostPositionEngine()


def build_engine_registry(
    *,
    engines: Mapping[str, PositionEngine] | None = None,
) -> PositionEngineRegistry:
    ...
    Args:
        engines: Optional mapping of registry names to engine instances.
            When ``None``, registers ``AverageCostPositionEngine`` under
            ``average_cost``.

    Returns:
        Fully populated ``PositionEngineRegistry``.
    """
...
def build_position_pipeline(
    options: PositionGenerationOptions,
    *,
    engine_registry: PositionEngineRegistry | None = None,
) -> PositionPipeline:
    """Compose ``PositionPipeline`` from injected engine registry deps.
    ...
    Args:
        ...
        engine_registry: Optional engine registry. When ``None``, a default
            registry containing ``AverageCostPositionEngine`` is built.

    Returns:
        Fully wired ``PositionPipeline``.
    """
    ...
    return PositionPipeline(engine_registry)
...
def discover_work(
    ...
    """Discover position-ready execution partitions matching CLI filters.

    Only execution partitions that exist are scheduled. Missing execution
    partitions are never invented. Partial position datasets are never
    generated.

    Args:
        trade_repository: Trade repository providing discovery APIs.
        options: CLI filters for manager, symbol, timeframe, and year.

    Returns:
        Deterministically ordered discovered work items.
    """
    partitions = trade_repository.discover_partitions(
...
def format_summary(summary: PositionGenerationSummary) -> str:
    """Render a deterministic position-generation summary report.
    ...
    lines = [
        "=====================================",
        "CQROS Position Generation Summary",
        "=====================================",
        "",
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the position-generation CLI.
...
async def run_generation(
    *,
    ...
    Args:
        pipeline: Injected position pipeline.
    ...
    output_directory = options.storage_root / STORAGE_DIR_POSITIONS

    if len(work) == 0:
        return PositionGenerationSummary(
...
def _group_partitions(
    partitions: Sequence[ExecutionPartitionRef],
    *,
    year_filter: tuple[int, ...] | None,
) -> tuple[DiscoveredWorkItem, ...]:
    """Group execution year partitions into manager/symbol/timeframe work items."""
...
async def _run_worker_pool(
    *,
    ...
    collected: list[PositionTaskResult] = []
    lock = asyncio.Lock()
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"generate-positions-worker-{index}")
...
async def _generate_symbol_work(
    *,
    ...
    """Generate position datasets for every discovered year for one symbol."""
    results: list[PositionTaskResult] = []
...
def _generate_partition(
    ...
    *,
    ...
    """Generate one position year partition synchronously."""
    if not overwrite and position_repository.exists(
    ...
    try:
        ...
        filtered = _filter_trades_for_model(
            trades,
            model_name=model_name,
            model_version=model_version,
        )
...
def _filter_trades_for_model(
    trades: pl.DataFrame,
    *,
    model_name: str | None,
    model_version: str | None,
) -> pl.DataFrame:
    """Return trade rows matching optional model identity filters."""
...
def _print_progress(result: PositionTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed position generation partition; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _build_summary(
    *,
    options: PositionGenerationOptions,
    work: Sequence[DiscoveredWorkItem],
    results: Sequence[PositionTaskResult],
    duration_seconds: float,
    output_directory: Path,
) -> PositionGenerationSummary:
