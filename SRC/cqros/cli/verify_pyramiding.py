"""CQROS pyramiding dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    pyramiding partitions and executes ``PyramidingVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for pyramiding dataset verification
    - Discover available pyramiding partitions through ``PyramidingRepository``
    - Load partitions and verify them with ``PyramidingVerifier``
...
Dependencies:
    ...
    ``cqros.pyramiding``, and ``cqros.storage``.
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-PYRAMIDING-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-PYRAMIDING-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-PYRAMIDING-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-PYRAMIDING-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-PYRAMIDING-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-PYRAMIDING-006"

_VERIFIER_NAME: Final[str] = "PyramidingVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Pyramiding"

_COL_MODEL_NAME: Final[str] = "model_name"
_COL_MODEL_VERSION: Final[str] = "model_version"
_COL_MANAGER: Final[str] = "manager"

# Attribution keys matching PyramidingVerifier warning text.
...
# category when reason warnings are present.
...
class VerifyPyramidingOptions:
    """Immutable CLI options for pyramiding dataset verification.

    Attributes:
        storage_root: Storage root containing ``pyramiding``.
...
class DiscoveredWorkItem:
    """One discovered pyramiding partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyPyramidingSummary:
    """Immutable aggregate summary for a pyramiding verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always pyramiding tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the pyramiding dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-pyramiding",
        description=("Verify CQROS pyramiding datasets across the discovered pyramiding universe."),
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the pyramiding tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded pyramiding rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded pyramiding rows. "
...
def build_options(args: argparse.Namespace) -> VerifyPyramidingOptions:
    """Map parsed CLI arguments onto ``VerifyPyramidingOptions``.
...
def discover_work(
    repository: PyramidingRepository,
    options: VerifyPyramidingOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover pyramiding partitions matching the CLI filters.

    Args:
        repository: Pyramiding repository providing discovery APIs.
...
def format_summary(summary: VerifyPyramidingSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the pyramiding dataset verification CLI."""
    parser = build_parser()
...
async def run_verification(
    *,
    ...
    if len(work) == 0:
        return VerifyPyramidingSummary(
            symbols_verified=0,
            datasets_verified=0,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-pyramiding-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> PyramidingTaskResult | None:
    """Verify one pyramiding year partition synchronously."""
    try:
...
def _report_task_failure(result: PyramidingTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed pyramiding partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _attribute_invalid_status_rows(report: VerificationReport) -> int:
    ...
    Maps ``reason`` verifier warnings onto the shared invalid-status summary
    ...
    if numeric == 0:
        return 0
    if _WARN_EMPTY_REASON in report.warnings or _WARN_INVALID_REASON in report.warnings:
        return numeric
    return 0
