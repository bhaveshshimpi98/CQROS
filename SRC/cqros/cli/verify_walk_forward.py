"""CQROS walk-forward dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    walk-forward panels and executes ``WalkForwardVerifier`` across
    the universe with bounded panel concurrency.

Responsibilities:
    - Parse CLI arguments for walk-forward dataset verification
    - Discover available partitions through ``WalkForwardRepository``
    - Load partitions and verify them with ``WalkForwardVerifier``
...
Dependencies:
    ...
    ``cqros.walk_forward``, ``cqros.processing.verification.report``, and
    ``cqros.storage``.
...
from cqros.walk_forward import WalkForwardRepository, WalkForwardVerifier
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-WALK-FORWARD-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-WALK-FORWARD-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-WALK-FORWARD-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-WALK-FORWARD-004"

_VERIFIER_NAME: Final[str] = "WalkForwardVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Walk-Forward"
...
class VerifyWalkForwardOptions:
    """Immutable CLI options for walk-forward dataset verification.
...
class DiscoveredWorkItem:
    """One discovered walk-forward panel group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyWalkForwardSummary:
    """Immutable aggregate summary for a walk-forward verification run.

    Attributes:
        panels_verified: Unique manager/timeframe/year panels attempted.
        datasets_verified: Unique datasets attempted (always walk-forward tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the walk-forward dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-walk-forward",
        description=(
            "Verify CQROS walk-forward datasets across the discovered walk-forward universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the walk-forward tier."
        ),
    )
    parser.add_argument(
        "--timeframes",
...
def build_options(args: argparse.Namespace) -> VerifyWalkForwardOptions:
    """Map parsed CLI arguments onto ``VerifyWalkForwardOptions``.
...
def discover_work(
    ...
    options: VerifyWalkForwardOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover walk-forward partitions matching the CLI filters.

    Args:
        repository: Walk-forward repository providing discovery APIs.
...
def format_summary(summary: VerifyWalkForwardSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the walk-forward dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Walk-forward partition repository.
        verifier: Walk-forward verifier instance.
    ...
    if len(work) == 0:
        return VerifyWalkForwardSummary(
            panels_verified=0,
            datasets_verified=0,
...
def _group_partitions(
    ...
    *,
    ...
    """Group walk-forward year partitions into work items."""
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-walk-forward-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one walk-forward year partition synchronously."""
    try:
...
def _report_task_failure(result: WalkForwardTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed walk-forward partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
