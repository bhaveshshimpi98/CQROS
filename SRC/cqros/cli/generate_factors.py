"""CQROS factor-generation CLI.

Purpose:
    Provide an argparse-based production entry point that discovers processed
    market partitions and executes ``FactorGenerationPipeline`` across the
    universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for factor generation
    - Discover available processed OHLCV partitions with required companions
    - Load and join companion processed series into a factor input frame
    - Sort the joined frame by ``(symbol, timeframe, open_time)`` before
      ``FactorPipeline`` (factor-specific alignment is applied inside the
      pipeline via ``FactorInputPartition``; global companion alignment
      remains available as ``align_factor_input_frame`` for compatibility)
    - Persist factors through the injected ``FactorsRepository``
...
Dependencies:
    ...
    ``cqros.factors``, ``cqros.ingestion``, and ``cqros.storage``.

Public API:
    ``align_factor_input_frame``, ``build_parser``, ``build_options``,
    ``build_factor_generation_pipeline``, ``discover_work``,
    ``format_summary``, ``load_factor_input_frame``, ``run_generation``, and
    ``main``.

Notes:
    This module is a thin composition root. It does not implement factor
    calculations, wide-to-long conversion, schema validation, repository
    filesystem walks beyond calling repository discovery and load APIs,
    factor validation, factor selection, or research runners.
    ``load_factor_input_frame`` preserves the full OHLCV timeline after
    causal companion joins (leading companion nulls may remain). Factor-
    specific alignment happens in ``FactorPipeline`` /
    ``FactorInputPartition``. ``align_factor_input_frame`` retains the
    legacy global companion-intersection behavior for compatibility. Funding
    settlement timestamps are floored to whole seconds before as-of join so
    sub-second exchange noise cannot reject an otherwise valid settlement.
"""
...
_ERROR_WORKERS: Final[str] = "CLI-GENERATE-FACTORS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-GENERATE-FACTORS-002"
_ERROR_YEAR: Final[str] = "CLI-GENERATE-FACTORS-003"
_ERROR_MANAGER: Final[str] = "CLI-GENERATE-FACTORS-004"
_ERROR_OHLCV_COLUMNS: Final[str] = "CLI-GENERATE-FACTORS-005"
_ERROR_COMPANION_COLUMNS: Final[str] = "CLI-GENERATE-FACTORS-006"
_ERROR_COMPANION_ALIGNMENT: Final[str] = "CLI-GENERATE-FACTORS-007"
...
_COL_TAKER_BUY_VOLUME: Final[str] = "taker_buy_volume"
_COL_TAKER_SELL_VOLUME: Final[str] = "taker_sell_volume"
...
# (``8h``), not under the OHLCV bar timeframe used for factor generation.
_FUNDING_STORAGE_TIMEFRAME: Final[Timeframe] = DEFAULT_FUNDING_TIMEFRAME

# Binance funding timestamps occasionally carry 1–16 ms of exchange noise
# past the nominal settlement second. Floor to whole seconds so a settlement
# at ``T + 8ms`` still matches the OHLCV bar opening at ``T``.
_MS_PER_SECOND: Final[int] = 1_000

# Raw input columns required from the merged processed market frame before
# FactorPipeline execution. Column names match the Factor Research Engine
# catalog (taker volumes are renamed from processed ``buy_volume`` /
# ``sell_volume``).
_REQUIRED_FACTOR_INPUT_COLUMNS: Final[tuple[str, ...]] = (
    _COL_SYMBOL,
    _COL_TIMEFRAME,
    _COL_OPEN_TIME,
    _COL_OPEN,
    ...
    _COL_MARK_PRICE,
    _COL_OPEN_INTEREST,
    _COL_TAKER_BUY_VOLUME,
    _COL_TAKER_SELL_VOLUME,
    _COL_LONG_SHORT_RATIO,
)

# Companion value columns that must be non-null before factor execution.
...
# Canonical ordering required before FactorPipeline rolling windows execute.
_FACTOR_INPUT_SORT_COLUMNS: Final[tuple[str, ...]] = (
    _COL_SYMBOL,
    _COL_TIMEFRAME,
    _COL_OPEN_TIME,
)
...
class FactorGenerationOptions:
    """Immutable CLI options for factor generation.

    Attributes:
        storage_root: Storage root containing ``processed`` and ``factors``.
        manager: Order manager identity used for factors partition lineage.
        ...
        overwrite: When ``True``, regenerate existing factor partitions.
...
class DiscoveredWorkItem:
    """One discovered processed partition group ready for factor generation.

    Attributes:
        symbol: Tradeable symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing processed OHLCV partitions that
            also have every required companion dataset.
    """
...
class FactorTaskResult:
    ...
    Attributes:
        ...
        total_registered_factors: Registered catalog size when succeeded.
        executable_factors: Executable factor count when succeeded.
        skipped_factors: Skipped factor count when succeeded (informational).
    ...
    total_registered_factors: int | None = None
    executable_factors: int | None = None
    skipped_factors: int | None = None
...
@dataclass(frozen=True, slots=True)
class FactorGenerationSummary:
    """Immutable aggregate summary for a factor generation run.

    Attributes:
        manager: Order manager identity used for factors lineage.
        ...
        total_registered_factors: Registered catalog size from a successful
            generation (``0`` when none succeeded).
        executable_factors: Executable factor count from a successful
            generation (``0`` when none succeeded).
        skipped_factors: Informational skipped-factor count from a successful
            generation (``0`` when none succeeded).
        duration_seconds: Wall-clock generation duration.
        output_directory: Factors-tier output directory.
...
def build_parser() -> argparse.ArgumentParser:
    """Create the factor-generation argument parser.

    Returns:
        Configured ``ArgumentParser`` for factor-generation flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-generate-factors",
        description=(
            "Generate CQROS factor datasets from discovered processed "
            "market partitions through FactorGenerationPipeline."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        required=True,
        metavar="NAME",
        help="Order manager identity used for factors partition lineage.",
    )
    parser.add_argument(
        "--symbols",
        dest="symbols",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Regenerate factor partitions that already exist.",
    )
    parser.add_argument(
        "--workers",
        dest="workers",
...
def build_options(args: argparse.Namespace) -> FactorGenerationOptions:
    """Map parsed CLI arguments onto ``FactorGenerationOptions``.
...
def build_factor_generation_pipeline(
    options: FactorGenerationOptions,
    *,
    logger: logging.Logger | None = None,
) -> FactorGenerationPipeline:
    """Compose ``FactorGenerationPipeline`` from shared production dependencies.
    ...
    Returns:
        Fully wired ``FactorGenerationPipeline``.
    """
    ...
    return FactorGenerationPipeline(
        registry,
        None,
        WideToLongFactorTransformer(logger=active_logger),
        factors_repository,
...
def discover_work(
    ...
    """Discover factor-ready processed partitions matching the CLI filters.

    OHLCV year partitions are included only when every companion dataset
    required by the Factor Research Engine also exists for that
...
def format_summary(summary: FactorGenerationSummary) -> str:
    """Render a deterministic factor-generation summary report.
    ...
    lines = [
        "=====================================",
        "CQROS Factor Generation Summary",
        "=====================================",
        "",
        ...
        "",
        f"Total registered factors: {summary.total_registered_factors}",
        f"Executable factors: {summary.executable_factors}",
        f"Skipped factors: {summary.skipped_factors}",
...
def load_factor_input_frame(
    ...
    *,
    ...
    """Load processed partitions and assemble a FactorPipeline input frame.

    OHLCV forms the bar timeline keyed by ``(symbol, timeframe, open_time)``.
    Companion series required by the Factor Research Engine are loaded and
    backward as-of joined on ``open_time``. Funding is loaded from the native
    settlement timeframe (``8h``), not ``timeframe``, because processed funding
    partitions are not duplicated per bar interval. Processed taker volumes are
    renamed to ``taker_buy_volume`` / ``taker_sell_volume``. Missing companion
    partitions raise ``DatasetNotFoundError``; they are never silently omitted.

    Leading companion nulls are retained. Factor-specific alignment is applied
    later by ``FactorPipeline`` / ``FactorInputPartition`` so OHLCV-only
    factors are not truncated by unrelated companion availability. Use
    ``align_factor_input_frame`` explicitly when the legacy global companion
    intersection is required. The frame is sorted by
    ``(symbol, timeframe, open_time)`` exactly once before return.
    ...
    Returns:
        Eager DataFrame sorted by ``(symbol, timeframe, open_time)`` containing
        factor raw inputs. Companion columns may be null on early bars.
    ...
    frame = _join_companion(
        ...
        value_columns=(_COL_FUNDING_RATE, _COL_MARK_PRICE),
        floor_time_to_second=True,
    )
    frame = _join_companion(
        frame,
    ...
    frame = frame.rename(
        {
            _COL_BUY_VOLUME: _COL_TAKER_BUY_VOLUME,
            _COL_SELL_VOLUME: _COL_TAKER_SELL_VOLUME,
        }
    )
    frame = _join_companion(
        frame,
    ...
    validated = _require_factor_input_columns(frame)
    return validated.sort(list(_FACTOR_INPUT_SORT_COLUMNS))


def align_factor_input_frame(frame: pl.DataFrame) -> pl.DataFrame:
    """Drop leading rows that lack complete companion factor inputs.
    ...
    rows are removed so ``FactorPipeline`` only receives bars where every
    present required companion input is non-null. Values are never filled.

    Args:
        frame: Joined factor-input frame sorted by ``open_time``.
    ...
    if not companion_columns:
        raise DatasetNotFoundError(
            "factor input frame has no required companion columns to align",
            error_code=_ERROR_COMPANION_ALIGNMENT,
            details={
                "required_companion_columns": _REQUIRED_COMPANION_INPUT_COLUMNS,
                "available_columns": tuple(frame.columns),
            },
            recovery_suggestion=("Ensure companion datasets were joined before factor generation."),
        )
    ...
    if first_complete.height == 0:
        raise DatasetNotFoundError(
            "no factor-input rows have complete companion coverage",
            error_code=_ERROR_COMPANION_ALIGNMENT,
            details={
...
async def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        factors_repository = FactorsRepository(layout, datastore)
        pipeline = build_factor_generation_pipeline(options)
...
async def run_generation(
    *,
    pipeline: FactorGenerationPipeline,
    ...
    Args:
        pipeline: Injected factor generation pipeline.
    ...
    output_directory = options.storage_root / STORAGE_DIR_FACTORS

    if len(work) == 0:
        return FactorGenerationSummary(
...
def _group_partitions(
    ...
    *,
    ...
    """Group processed year partitions into symbol/timeframe work items."""
...
async def _run_worker_pool(
    *,
    ...
    collected: list[FactorTaskResult] = []
    lock = asyncio.Lock()
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"generate-factors-worker-{index}")
...
async def _generate_symbol_work(
    *,
    ...
    """Generate factors for every discovered year for one symbol."""
...
def _generate_partition(
    ...
    *,
    ...
    """Generate one factor year partition synchronously."""
    if not overwrite and factors_repository.exists(
...
def _print_progress(result: FactorTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed factor generation partition; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _build_summary(
    *,
    options: FactorGenerationOptions,
    work: Sequence[DiscoveredWorkItem],
    results: Sequence[FactorTaskResult],
    duration_seconds: float,
    output_directory: Path,
) -> FactorGenerationSummary:
    ...
    for result in results:
        ...
        if result.status == "succeeded":
            ...
            if result.total_registered_factors is not None:
                total_registered_factors = result.total_registered_factors
            if result.executable_factors is not None:
                executable_factors = result.executable_factors
            if result.skipped_factors is not None:
    ...
    return FactorGenerationSummary(
        ...
        total_registered_factors=total_registered_factors,
        executable_factors=executable_factors,
...
def _prepare_ohlcv_frame(
    ...
    *,
    ...
    """Normalize OHLCV columns required by the Factor Research Engine."""
    required = (
        _COL_OPEN_TIME,
        _COL_OPEN,
        _COL_HIGH,
...
def _require_factor_input_columns(frame: pl.DataFrame) -> pl.DataFrame:
    """Fail fast when the merged input frame lacks catalog-required columns."""
    missing = tuple(name for name in _REQUIRED_FACTOR_INPUT_COLUMNS if name not in frame.columns)
    if missing:
        raise ValidationError(
            f"factor input frame missing required columns: {list(missing)}",
            error_code=_ERROR_COMPANION_COLUMNS,
            details={
                "missing_columns": missing,
                "required_columns": _REQUIRED_FACTOR_INPUT_COLUMNS,
                "available_columns": tuple(frame.columns),
            },
        )
    return frame
...
def _log_factor_input_debug(
    ...
    *,
    ...
    """Log merged factor-input diagnostics when ``--debug`` is enabled."""
    funding_nulls = (
        int(frame.get_column(_COL_FUNDING_RATE).null_count())
        if _COL_FUNDING_RATE in frame.columns
        else -1
    )
    open_interest_nulls = (
        int(frame.get_column(_COL_OPEN_INTEREST).null_count())
        if _COL_OPEN_INTEREST in frame.columns
        else -1
    )
    first_timestamp = int(frame.get_column(_COL_OPEN_TIME).min()) if frame.height else None
    last_timestamp = int(frame.get_column(_COL_OPEN_TIME).max()) if frame.height else None
    is_sorted = (
        bool(frame.select(pl.col(_COL_OPEN_TIME).is_sorted()).item()) if frame.height else True
    )
    _logger.debug(
        "Merged factor input diagnostics",
        extra={
            "symbol": symbol,
            "timeframe": timeframe,
            "year": year,
            "total_rows": frame.height,
            "funding_null_count": funding_nulls,
            "open_interest_null_count": open_interest_nulls,
            "first_timestamp": first_timestamp,
            "last_timestamp": last_timestamp,
            "is_frame_sorted": is_sorted,
        },
    )
    print(
        (
            f"DEBUG factor input {symbol} {timeframe} {year}: "
            f"rows={frame.height} funding_nulls={funding_nulls} "
            f"oi_nulls={open_interest_nulls} first={first_timestamp} "
            f"last={last_timestamp} sorted={is_sorted}"
        ),
        flush=True,
    )
    if funding_nulls <= 0 or _COL_FUNDING_RATE not in frame.columns:
        return

    null_times = (
        frame.filter(pl.col(_COL_FUNDING_RATE).is_null())
        .get_column(_COL_OPEN_TIME)
        .head(20)
        .to_list()
    )
    funding_times = (
        frame.filter(pl.col(_COL_FUNDING_RATE).is_not_null()).get_column(_COL_OPEN_TIME).to_list()
    )
    for open_time in null_times:
        previous = max((ts for ts in funding_times if ts < open_time), default=None)
        nxt = min((ts for ts in funding_times if ts > open_time), default=None)
        print(
            (
                f"DEBUG funding NULL open_time={open_time} "
                f"prev_funding={previous} next_funding={nxt}"
            ),
            flush=True,
        )
        _logger.debug(
            "Funding NULL join context",
            extra={
                "symbol": symbol,
                "timeframe": timeframe,
                "year": year,
                "ohlcv_open_time": open_time,
                "previous_funding_timestamp": previous,
                "next_funding_timestamp": nxt,
            },
        )
...
def _join_companion(
    ...
    *,
    ...
    floor_time_to_second: bool = False,
) -> pl.DataFrame:
    """Backward as-of join companion value columns onto ``open_time``.

    Args:
        base: Left OHLCV (or partially joined) frame.
        companion: Companion partition frame.
        dataset: Companion dataset name for error context.
        time_column: Companion timestamp column to align onto ``open_time``.
        value_columns: Companion value columns to attach.
        floor_time_to_second: When ``True``, floor companion timestamps to
            whole seconds before the join (funding settlement noise).
    """
    ...
    selected = companion.select([time_column, *value_columns])
    if floor_time_to_second:
        selected = selected.with_columns(
            ((pl.col(time_column) // _MS_PER_SECOND) * _MS_PER_SECOND).alias(time_column)
        )
    right = selected.rename({time_column: _COL_OPEN_TIME}).sort(_COL_OPEN_TIME)
    # ``join_asof`` requires both frames sorted by the join key. The left
    # frame is already ordered by prior joins in the assembly path; sort only
    # when needed to avoid redundant full-frame sorts.
    left = base if base.get_column(_COL_OPEN_TIME).is_sorted() else base.sort(_COL_OPEN_TIME)
