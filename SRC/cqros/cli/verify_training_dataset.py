"""CQROS training-dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers training
    partitions and executes ``TrainingVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for training dataset verification
    - Discover available training partitions through ``TrainingRepository``
    - Load partitions and verify them with ``TrainingVerifier``
...
Dependencies:
    ...
    ``cqros.training.verification``, and ``cqros.storage``.
...
from cqros.storage import ParquetStore, StorageLayout, TrainingPartitionRef, TrainingRepository
from cqros.training.verification import TrainingVerifier, VerificationReport

__all__ = [
    "DiscoveredWorkItem",
    "TrainingTaskResult",
    "VerifyTrainingOptions",
    "VerifyTrainingSummary",
    "build_options",
    "build_parser",
    "discover_work",
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-TRAINING-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-TRAINING-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-TRAINING-003"

_VERIFIER_NAME: Final[str] = "TrainingVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Training"


@dataclass(frozen=True, slots=True)
class VerifyTrainingOptions:
    """Immutable CLI options for training dataset verification.

    Attributes:
        storage_root: Storage root containing ``training``.
...
class DiscoveredWorkItem:
    """One discovered training partition group ready for verification.

    Attributes:
        symbol: Training symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing training parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class VerifyTrainingSummary:
    """Immutable aggregate summary for a training verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always training tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the training-dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-training-dataset",
        description=(
            "Verify CQROS merged training datasets across the discovered " "training universe."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyTrainingOptions:
    """Map parsed CLI arguments onto ``VerifyTrainingOptions``.
...
def discover_work(
    ...
    options: VerifyTrainingOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover training partitions matching the CLI filters.

    Args:
        repository: Training repository providing discovery APIs.
...
def format_partition_failure(
    *,
    ...
    Args:
        ...
        code: Optional CQROS / training validation error code.
...
def format_summary(summary: VerifyTrainingSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the training-dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Injected training repository.
        verifier: Injected training verifier.
...
def _group_partitions(
    partitions: Sequence[TrainingPartitionRef],
    *,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-training-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one training year partition synchronously."""
    try:
...
def _report_task_failure(result: TrainingTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed training partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
