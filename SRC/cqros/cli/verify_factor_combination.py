"""CQROS factor combination dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    factor combination panels and executes
    ``FactorCombinationVerifier.verify`` followed by
    ``FactorCombinationVerifier.verify_against_fta`` across the universe
    with bounded panel concurrency.

Responsibilities:
    - Parse CLI arguments for factor combination dataset verification
    - Discover available partitions through ``FactorCombinationRepository``
    - Load combination frames and corresponding FTA frames via
      ``FactorTimeframeAnalysisRepository``
    - Verify each combination frame structurally and against FTA lineage
...
Dependencies:
    ...
    ``cqros.factor_combination``, ``cqros.factor_timeframe_analysis``,
    and ``cqros.storage``.

Public API:
    ``DiscoveredWorkItem``, ``FactorCombinationTaskResult``,
    ``VerifyFactorCombinationOptions``, ``VerifyFactorCombinationSummary``,
    ``build_options``, ``build_parser``, ``discover_work``,
    ``format_partition_failure``, ``format_summary``, ``main``,
    ``run_verification``.
...
Notes:
    ...
    repository discovery and load APIs. Combination partitions are keyed
    by manager/timeframe/year. The corresponding FTA partition is loaded
    by manager/year (no timeframe in FTA path).
"""
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-FCOMB-001"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-FCOMB-002"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-FCOMB-003"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-FCOMB-004"

_VERIFIER_NAME: Final[str] = "FactorCombinationVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Factor Combination"


@dataclass(frozen=True, slots=True)
class VerifyFactorCombinationOptions:
    """Immutable CLI options for factor combination dataset verification.

    Attributes:
        storage_root: Storage root containing ``factor_combination`` and
...
class DiscoveredWorkItem:
    """One discovered combination partition ready for verification.

    Attributes:
        manager: Order manager identifier.
        timeframe: Trade bar interval of the combination partition.
        year: Calendar year of the combination partition.
    """
...
class FactorCombinationTaskResult:
    ...
    Attributes:
        manager: Order manager identifier.
        timeframe: Trade bar interval of the partition.
        year: Calendar year of the partition.
        status: ``succeeded`` or ``failed``.
        rows_checked: Number of rows that were verified on success.
...
@dataclass(frozen=True, slots=True)
class VerifyFactorCombinationSummary:
    """Immutable aggregate summary for a combination verification run.

    Attributes:
        panels_verified: Unique manager/timeframe/year panels attempted.
        datasets_verified: Unique datasets attempted (always combination).
        ...
        rows_checked: Sum of rows checked across successes.
...
def build_parser() -> argparse.ArgumentParser:
    """Create the factor combination dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-factor-combination",
        description=(
            "Verify CQROS factor combination datasets across the discovered "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Omit to verify all managers present under the factor combination tier."
        ),
    )
    parser.add_argument(
        "--years",
...
def build_options(args: argparse.Namespace) -> VerifyFactorCombinationOptions:
    """Map parsed CLI arguments onto ``VerifyFactorCombinationOptions``.
...
def discover_work(
    ...
    options: VerifyFactorCombinationOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover combination partitions matching the CLI filters.

    Args:
        repository: Factor combination repository providing discovery APIs.
...
def format_summary(summary: VerifyFactorCombinationSummary) -> str:
    """Render a deterministic combination verification summary report.
    ...
    lines = [
        "=====================================",
        "CQROS Factor Combination Verification Summary",
        "=====================================",
        "",
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the factor combination dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        combination_repository: Factor combination partition repository.
        fta_repository: FTA repository for cross-frame lineage loading.
        verifier: Factor combination verifier instance.
    ...
    if len(work) == 0:
        return VerifyFactorCombinationSummary(
            panels_verified=0,
            datasets_verified=0,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-combination-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one combination year partition synchronously."""
    try:
        ...
        rows_checked = combination_frame.height
    except Exception as exc:
...
def _report_task_failure(result: FactorCombinationTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed combination partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
