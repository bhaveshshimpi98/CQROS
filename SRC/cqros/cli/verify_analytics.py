"""CQROS analytics dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    analytics partitions and executes ``AnalyticsVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for analytics dataset verification
    - Discover available analytics partitions through ``AnalyticsRepository``
    - Load partitions and verify them with ``AnalyticsVerifier``
...
Dependencies:
    ...
    ``cqros.analytics``, ``cqros.processing.verification.report``, and
    ``cqros.storage``.
...
from cqros.analytics import AnalyticsRepository, AnalyticsVerifier
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-ANALYTICS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-ANALYTICS-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-ANALYTICS-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-ANALYTICS-004"

_VERIFIER_NAME: Final[str] = "AnalyticsVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Analytics"
...
class VerifyAnalyticsOptions:
    """Immutable CLI options for analytics dataset verification.

    Attributes:
        storage_root: Storage root containing ``analytics``.
...
class DiscoveredWorkItem:
    """One discovered analytics partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyAnalyticsSummary:
    """Immutable aggregate summary for an analytics verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always analytics tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the analytics dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-analytics",
        description=("Verify CQROS analytics datasets across the discovered analytics universe."),
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the analytics tier."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyAnalyticsOptions:
    """Map parsed CLI arguments onto ``VerifyAnalyticsOptions``.
...
def discover_work(
    ...
    options: VerifyAnalyticsOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover analytics partitions matching the CLI filters.

    Args:
        repository: Analytics repository providing discovery APIs.
...
def format_summary(summary: VerifyAnalyticsSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the analytics dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Analytics partition repository.
        verifier: Analytics verifier instance.
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-analytics-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one analytics year partition synchronously."""
    try:
...
def _report_task_failure(result: AnalyticsTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed analytics partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
