"""CQROS monitoring dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    monitoring partitions and executes ``MonitoringVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for monitoring dataset verification
    - Discover available monitoring partitions through ``MonitoringRepository``
    - Load partitions and verify them with ``MonitoringVerifier``
...
Dependencies:
    ...
    ``cqros.monitoring``, ``cqros.processing.verification.report``, and
    ``cqros.storage``.
...
from cqros.monitoring import MonitoringRepository, MonitoringVerifier
from cqros.monitoring.repository import MonitoringPartitionRef
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-MONITORING-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-MONITORING-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-MONITORING-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-MONITORING-004"

_VERIFIER_NAME: Final[str] = "MonitoringVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Monitoring"
...
class VerifyMonitoringOptions:
    """Immutable CLI options for monitoring dataset verification.

    Attributes:
        storage_root: Storage root containing ``monitoring``.
...
class DiscoveredWorkItem:
    """One discovered monitoring partition group ready for verification.

    Attributes:
        manager: Order manager identifier.
        symbol: Tradeable symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing monitoring parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class VerifyMonitoringSummary:
    """Immutable aggregate summary for a monitoring verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always monitoring tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the monitoring dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-monitoring",
        description=("Verify CQROS monitoring datasets across the discovered monitoring universe."),
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the monitoring tier."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyMonitoringOptions:
    """Map parsed CLI arguments onto ``VerifyMonitoringOptions``.
...
def discover_work(
    ...
    options: VerifyMonitoringOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover monitoring partitions matching the CLI filters.

    Args:
        repository: Monitoring repository providing discovery APIs.
...
def format_summary(summary: VerifyMonitoringSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the monitoring dataset verification CLI.
    ...
    try:
        ...
        repository = MonitoringRepository(layout, ParquetStore())
...
async def run_verification(
    *,
    ...
    Args:
        repository: Monitoring partition repository.
        verifier: Monitoring verifier instance.
    ...
    if len(work) == 0:
        return VerifyMonitoringSummary(
            symbols_verified=0,
            datasets_verified=0,
...
def _group_partitions(
    partitions: Sequence[MonitoringPartitionRef],
    *,
    year_filter: tuple[int, ...] | None,
) -> tuple[DiscoveredWorkItem, ...]:
    """Group monitoring year partitions into manager/symbol/timeframe work items."""
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-monitoring-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one monitoring year partition synchronously."""
    try:
...
def _report_task_failure(result: MonitoringTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed monitoring partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
