"""CQROS exit-engine dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    exit-engine partitions and executes ``ExitEngineVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for exit-engine dataset verification
    - Discover available exit-engine partitions through ``ExitRepository``
    - Load partitions and verify them with ``ExitEngineVerifier``
...
Dependencies:
    ...
    ``cqros.exit_engine``, and ``cqros.storage``.
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-EXIT-ENGINE-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-EXIT-ENGINE-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-EXIT-ENGINE-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-EXIT-ENGINE-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-EXIT-ENGINE-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-EXIT-ENGINE-006"

_VERIFIER_NAME: Final[str] = "ExitEngineVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Exit Engine"

_COL_MANAGER: Final[str] = "manager"

# Attribution keys matching ExitEngineVerifier warning text.
...
# category when action or reason warnings are present.
_WARN_EMPTY_EXIT_ACTION: Final[str] = "Empty exit_action values detected."
_WARN_INVALID_EXIT_ACTION: Final[str] = "Invalid ExitAction values detected."
_WARN_EMPTY_EXIT_REASON: Final[str] = "Empty exit_reason values detected."
_WARN_INVALID_EXIT_REASON: Final[str] = "Invalid ExitReason values detected."


@dataclass(frozen=True, slots=True)
class VerifyExitEngineOptions:
    """Immutable CLI options for exit-engine dataset verification.

    Attributes:
        storage_root: Storage root containing ``exit_engine``.
        ...
        model: Optional model-name allowlist identity. ``None`` keeps all.
            Ignored when the exit-engine schema does not include
...
class DiscoveredWorkItem:
    """One discovered exit-engine partition group ready for verification.

    Attributes:
        manager: Order manager identifier.
        symbol: Tradeable symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing exit_engine parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class VerifyExitEngineSummary:
    """Immutable aggregate summary for an exit-engine verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always exit-engine tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the exit-engine dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-exit-engine",
        description="Verify CQROS exit-engine datasets across the discovered exit-engine universe.",
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the exit-engine tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded exit-engine rows. "
            "Ignored when the exit-engine schema does not include model_name. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded exit-engine rows. "
            "Ignored when the exit-engine schema does not include model_version. "
            "Omit to verify all rows present in each partition."
        ),
    )
    parser.add_argument(
...
def build_options(args: argparse.Namespace) -> VerifyExitEngineOptions:
    """Map parsed CLI arguments onto ``VerifyExitEngineOptions``.
...
def discover_work(
    ...
    options: VerifyExitEngineOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover exit-engine partitions matching the CLI filters.

    Args:
        repository: Exit-engine repository providing discovery APIs.
...
def format_summary(summary: VerifyExitEngineSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the exit-engine dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Exit-engine partition repository.
        verifier: Exit-engine verifier instance.
    ...
    if len(work) == 0:
        return VerifyExitEngineSummary(
            symbols_verified=0,
            datasets_verified=0,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-exit-engine-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> ExitEngineTaskResult | None:
    """Verify one exit-engine year partition synchronously."""
    try:
...
def _apply_identity_filters(
    ...
    *,
    ...
    ``frame``. Exit-engine schema does not include ``model_name`` or
...
def _report_task_failure(result: ExitEngineTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed exit-engine partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _attribute_invalid_status_rows(report: VerificationReport) -> int:
    ...
    Maps ``exit_action`` and ``exit_reason`` verifier warnings onto the
    shared invalid-status summary field, consistent with how other dataset
    verifiers attribute enumeration warnings.
    """
    numeric = report.invalid_numeric_rows
    if numeric == 0:
        return 0
    action_warning = (
        _WARN_EMPTY_EXIT_ACTION in report.warnings or _WARN_INVALID_EXIT_ACTION in report.warnings
    )
    reason_warning = (
        _WARN_EMPTY_EXIT_REASON in report.warnings or _WARN_INVALID_EXIT_REASON in report.warnings
    )
    if action_warning or reason_warning:
        return numeric
    return 0
