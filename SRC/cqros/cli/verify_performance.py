"""CQROS performance dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    performance partitions and executes ``PerformanceVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for performance dataset verification
    - Discover available performance partitions through ``PerformanceRepository``
    - Load partitions and verify them with ``PerformanceVerifier``
...
Dependencies:
    ...
    ``cqros.performance``, ``cqros.processing.verification.report``, and
    ``cqros.storage``.
...
from cqros.performance import PerformanceRepository, PerformanceVerifier
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-PERFORMANCE-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-PERFORMANCE-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-PERFORMANCE-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-PERFORMANCE-004"

_VERIFIER_NAME: Final[str] = "PerformanceVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Performance"
...
class VerifyPerformanceOptions:
    """Immutable CLI options for performance dataset verification.

    Attributes:
        storage_root: Storage root containing ``performance``.
...
class DiscoveredWorkItem:
    """One discovered performance partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyPerformanceSummary:
    """Immutable aggregate summary for a performance verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always performance tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the performance dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-performance",
        description=(
            "Verify CQROS performance datasets across the discovered performance universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the performance tier."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyPerformanceOptions:
    """Map parsed CLI arguments onto ``VerifyPerformanceOptions``.
    ...
    return VerifyPerformanceOptions(
...
def discover_work(
    ...
    options: VerifyPerformanceOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover performance partitions matching the CLI filters.

    Args:
        repository: Performance repository providing discovery APIs.
...
def format_summary(summary: VerifyPerformanceSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the performance dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Performance partition repository.
        verifier: Performance verifier instance.
    ...
    if len(work) == 0:
        return VerifyPerformanceSummary(
            symbols_verified=0,
            datasets_verified=0,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-performance-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one performance year partition synchronously."""
    try:
...
def _report_task_failure(result: PerformanceTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed performance partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
