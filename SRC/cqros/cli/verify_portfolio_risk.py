"""CQROS portfolio-risk dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    portfolio-risk partitions and executes ``PortfolioRiskVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for portfolio-risk dataset verification
    - Discover available portfolio-risk partitions through
      ``PortfolioRiskRepository``
    - Load partitions and verify them with ``PortfolioRiskVerifier``
...
Dependencies:
    ...
    ``cqros.portfolio_risk``, and ``cqros.storage``.
...
__all__ = [
    ...
    "VerifyPortfolioRiskOptions",
    "VerifyPortfolioRiskSummary",
    "build_options",
    "build_parser",
    "discover_work",
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-PORTFOLIO-RISK-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-PORTFOLIO-RISK-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-PORTFOLIO-RISK-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-PORTFOLIO-RISK-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-PORTFOLIO-RISK-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-PORTFOLIO-RISK-006"

_VERIFIER_NAME: Final[str] = "PortfolioRiskVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Portfolio Risk"

_COL_MODEL_NAME: Final[str] = "model_name"
_COL_MODEL_VERSION: Final[str] = "model_version"
_COL_MANAGER: Final[str] = "manager"

# Attribution keys matching PortfolioRiskVerifier warning text.
# VerificationReport exposes only the combined ``invalid_numeric_rows``
# counter, so the CLI maps that union onto the invalid-status summary
# category when portfolio_risk_state / shutdown_reason warnings are present.
...
class VerifyPortfolioRiskOptions:
    """Immutable CLI options for portfolio-risk dataset verification.

    Attributes:
        storage_root: Storage root containing ``portfolio_risk``.
...
class DiscoveredWorkItem:
    """One discovered portfolio-risk partition group ready for verification.

    Attributes:
        manager: Order manager identifier.
        symbol: Tradeable symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing portfolio-risk parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class VerifyPortfolioRiskSummary:
    """Immutable aggregate summary for a portfolio-risk verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always portfolio-risk
            tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the portfolio-risk dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-portfolio-risk",
        description=(
            "Verify CQROS portfolio-risk datasets across the discovered " "portfolio-risk universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the portfolio-risk tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded portfolio-risk rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded portfolio-risk "
            "rows. Omit to verify all versions present in each partition."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyPortfolioRiskOptions:
    """Map parsed CLI arguments onto ``VerifyPortfolioRiskOptions``.
    ...
    return VerifyPortfolioRiskOptions(
...
def discover_work(
    ...
    options: VerifyPortfolioRiskOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover portfolio-risk partitions matching the CLI filters.

    Args:
        repository: Portfolio-risk repository providing discovery APIs.
...
def format_summary(summary: VerifyPortfolioRiskSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the portfolio-risk dataset verification CLI."""
    parser = build_parser()
    ...
    try:
        ...
        summary = await run_verification(
            ...
            verifier=PortfolioRiskVerifier(),
            options=options,
            work=work,
        )
...
async def run_verification(
    *,
    ...
    if len(work) == 0:
        return VerifyPortfolioRiskSummary(
            symbols_verified=0,
            datasets_verified=0,
...
def _group_partitions(
    partitions: Sequence[PortfolioRiskPartitionRef],
    *,
    year_filter: tuple[int, ...] | None,
) -> tuple[DiscoveredWorkItem, ...]:
    """Group portfolio-risk year partitions into manager/symbol/timeframe work items."""
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-portfolio-risk-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> PortfolioRiskTaskResult | None:
    """Verify one portfolio-risk year partition synchronously."""
    try:
...
def _report_task_failure(result: PortfolioRiskTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed portfolio-risk partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _attribute_invalid_status_rows(report: VerificationReport) -> int:
    ...
    Maps ``portfolio_risk_state`` and ``shutdown_reason`` verifier warnings
    onto the shared invalid-status summary field the same way accounting maps
    ``PositionStatus`` warnings.
    """
    numeric = report.invalid_numeric_rows
    if numeric == 0:
        return 0
    if (
        _WARN_INVALID_STATE in report.warnings
        or _WARN_EMPTY_STATE in report.warnings
        or _WARN_INVALID_REASON in report.warnings
    ):
        return numeric
    return 0
