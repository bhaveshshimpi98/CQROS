"""CQROS feature-dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers feature
    partitions and executes ``FeatureVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for feature verification
    - Discover available feature partitions through ``FeatureRepository``
    - Load partitions and verify them with ``FeatureVerifier``
...
Dependencies:
    ...
    ``cqros.features.verification``, and ``cqros.storage``.
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-FEATURES-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-FEATURES-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-FEATURES-003"

_VERIFIER_NAME: Final[str] = "FeatureVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Features"


@dataclass(frozen=True, slots=True)
class VerifyFeaturesOptions:
    """Immutable CLI options for feature verification.

    Attributes:
        storage_root: Storage root containing ``features``.
...
class DiscoveredWorkItem:
    """One discovered feature partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyFeaturesSummary:
    """Immutable aggregate summary for a feature verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always features tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the feature-verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-features",
        description=(
            "Verify CQROS merged feature datasets across the discovered " "features universe."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyFeaturesOptions:
    """Map parsed CLI arguments onto ``VerifyFeaturesOptions``.
...
def discover_work(
    ...
    options: VerifyFeaturesOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover feature partitions matching the CLI filters.

    Args:
        repository: Feature repository providing discovery APIs.
...
def format_partition_failure(
    *,
    ...
    Args:
        ...
        code: Optional CQROS / feature validation error code.
...
def format_summary(summary: VerifyFeaturesSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the feature-verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Injected feature repository.
        verifier: Injected feature verifier.
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-features-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one feature year partition synchronously."""
    try:
...
def _report_task_failure(result: FeatureTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed feature partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
