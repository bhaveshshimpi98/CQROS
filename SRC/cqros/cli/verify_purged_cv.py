"""CQROS purged-CV dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    purged-CV panels and executes ``PurgedCVVerifier`` across
    the universe with bounded panel concurrency.

Responsibilities:
    - Parse CLI arguments for purged-CV dataset verification
    - Discover available purged-CV partitions through ``PurgedCVRepository``
    - Load partitions and verify them with ``PurgedCVVerifier``
...
Dependencies:
    ...
    ``cqros.purged_cv``, ``cqros.processing.verification.report``, and
    ``cqros.storage``.
...
from cqros.purged_cv import PurgedCVRepository, PurgedCVVerifier
from cqros.purged_cv.repository import PurgedCVPartitionRef
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-PURGED-CV-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-PURGED-CV-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-PURGED-CV-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-PURGED-CV-004"

_VERIFIER_NAME: Final[str] = "PurgedCVVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Purged-CV"
...
class VerifyPurgedCVOptions:
    """Immutable CLI options for purged-CV dataset verification.

    Attributes:
        storage_root: Storage root containing ``purged_cv``.
...
class DiscoveredWorkItem:
    """One discovered purged-CV panel group ready for verification.

    Attributes:
        manager: Order manager identifier.
        timeframe: Available bar interval.
        years: Calendar years with existing purged-CV parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class VerifyPurgedCVSummary:
    """Immutable aggregate summary for a purged-CV verification run.

    Attributes:
        panels_verified: Unique manager/timeframe/year panels attempted.
        datasets_verified: Unique manager/timeframe/year partition datasets
            attempted (one parquet partition per panel).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the purged-CV dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-purged-cv",
        description=(
            "Verify CQROS purged cross-validation datasets across the discovered "
            "purged-CV universe."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Omit to verify all managers present under the purged-CV tier."
        ),
    )
    parser.add_argument(
        "--timeframes",
...
def build_options(args: argparse.Namespace) -> VerifyPurgedCVOptions:
    """Map parsed CLI arguments onto ``VerifyPurgedCVOptions``.
...
def discover_work(
    repository: PurgedCVRepository,
    options: VerifyPurgedCVOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover purged-CV partitions matching the CLI filters.

    Args:
        repository: Purged-CV repository providing discovery APIs.
...
def format_summary(summary: VerifyPurgedCVSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the purged-CV dataset verification CLI.
    ...
    try:
        ...
        repository = PurgedCVRepository(layout, ParquetStore())
...
async def run_verification(
    *,
    ...
    Args:
        repository: Purged-CV partition repository.
        verifier: Purged-CV verifier instance.
...
def _group_partitions(
    ...
    *,
    ...
    """Group purged-CV year partitions into work items."""
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-purged-cv-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one purged-CV year partition synchronously."""
    try:
...
def _report_task_failure(result: PurgedCVTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed purged-CV partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _build_summary(
    *,
    ...
    datasets_verified = len(panels_verified)
    return VerifyPurgedCVSummary(
