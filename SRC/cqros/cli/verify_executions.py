"""CQROS executed-trade dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers execution
    partitions and executes ``ExecutionVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for executed-trade dataset verification
    - Discover available execution partitions through ``TradeRepository``
    - Load partitions and verify them with ``ExecutionVerifier``
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-EXECUTIONS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-EXECUTIONS-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-EXECUTIONS-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-EXECUTIONS-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-EXECUTIONS-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-EXECUTIONS-006"

_VERIFIER_NAME: Final[str] = "ExecutionVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Executions"
...
class VerifyExecutionOptions:
    """Immutable CLI options for executed-trade dataset verification.

    Attributes:
        storage_root: Storage root containing ``executions``.
...
class DiscoveredWorkItem:
    """One discovered execution partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyExecutionSummary:
    """Immutable aggregate summary for an execution verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always executions tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the executed-trade dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-executions",
        description=(
            "Verify CQROS executed-trade datasets across the discovered " "executions universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the executions tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded trade rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded trade rows. "
...
def build_options(args: argparse.Namespace) -> VerifyExecutionOptions:
    """Map parsed CLI arguments onto ``VerifyExecutionOptions``.
...
def discover_work(
    ...
    """Discover execution partitions matching the CLI filters.

    Args:
        repository: Trade repository providing discovery APIs.
...
def format_summary(summary: VerifyExecutionSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the executed-trade dataset verification CLI."""
    parser = build_parser()
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-executions-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> ExecutionTaskResult | None:
    """Verify one execution year partition synchronously."""
    try:
...
def _report_task_failure(result: ExecutionTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed execution partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
