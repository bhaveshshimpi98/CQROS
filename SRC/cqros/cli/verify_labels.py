"""CQROS label-dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers label
    partitions and executes ``LabelVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for label verification
    - Discover available label partitions through ``LabelRepository``
    - Load partitions and verify them with ``LabelVerifier``
...
Dependencies:
    ...
    ``cqros.labels.verification``, and ``cqros.storage``.
...
from cqros.labels.verification import LabelVerifier, VerificationReport
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-LABELS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-LABELS-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-LABELS-003"

_VERIFIER_NAME: Final[str] = "LabelVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Labels"


@dataclass(frozen=True, slots=True)
class VerifyLabelsOptions:
    """Immutable CLI options for label verification.

    Attributes:
        storage_root: Storage root containing ``labels``.
...
class DiscoveredWorkItem:
    """One discovered label partition group ready for verification.

    Attributes:
        symbol: Label symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing label parquet partitions.
    """
...
@dataclass(frozen=True, slots=True)
class VerifyLabelsSummary:
    """Immutable aggregate summary for a label verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always labels tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the label-verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-labels",
        description=(
            "Verify CQROS merged label datasets across the discovered " "labels universe."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyLabelsOptions:
    """Map parsed CLI arguments onto ``VerifyLabelsOptions``.
...
def discover_work(
    ...
    options: VerifyLabelsOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover label partitions matching the CLI filters.

    Args:
        repository: Label repository providing discovery APIs.
...
def format_partition_failure(
    *,
    ...
    Args:
        ...
        code: Optional CQROS / label validation error code.
...
def format_summary(summary: VerifyLabelsSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the label-verification CLI.
    ...
    try:
        ...
        repository = LabelRepository(layout, ParquetStore())
...
async def run_verification(
    *,
    ...
    Args:
        repository: Injected label repository.
        verifier: Injected label verifier.
...
def _group_partitions(
    partitions: Sequence[LabelPartitionRef],
    *,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-labels-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one label year partition synchronously."""
    try:
...
def _report_task_failure(result: LabelTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed label partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
