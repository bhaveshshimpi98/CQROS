"""CQROS 1d factor-input partitioning investigation CLI.

Purpose:
    Provide an argparse entry point that runs the read-only 1d factor-input
    partitioning architecture investigation without mutating production lake
    ledgers.

Responsibilities:
    - Parse CLI arguments for the input-partitioning reporter
    - Wire ``FactorStability1dFactorInputPartitioningReporter``
...
Dependencies:
    ...
    ``cqros.reporting.factor_stability_1d_factor_input_partitioning``.

Public API:
    ``ReportFactorStability1dFactorInputPartitioningOptions``,
    ``build_parser``, ``build_options``, ``run_report``, ``format_summary``,
    and ``main``.
"""
...
_ERROR_MANAGER: Final[str] = "CLI-REPORT-1D-INPUT-PART-001"
_ERROR_OUTPUT: Final[str] = "CLI-REPORT-1D-INPUT-PART-002"
_ERROR_YEAR: Final[str] = "CLI-REPORT-1D-INPUT-PART-003"


@dataclass(frozen=True, slots=True)
class ReportFactorStability1dFactorInputPartitioningOptions:
    """Immutable CLI options for the input-partitioning investigation."""
...
def build_parser() -> argparse.ArgumentParser:
    """Create the 1d factor-input partitioning investigation argument parser."""
    parser = argparse.ArgumentParser(
        prog="cqros-report-factor-stability-1d-factor-input-partitioning",
        description=(
            "Generate read-only 1d factor-input partitioning architecture "
            "reports from processed / factor / selection artifacts."
        ),
    )
    parser.add_argument(
        "--manager",
...
def build_options(
    ...
    return ReportFactorStability1dFactorInputPartitioningOptions(
...
def run_report(
    options: ReportFactorStability1dFactorInputPartitioningOptions,
) -> FactorStability1dFactorInputPartitioningResult:
    """Execute the 1d factor-input partitioning reporter."""
    _configure_logging(verbose=options.verbose, debug=options.debug)
    reporter = FactorStability1dFactorInputPartitioningReporter(
...
def format_summary(result: FactorStability1dFactorInputPartitioningResult) -> str:
    ...
    lines.extend(
        [
            ...
            f"production_artifacts_unchanged={result.production_artifacts_unchanged}",
            f"ohlcv_only_truncated={result.ohlcv_only_truncated}",
            f"bars_recovered_pvt={result.bars_recovered_pvt}",
            f"bars_recovered_obv={result.bars_recovered_obv}",
            "",
        ]
    )
    return "\n".join(lines)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (CQROSError, ReportingValidationError, ValidationError) as exc:
        if args.debug:
            raise
