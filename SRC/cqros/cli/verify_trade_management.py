"""CQROS trade-management dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    trade-management partitions and executes ``TradeManagementVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for trade-management dataset verification
    - Discover available trade-management partitions through
      ``TradeManagementRepository``
    - Load partitions and verify them with ``TradeManagementVerifier``
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-TRADE-MANAGEMENT-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-TRADE-MANAGEMENT-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-TRADE-MANAGEMENT-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-TRADE-MANAGEMENT-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-TRADE-MANAGEMENT-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-TRADE-MANAGEMENT-006"

_VERIFIER_NAME: Final[str] = "TradeManagementVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Trade Management"

_COL_MODEL_NAME: Final[str] = "model_name"
_COL_MODEL_VERSION: Final[str] = "model_version"
_COL_MANAGER: Final[str] = "manager"

# Attribution keys matching TradeManagementVerifier warning text.
...
# category when management_action / action_reason warnings are present.
...
class VerifyTradeManagementOptions:
    """Immutable CLI options for trade-management dataset verification.

    Attributes:
        storage_root: Storage root containing ``trade_management``.
...
class DiscoveredWorkItem:
    """One discovered trade-management partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyTradeManagementSummary:
    """Immutable aggregate summary for a trade-management verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always trade-management
            tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the trade-management dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-trade-management",
        description=(
            "Verify CQROS trade-management datasets across the discovered "
            "trade-management universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the trade-management tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded trade-management rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded trade-management "
...
def build_options(args: argparse.Namespace) -> VerifyTradeManagementOptions:
    """Map parsed CLI arguments onto ``VerifyTradeManagementOptions``.
...
def discover_work(
    repository: TradeManagementRepository,
    options: VerifyTradeManagementOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover trade-management partitions matching the CLI filters.

    Args:
        repository: Trade-management repository providing discovery APIs.
...
def format_summary(summary: VerifyTradeManagementSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the trade-management dataset verification CLI."""
    parser = build_parser()
...
async def run_verification(
    *,
    ...
    if len(work) == 0:
        return VerifyTradeManagementSummary(
            symbols_verified=0,
            datasets_verified=0,
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-trade-management-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> TradeManagementTaskResult | None:
    """Verify one trade-management year partition synchronously."""
    try:
...
def _report_task_failure(result: TradeManagementTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed trade-management partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _attribute_invalid_status_rows(report: VerificationReport) -> int:
    ...
    Maps ``management_action`` and ``action_reason`` verifier warnings onto
    the shared invalid-status summary field the same way portfolio-risk maps
    ``portfolio_risk_state`` / ``shutdown_reason`` warnings.
    """
    numeric = report.invalid_numeric_rows
    if numeric == 0:
        return 0
    if (
        _WARN_EMPTY_ACTION in report.warnings
        or _WARN_INVALID_ACTION in report.warnings
