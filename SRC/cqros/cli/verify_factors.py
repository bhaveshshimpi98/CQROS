"""CQROS factor-dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers factor
    partitions and executes ``FactorVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for factor verification
    - Discover available factor partitions through ``FactorsRepository``
    - Load partitions and verify them with ``FactorVerifier``
    - Aggregate results into a final PASS/FAIL report
    - Print structured per-partition failure diagnostics
    - Print detailed factor diagnostics when ``--debug`` is enabled
    - Print a global Unexpected NULL / +Inf failure report on FAIL
      when ``--debug`` is enabled
    - Print the report and return an exit code

Dependencies:
    ``argparse``, ``asyncio``, ``cqros.config``, ``cqros.core``,
    ``cqros.factors``, and ``cqros.storage``.

Public API:
    ``build_parser``, ``build_options``, ``discover_work``,
    ``format_partition_failure``, ``format_debug_diagnostics``,
    ``format_global_failure_report``, ``format_summary``,
    ``run_verification``, and ``main``.

Notes:
    This module is a thin composition root. It does not implement verification
    logic, factor computation, Polars transforms, or repository filesystem
    walks beyond calling repository discovery APIs.
"""
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-FACTORS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-FACTORS-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-FACTORS-003"

_VERIFIER_NAME: Final[str] = "FactorVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Factors"


@dataclass(frozen=True, slots=True)
class VerifyFactorsOptions:
    """Immutable CLI options for factor verification.

    Attributes:
        storage_root: Storage root containing ``factors``.
...
class DiscoveredWorkItem:
    """One discovered factor partition group ready for verification.

    Attributes:
        manager: Order manager identifier.
        symbol: Factor symbol.
        timeframe: Available bar interval.
        years: Calendar years with existing factor parquet partitions.
    """
...
class FactorTaskResult:
    ...
    year: int
    status: str
    report: FactorVerificationReport | VerificationReport | None = None
...
@dataclass(frozen=True, slots=True)
class VerifyFactorsSummary:
    """Immutable aggregate summary for a factor verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always factors tier).
        ...
        warmup_null_rows: Sum of warmup-only ``factor_value`` NULL counters.
        domain_null_rows: Sum of domain ``factor_value`` NULL counters.
        unexpected_null_rows: Sum of unexpected NULL counters.
        positive_inf_rows: Sum of ``+inf`` counters.
        negative_inf_rows: Sum of ``-inf`` counters.
        non_finite_rows: Sum of non-finite counters.
        ...
        failure_findings: Unexpected NULL and +Inf findings across
            partitions, used for the global ``--debug`` failure report.
    """
    ...
    failure_findings: tuple[GlobalFailureFinding, ...] = ()


def build_parser() -> argparse.ArgumentParser:
    """Create the factor-verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-factors",
        description=("Verify CQROS factor datasets across the discovered " "factors universe."),
    )
    parser.add_argument(
        "--symbols",
        dest="symbols",
...
def build_options(args: argparse.Namespace) -> VerifyFactorsOptions:
    """Map parsed CLI arguments onto ``VerifyFactorsOptions``.
...
def discover_work(
    ...
    options: VerifyFactorsOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover factor partitions matching the CLI filters.

    Args:
        repository: Factor repository providing discovery APIs.
...
def format_partition_failure(
    *,
    ...
    Args:
        ...
        code: Optional CQROS / factor validation error code.
...
def format_summary(summary: VerifyFactorsSummary) -> str:
    ...
    lines = [
        ...
        f"Warmup NULLs: {summary.warmup_null_rows}",
        f"Domain NULLs: {summary.domain_null_rows}",
        f"Unexpected NULLs: {summary.unexpected_null_rows}",
        f"Invalid +Inf: {summary.positive_inf_rows}",
        f"Invalid -Inf: {summary.negative_inf_rows}",
        f"Invalid NonFinite: {summary.non_finite_rows}",
...
def format_debug_diagnostics(
    *,
    ...
    timeframe: Timeframe,
    year: int,
    report: FactorVerificationReport | VerificationReport,
) -> str:
    """Render detailed factor diagnostics for ``--debug`` mode.
    ...
    Args:
        ...
        timeframe: Bar interval.
        year: Calendar year of the partition.
        report: Verification report that may carry structured diagnostics.

    Returns:
        Multi-line diagnostic text, or an empty string when unavailable.
    """
    diagnostics = getattr(report, "diagnostics", None)
    if diagnostics is None:
        return ""
    body = format_factor_diagnostics(diagnostics)
    if body == "":
        return ""
    header = (
        f"Debug diagnostics: manager={manager} symbol={symbol} "
        f"timeframe={timeframe} year={year}\n"
    )
    return header + body


async def main(argv: Sequence[str] | None = None) -> int:
    """Run the factor-verification CLI.
    ...
    if options.debug and not summary.repository_passed:
        failure_report = format_global_failure_report(summary.failure_findings)
        if failure_report:
            print(failure_report, end="")
...
async def run_verification(
    *,
    ...
    Args:
        repository: Injected factors repository.
        verifier: Injected factor verifier.
...
def _ensure_utf8_stdout() -> None:
    """Prefer UTF-8 stdout when available (reports themselves are ASCII-only)."""
...
def _group_partitions(
    ...
    *,
    ...
    """Group year partitions into manager/symbol/timeframe work items."""
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-factors-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one factor year partition synchronously."""
    try:
    ...
    if debug:
        debug_text = format_debug_diagnostics(
            manager=manager,
            symbol=symbol,
            timeframe=timeframe,
            year=year,
...
def _report_task_failure(result: FactorTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed factor partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _build_summary(
    *,
    ...
    warnings = 0
    report_failed = False
    failure_findings: list[GlobalFailureFinding] = []
    ...
    for result in results:
        ...
        if result.status == "succeeded":
            ...
            if report is not None:
                ...
                warmup_null_rows += int(getattr(report, "warmup_null_rows", 0))
                domain_null_rows += int(getattr(report, "domain_null_rows", 0))
                unexpected_null_rows += int(getattr(report, "unexpected_null_rows", 0))
                positive_inf_rows += int(getattr(report, "positive_inf_rows", 0))
                negative_inf_rows += int(getattr(report, "negative_inf_rows", 0))
                non_finite_rows += int(getattr(report, "non_finite_rows", 0))
    ...
    # Warmup and domain NULLs alone must not fail: rely on verifier ``passed``.
    repository_passed = failed_tasks == 0 and not report_failed
    ...
    return VerifyFactorsSummary(
        ...
        failure_findings=ordered_findings,
    )
