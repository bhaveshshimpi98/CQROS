"""CQROS risk-decision dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers risk
    partitions and executes ``RiskVerifier`` across the universe with bounded
    symbol concurrency.

Responsibilities:
    - Parse CLI arguments for risk-decision dataset verification
    - Discover available risk partitions through ``RiskRepository``
    - Load partitions and verify them with ``RiskVerifier``
...
Dependencies:
    ...
    ``cqros.risk.verification``, and ``cqros.storage``.
...
Notes:
    ...
    logic, risk-policy semantics, or repository filesystem walks beyond
    calling repository discovery and load APIs. Optional ``--policy`` filters
    discovery by risk-policy path segment. Optional ``--model`` / ``--version``
    filters restrict verification to matching identity rows after load when
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-RISK-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-RISK-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-RISK-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-RISK-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-RISK-005"
_ERROR_POLICY: Final[str] = "CLI-VERIFY-RISK-006"

_VERIFIER_NAME: Final[str] = "RiskVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Risks"

_COL_MODEL_NAME: Final[str] = "model_name"
_COL_MODEL_VERSION: Final[str] = "model_version"

# Attribution keys matching RiskVerifier warning text. VerificationReport
...
class VerifyRiskOptions:
    """Immutable CLI options for risk-decision dataset verification.

    Attributes:
        storage_root: Storage root containing ``risks``.
        policy: Optional risk-policy allowlist identity. ``None`` discovers all.
...
class DiscoveredWorkItem:
    """One discovered risk partition group ready for verification.
...
class RiskTaskResult:
    """Immutable result for one policy/symbol/timeframe/year verification task.
...
@dataclass(frozen=True, slots=True)
class VerifyRiskSummary:
    """Immutable aggregate summary for a risk verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always risks tier).
        ...
        invalid_decision_rows: Sum of attributed invalid-decision counters.
        invalid_target_weight_rows: Sum of attributed invalid target-weight
            counters.
        invalid_approved_weight_rows: Sum of attributed invalid approved-weight
            counters.
...
def build_parser() -> argparse.ArgumentParser:
    """Create the risk-decision dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-risk",
        description=(
            "Verify CQROS risk-decision datasets across the discovered " "risks universe."
        ),
    )
    parser.add_argument(
        "--policy",
        ...
        metavar="NAME",
        help=(
            "Optional risk-policy filter applied to partition discovery. "
            "Omit to verify all policies present under the risks tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded risk rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded risk rows. "
...
def build_options(args: argparse.Namespace) -> VerifyRiskOptions:
    """Map parsed CLI arguments onto ``VerifyRiskOptions``.
...
def discover_work(
    ...
    options: VerifyRiskOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover risk partitions matching the CLI filters.

    Args:
        repository: Risk repository providing discovery APIs.
...
def format_partition_failure(
    *,
    ...
    Args:
        ...
        code: Optional CQROS / risk validation error code.
...
def format_summary(summary: VerifyRiskSummary) -> str:
    ...
    lines = [
        ...
        f"Invalid approved weight rows: {summary.invalid_approved_weight_rows}",
        "",
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the risk-decision dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Injected risk repository.
        verifier: Injected risk verifier.
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-risk-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one risk year partition synchronously.
...
def _apply_identity_filters(
    ...
    *,
    ...
    """Restrict ``frame`` to optional model/version identity filters."""
    filtered = frame
...
def _report_task_failure(result: RiskTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed risk partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
...
def _attribute_invalid_rows(report: VerificationReport) -> tuple[int, int, int]:
    ...
    RiskVerifier warnings identify which categories contributed, so the CLI
    ...
    decision_rows = (
        numeric
        if (_WARN_INVALID_DECISION in report.warnings or _WARN_EMPTY_DECISION in report.warnings)
        else 0
    )
    target_weight_rows = numeric if _WARN_INVALID_TARGET_WEIGHT in report.warnings else 0
    approved_weight_rows = numeric if _WARN_INVALID_APPROVED_WEIGHT in report.warnings else 0
    return decision_rows, target_weight_rows, approved_weight_rows
...
def _build_summary(
    *,
    ...
    for result in results:
        ...
        if result.status == "succeeded":
            ...
            if report is not None:
                ...
                decision_rows, target_rows, approved_rows = _attribute_invalid_rows(report)
                invalid_decision_rows += decision_rows
                invalid_target_weight_rows += target_rows
                invalid_approved_weight_rows += approved_rows
    ...
    repository_passed = (
        ...
        and invalid_decision_rows == 0
        and invalid_target_weight_rows == 0
        and invalid_approved_weight_rows == 0
        and warnings == 0
    )
