"""CQROS factor validation dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    factor validation panels and executes ``FactorValidationVerifier``
    across the universe with bounded panel concurrency.

Responsibilities:
    - Parse CLI arguments for factor validation dataset verification
    - Discover available partitions through ``FactorValidationRepository``
    - Load partitions and verify them with ``FactorValidationVerifier``
...
Dependencies:
    ...
    ``cqros.factor_validation``, ``cqros.processing.verification.report``,
    and ``cqros.storage``.
...
from cqros.factor_validation import FactorValidationRepository, FactorValidationVerifier
...
__all__ = [
    ...
    "VerifyFactorValidationOptions",
    "VerifyFactorValidationSummary",
    "build_options",
    "build_parser",
    "discover_work",
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-FACTOR-VALIDATION-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-FACTOR-VALIDATION-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-FACTOR-VALIDATION-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-FACTOR-VALIDATION-004"

_VERIFIER_NAME: Final[str] = "FactorValidationVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Factor Validation"
...
class VerifyFactorValidationOptions:
    """Immutable CLI options for factor validation dataset verification.
...
class DiscoveredWorkItem:
    """One discovered factor validation panel group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyFactorValidationSummary:
    """Immutable aggregate summary for a factor validation verification run.

    Attributes:
        panels_verified: Unique manager/timeframe/year panels attempted.
        datasets_verified: Unique datasets attempted (always factor validation).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the factor validation dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-factor-validation",
        description=(
            "Verify CQROS factor validation datasets across the discovered "
            "factor validation universe."
        ),
    )
    parser.add_argument(
        "--manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the factor validation tier."
        ),
    )
    parser.add_argument(
        "--timeframes",
...
def build_options(args: argparse.Namespace) -> VerifyFactorValidationOptions:
    """Map parsed CLI arguments onto ``VerifyFactorValidationOptions``.
    ...
    return VerifyFactorValidationOptions(
...
def discover_work(
    ...
    options: VerifyFactorValidationOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover factor validation partitions matching the CLI filters.

    Args:
        repository: Factor validation repository providing discovery APIs.
...
def format_summary(summary: VerifyFactorValidationSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the factor validation dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Factor validation partition repository.
        verifier: Factor validation verifier instance.
    ...
    if len(work) == 0:
        return VerifyFactorValidationSummary(
            panels_verified=0,
            datasets_verified=0,
...
def _group_partitions(
    ...
    *,
    ...
    """Group factor validation year partitions into work items."""
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-factor-validation-worker-{index}")
...
async def _verify_timeframe_work(
    *,
    ...
    results: list[FactorValidationTaskResult] = []
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one factor validation year partition synchronously."""
    try:
...
def _report_task_failure(result: FactorValidationTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed factor validation partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
