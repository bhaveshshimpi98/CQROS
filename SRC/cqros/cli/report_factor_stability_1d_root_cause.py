"""CQROS 1d Factor Stability root-cause investigation CLI.

Purpose:
    Provide an argparse entry point that runs the read-only 1d root-cause
    investigation without mutating production lake ledgers.

Responsibilities:
    - Parse CLI arguments for the 1d root-cause reporter
    - Wire ``FactorStability1dRootCauseReporter``
...
Dependencies:
    ...
    ``cqros.reporting.factor_stability_1d_root_cause``.

Public API:
    ``ReportFactorStability1dRootCauseOptions``, ``build_parser``,
...
__all__ = [
    "ReportFactorStability1dRootCauseOptions",
    "build_options",
    "build_parser",
    "format_summary",
    "main",
...
_ERROR_MANAGER: Final[str] = "CLI-REPORT-1D-ROOT-CAUSE-001"
_ERROR_OUTPUT: Final[str] = "CLI-REPORT-1D-ROOT-CAUSE-002"
_ERROR_YEAR: Final[str] = "CLI-REPORT-1D-ROOT-CAUSE-003"


@dataclass(frozen=True, slots=True)
class ReportFactorStability1dRootCauseOptions:
    """Immutable CLI options for the 1d root-cause investigation."""
...
def build_parser() -> argparse.ArgumentParser:
    """Create the 1d root-cause investigation argument parser."""
    parser = argparse.ArgumentParser(
        prog="cqros-report-factor-stability-1d-root-cause",
        description=(
            "Generate read-only 1d factor-stability root-cause reports from "
            "existing factor_selection / walk_forward / purged_cv artifacts."
        ),
    )
    parser.add_argument(
        "--manager",
...
def build_options(args: argparse.Namespace) -> ReportFactorStability1dRootCauseOptions:
    ...
    return ReportFactorStability1dRootCauseOptions(
...
def run_report(
    options: ReportFactorStability1dRootCauseOptions,
) -> FactorStability1dRootCauseResult:
    """Execute the 1d root-cause reporter."""
    _configure_logging(verbose=options.verbose, debug=options.debug)
    reporter = FactorStability1dRootCauseReporter(
...
def format_summary(result: FactorStability1dRootCauseResult) -> str:
...
def main(argv: Sequence[str] | None = None) -> int:
    """CLI entry point for the 1d root-cause investigation."""
    parser = build_parser()
    ...
    try:
        ...
        if bool(getattr(args, "debug", False)):
            _logger.exception("1d root-cause investigation failed")
