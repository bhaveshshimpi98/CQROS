"""CQROS position dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers position
    partitions and executes ``PositionVerifier`` across the universe with
    bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for position dataset verification
    - Discover available position partitions through ``PositionRepository``
    - Load partitions and verify them with ``PositionVerifier``
...
Dependencies:
    ...
    ``cqros.positions``, and ``cqros.storage``.
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-POSITIONS-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-POSITIONS-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-POSITIONS-003"
_ERROR_MODEL: Final[str] = "CLI-VERIFY-POSITIONS-004"
_ERROR_VERSION: Final[str] = "CLI-VERIFY-POSITIONS-005"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-POSITIONS-006"

_VERIFIER_NAME: Final[str] = "PositionVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Positions"
...
class VerifyPositionOptions:
    """Immutable CLI options for position dataset verification.

    Attributes:
        storage_root: Storage root containing ``positions``.
...
class DiscoveredWorkItem:
    """One discovered position partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyPositionSummary:
    """Immutable aggregate summary for a position verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always positions tier).
        ...
        duplicate_timestamps: Sum of duplicate-position-id counters.
...
def build_parser() -> argparse.ArgumentParser:
    """Create the position dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-positions",
        description=("Verify CQROS position datasets across the discovered " "positions universe."),
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the positions tier."
        ),
    )
    parser.add_argument(
        "--model",
        ...
        metavar="NAME",
        help=(
            "Optional model-name filter applied to loaded position rows. "
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        help=(
            "Optional model-version filter applied to loaded position rows. "
...
def build_options(args: argparse.Namespace) -> VerifyPositionOptions:
    """Map parsed CLI arguments onto ``VerifyPositionOptions``.
...
def discover_work(
    ...
    options: VerifyPositionOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover position partitions matching the CLI filters.

    Args:
        repository: Position repository providing discovery APIs.
...
def format_summary(summary: VerifyPositionSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the position dataset verification CLI."""
    parser = build_parser()
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-positions-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    model: str | None,
    version: str | None,
) -> PositionTaskResult | None:
    """Verify one position year partition synchronously."""
    try:
...
def _report_task_failure(result: PositionTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed position partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
