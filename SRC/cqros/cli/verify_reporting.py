"""CQROS reporting dataset verification CLI.

Purpose:
    Provide an argparse-based production entry point that discovers
    reporting partitions and executes ``ReportingVerifier`` across
    the universe with bounded symbol concurrency.

Responsibilities:
    - Parse CLI arguments for reporting dataset verification
    - Discover available reporting partitions through ``ReportingRepository``
    - Load partitions and verify them with ``ReportingVerifier``
...
Dependencies:
    ...
    ``cqros.reporting``, ``cqros.processing.verification.report``, and
    ``cqros.storage``.
...
from cqros.reporting import ReportingRepository, ReportingVerifier
...
__all__ = [
    "DiscoveredWorkItem",
    "ReportingTaskResult",
    "VerifyReportingOptions",
    "VerifyReportingSummary",
    "build_options",
    "build_parser",
    "discover_work",
...
_ERROR_WORKERS: Final[str] = "CLI-VERIFY-REPORTING-001"
_ERROR_TIMEFRAME: Final[str] = "CLI-VERIFY-REPORTING-002"
_ERROR_YEAR: Final[str] = "CLI-VERIFY-REPORTING-003"
_ERROR_MANAGER: Final[str] = "CLI-VERIFY-REPORTING-004"

_VERIFIER_NAME: Final[str] = "ReportingVerifier"
_DATASET_DISPLAY_NAME: Final[str] = "Reporting"
...
class VerifyReportingOptions:
    """Immutable CLI options for reporting dataset verification.

    Attributes:
        storage_root: Storage root containing ``reporting``.
...
class DiscoveredWorkItem:
    """One discovered reporting partition group ready for verification.
...
@dataclass(frozen=True, slots=True)
class VerifyReportingSummary:
    """Immutable aggregate summary for a reporting verification run.

    Attributes:
        symbols_verified: Unique symbols for which verification was attempted.
        datasets_verified: Unique datasets attempted (always reporting tier).
...
def build_parser() -> argparse.ArgumentParser:
    """Create the reporting dataset verification argument parser.

    Returns:
        Configured ``ArgumentParser`` for verification flags.
    """
    parser = argparse.ArgumentParser(
        prog="cqros-verify-reporting",
        description=("Verify CQROS reporting datasets across the discovered reporting universe."),
    )
    parser.add_argument(
        "--manager",
        dest="manager",
        ...
        help=(
            ...
            "Omit to verify all managers present under the reporting tier."
        ),
    )
    parser.add_argument(
        "--symbols",
...
def build_options(args: argparse.Namespace) -> VerifyReportingOptions:
    """Map parsed CLI arguments onto ``VerifyReportingOptions``.
...
def discover_work(
    ...
    options: VerifyReportingOptions,
) -> tuple[DiscoveredWorkItem, ...]:
    """Discover reporting partitions matching the CLI filters.

    Args:
        repository: Reporting repository providing discovery APIs.
...
def format_summary(summary: VerifyReportingSummary) -> str:
...
async def main(argv: Sequence[str] | None = None) -> int:
    """Run the reporting dataset verification CLI.
...
async def run_verification(
    *,
    ...
    Args:
        repository: Reporting partition repository.
        verifier: Reporting verifier instance.
...
async def _run_worker_pool(
    *,
    ...
    worker_tasks = [
        asyncio.create_task(worker(), name=f"verify-reporting-worker-{index}")
...
def _verify_partition(
    ...
    *,
    ...
    """Verify one reporting year partition synchronously."""
    try:
...
def _report_task_failure(result: ReportingTaskResult) -> None:
...
def _log_partition_failure(
    *,
    ...
    if debug:
        _logger.exception(
            "Failed reporting partition verification; continuing",
            extra=log_extra,
            exc_info=True,
        )
    else:
