"""CQROS open interest change feature.

Purpose:
    Compute one-period open interest change from raw open-interest repository
    data.

Responsibilities:
    - Expose immutable ``OIChangeFeature`` metadata
    - Append an ``oi_change`` column using Polars expressions only
...
__all__ = ["OIChangeFeature"]

_OI_COLUMN: Final[str] = "open_interest"
_OUTPUT_COLUMN: Final[str] = "oi_change"
_FEATURE_NAME: Final[str] = "oi_change"
_FEATURE_VERSION: Final[str] = "1.0.0"
_FEATURE_CATEGORY: Final[str] = "open_interest"
_FEATURE_DESCRIPTION: Final[str] = (
    "One-period change in open interest as open_interest minus previous value."
)
_ERROR_MISSING_OI: Final[str] = "FEATURE-OI-CHANGE-001"


@dataclass(frozen=True, slots=True)
class OIChangeFeature(BaseFeature):
    """One-period absolute change in open interest.

    Computes ``open_interest - open_interest.shift(1)`` and appends the result
    as ``oi_change``. The first row is null. Missing values are not filled.

    Attributes:
        name: Stable feature identifier (``oi_change``).
    ...
    def transform(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append OI change without mutating ``frame``.

        Args:
            frame: Input DataFrame containing an ``open_interest`` column.

        Returns:
            A new DataFrame with all original columns plus ``oi_change``.
        ...
        change_expr = oi - oi.shift(1)  # pyright: ignore[reportUnknownMemberType]
