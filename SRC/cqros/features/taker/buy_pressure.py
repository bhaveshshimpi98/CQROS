"""CQROS taker buy pressure feature.

Purpose:
    Compute taker buy pressure from raw taker-volume repository data.

Responsibilities:
    - Expose immutable ``BuyPressureFeature`` metadata
    - Append a ``buy_pressure`` column using Polars expressions only
...
__all__ = ["BuyPressureFeature"]

_BUY_COLUMN: Final[str] = "buy_volume"
_SELL_COLUMN: Final[str] = "sell_volume"
_OUTPUT_COLUMN: Final[str] = "buy_pressure"
_FEATURE_NAME: Final[str] = "buy_pressure"
_FEATURE_VERSION: Final[str] = "1.0.0"
_FEATURE_CATEGORY: Final[str] = "taker"
_FEATURE_DESCRIPTION: Final[str] = "Taker buy pressure as buy_volume / (buy_volume + sell_volume)."
_ERROR_MISSING_COLUMN: Final[str] = "FEATURE-BUY-PRESSURE-001"


@dataclass(frozen=True, slots=True)
class BuyPressureFeature(BaseFeature):
    """Taker buy pressure share of total taker volume.

    Computes ``buy_volume / (buy_volume + sell_volume)``. When total volume is
    zero the output is null. Missing values are not filled.

    Attributes:
        name: Stable feature identifier (``buy_pressure``).
    ...
    def transform(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append buy pressure without mutating ``frame``.

        Args:
            frame: Input DataFrame containing ``buy_volume`` and ``sell_volume``.

        Returns:
            A new DataFrame with all original columns plus ``buy_pressure``.
        ...
        total = buy + sell
        pressure_expr = pl.when(total == 0).then(None).otherwise(buy / total)
