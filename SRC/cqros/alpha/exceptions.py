"""CQROS Alpha Engine exception hierarchy.

Purpose:
    Provide alpha-specific exception types used by engines, pipelines,
    repositories, and verification workflows.

Responsibilities:
    - Define the package ``AlphaException`` root under ``ResearchError``
    - Expose ``AlphaError`` for input and contract validation
...
class AlphaException(ResearchError):  # noqa: N818
    """Raised when alpha workflows or artifacts fail."""

    __slots__ = ()


class AlphaError(AlphaException):
    """Raised when alpha inputs, outputs, or contracts fail."""
