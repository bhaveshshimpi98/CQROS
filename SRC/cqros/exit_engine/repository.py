"""CQROS exit-engine recommendation dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    exit-engine year partitions produced by CQROS Exit Engine evaluation.

Responsibilities:
    - Resolve storage locations for exit-engine year partitions via
      ``StorageLayout.exit_engine_path``
    - Persist, load, check existence, and delete exit-engine partitions
    ...
    - Remain free of exit math, schema validation, and trading logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``ExitEnginePartitionRef``, ``ExitRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical
    exit-engine schema validation belongs to ``ExitEnginePipeline`` and
    ``ExitEngineVerifier``. This module must not import exit engines.
...
class ExitEnginePartitionRef:
    """Identity of one discovered exit-engine year partition.
...
class ExitRepository:
    """Repository facade for canonical exit-engine datasets.

    Callers identify partitions by manager, exchange, market, symbol,
    timeframe, and year. Paths are composed privately via
    ``StorageLayout.exit_engine_path`` and never returned. Persistence is
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as exit-engine partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[ExitEnginePartitionRef, ...]:
        """Discover exit-engine year partitions matching optional filters.

        Missing exit-engine trees are skipped. Only year partitions that exist
        as ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the exit-engine tier.
        ...
        items: list[ExitEnginePartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether an exit-engine year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.exit_engine_path``. Parquet contents
        ...
        Returns:
            ``True`` when the yearly exit-engine partition exists; otherwise
            ``False``.
        """
        path = self._layout.exit_engine_path(
            manager,
            exchange,
            market,
            symbol,
        ...
        self._logger.debug(
            ("Exit-engine dataset exists" if present else "Exit-engine dataset does not exist"),
            extra=_exit_engine_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def save(
        ...
        *,
        ...
        """Persist an exit-engine year partition, overwriting when present.

        Args:
            dataframe: Exit-engine frame to store.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
    ...
    def load(
        ...
        *,
        ...
        """Load an exit-engine year partition.
        ...
        Returns:
            Eagerly loaded exit-engine DataFrame.
        """
        path = self._layout.exit_engine_path(
            manager,
            exchange,
    ...
    def delete(
        ...
        *,
        ...
        """Delete an exit-engine year partition.
    ...
    def _exit_engine_root(self) -> Path:
        """Return the exit-engine directory under the storage root."""
        return self._layout.root / STORAGE_DIR_EXIT_ENGINE
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the exit-engine directory for manager/exchange/market."""
        return self._exit_engine_root() / manager / exchange / market
...
def _exit_engine_log_extra(
    *,
    ...
    """Build structured log fields for an exit-engine dataset operation."""
