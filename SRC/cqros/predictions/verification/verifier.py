"""CQROS merged prediction dataset verification.

Purpose:
    Inspect merged prediction frames and report structural findings without
    cleaning or mutating input data.

Responsibilities:
    - Validate required merged-prediction columns and expected dtypes
...
Dependencies:
    ``polars``, the Python standard library, ``cqros.predictions.schema``,
    ``cqros.predictions.verification.exceptions``,
...
# Non-primary-key value columns inspected for NaN / infinite numeric issues.
...
_WARN_NUMERIC: Final[str] = "Infinite prediction values detected."
...
class PredictionVerifier(BaseVerifier):
    """Deterministic merged-prediction verifier that reports findings only.

    Inspects structural quality of a merged prediction frame against
    ``cqros.predictions.schema``. Does not clean rows, fill gaps, sort
    timestamps, mutate values, access storage, generate predictions, or apply
    trading logic.
    """
    ...
    def verify(self, frame: pl.DataFrame) -> VerificationReport:
        ...
        Args:
            frame: Input merged prediction DataFrame. Must not be mutated.
        ...
        Raises:
            PredictionValidationError: If any required column is missing or
                column dtypes do not match the merged prediction schema.
        """
    ...
    def _validate_required_columns(
        ...
        Raises:
            PredictionValidationError: If one or more required columns are
                missing.
        """
    ...
    def _validate_column_dtypes(self, frame: pl.DataFrame) -> None:
        ...
        Raises:
            PredictionValidationError: If one or more column dtypes do not
                match ``COLUMN_DTYPES``.
        """
    ...
    def _count_duplicate_key_rows(self, frame: pl.DataFrame) -> int:
        ...
        Uses keep-first semantics over the canonical merged-prediction primary
        key.

        Args:
            frame: Input prediction DataFrame. Must not be mutated.
    ...
    def _count_infinite_rows(self, frame: pl.DataFrame) -> int:
        ...
        Non-floating columns listed in value columns are ignored. NaN is not
        treated as infinite.
        ...
        Returns:
            Count of rows with one or more infinite prediction values.
        """
        if frame.height == 0:
            return 0
