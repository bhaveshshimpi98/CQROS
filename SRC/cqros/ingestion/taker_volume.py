"""CQROS Binance historical taker buy/sell volume downloader.

Purpose:
    Plan and execute historical taker buy/sell volume downloads from Binance
    USDⓈ-M Futures, persisting raw taker-volume partitions through
    ``MarketDataRepository``.

Responsibilities:
    - Represent immutable ``TakerVolumeDownloadTask`` time-range units
    - Split long ranges into sequential tasks via
      ``TakerVolumeDownloadPlanner``
    - Fetch taker volume history through ``BinanceClient`` and persist via
      repository
    - Paginate until each requested window is complete
    - Validate every exchange response before conversion
    - Derive ``buy_sell_ratio`` as ``buy_volume / sell_volume`` (null when
      ``sell_volume`` is zero)
    - Keep planning and execution separated
    - Remain free of filesystem path construction, feature engineering beyond
      the stored ratio, and research logic
...
Public API:
    ``TakerVolumeDownloadTask``, ``TakerVolumeDownloadPlanner``,
    ``TakerVolumeDownloader``, and the default limit / period / chunk
...
# Binance ``takerlongshortRatio`` periods and max page size.
TAKER_VOLUME_PERIODS: Final[frozenset[str]] = frozenset(
    {
        "5m",
        "15m",
        "30m",
...
DEFAULT_TAKER_VOLUME_PERIOD: Final[Timeframe] = "5m"
DEFAULT_TAKER_VOLUME_REQUEST_LIMIT: Final[int] = 500
DEFAULT_TAKER_VOLUME_CHUNK_SAFETY_FACTOR: Final[float] = DEFAULT_CHUNK_SAFETY_FACTOR

_EXCHANGE: Final[Exchange] = EXCHANGE_BINANCE
_MARKET: Final[Market] = MARKET_USDT_PERPETUAL

_TAKER_VOLUME_SCHEMA: Final[pl.Schema] = pl.Schema(
    {
        "symbol": pl.String,
        "timestamp": pl.Int64,
...
class TakerVolumeDownloadTask:
    """Immutable unit of work for a single taker-volume download window.

    Attributes:
        symbol: Tradeable symbol (for example ``BTCUSDT``).
        period: Binance taker-volume aggregation period (for example ``5m``).
...
class TakerVolumeDownloadPlanner:
    """Split a historical taker-volume range into sequential download tasks.
    ...
    Args:
        ...
        request_limit: Maximum taker-volume rows per request used by the
    ...
    def __init__(
        ...
        *,
        ...
        request_limit: int = DEFAULT_TAKER_VOLUME_REQUEST_LIMIT,
        safety_factor: float = DEFAULT_TAKER_VOLUME_CHUNK_SAFETY_FACTOR,
    ) -> None:
    ...
    def plan(
        ...
        *,
        ...
        start_time: UnixTimestampMs,
        end_time: UnixTimestampMs,
    ) -> tuple[TakerVolumeDownloadTask, ...]:
        """Produce sequential taker-volume download tasks covering the range.

        Args:
            symbol: Tradeable symbol (for example ``BTCUSDT``).
            period: Binance taker-volume aggregation period.
            start_time: Inclusive range start as UTC Unix milliseconds.
            end_time: Inclusive range end as UTC Unix milliseconds.

        Returns:
            Immutable tuple of ``TakerVolumeDownloadTask`` instances in
        ...
        tasks: list[TakerVolumeDownloadTask] = []
        cursor = start_time
    ...
    def _resolve_chunk_size_ms(self, period: Timeframe) -> int:
        ...
        Args:
            period: Validated taker-volume aggregation period.
        ...
        if chunk_size <= 0:
            raise ValidationError(
                "derived taker-volume chunk size must be greater than 0",
                error_code="INGESTION-TAKER-VOLUME-004",
                details={
                    "period": period,
                    "period_ms": period_ms,
...
class TakerVolumeDownloader:
    """Execute planned historical taker-volume downloads into storage.

    Planning remains the responsibility of ``TakerVolumeDownloadPlanner``.
    This class fetches taker buy/sell volume for each task, converts vendor
    ...
    Args:
        client: Open ``BinanceClient`` used for taker-volume requests.
        repository: Market-data repository used for taker-volume persistence.
        planner: Planner that splits long ranges into sequential tasks.
        request_limit: Maximum taker-volume records requested per API call.
    ...
    _planner: TakerVolumeDownloadPlanner
    _request_limit: int
    ...
    def __init__(
        ...
        planner: TakerVolumeDownloadPlanner,
        *,
        request_limit: int = DEFAULT_TAKER_VOLUME_REQUEST_LIMIT,
        ...
        Args:
            ...
            repository: Repository used to persist taker-volume partitions.
            planner: Taker-volume download range planner.
            request_limit: Maximum taker-volume records per API call.
        ...
        if request_limit <= 0:
            raise ValidationError(
                "request_limit must be greater than 0",
                error_code="INGESTION-TAKER-VOLUME-005",
                details={"parameter": "request_limit", "value": request_limit},
            )
        self._client = client
    ...
    def request_limit(self) -> int:
        """Return the configured per-request taker-volume limit."""
    ...
    async def fetch_symbol(
        ...
        *,
        ...
        start_time: UnixTimestampMs,
        end_time: UnixTimestampMs,
        period: Timeframe = DEFAULT_TAKER_VOLUME_PERIOD,
    ) -> pl.DataFrame:
        """Download historical taker volume for a symbol without persisting.
        ...
        Returns:
            Combined taker-volume DataFrame for the requested range. Empty when
        ...
        self._logger.info(
            "Starting symbol taker-volume fetch",
            extra={
                "exchange": _EXCHANGE,
                "market": _MARKET,
                "symbol": symbol,
        ...
        if not frames:
            self._logger.info(
                "Symbol taker-volume fetch produced no rows",
                extra={
                    "symbol": symbol,
                    "period": period,
                    "start_time": start_time,
            ...
            return pl.DataFrame(schema=_TAKER_VOLUME_SCHEMA)

        combined = pl.concat(frames, how="vertical")
        self._logger.info(
            "Completed symbol taker-volume fetch",
            extra={
                "symbol": symbol,
                "period": period,
                "start_time": start_time,
    ...
    async def download_symbol(
        ...
        *,
        ...
        """Download and persist historical taker volume for a single symbol.
        ...
        latest = coerce_latest_timestamp(
            self._repository.get_latest_taker_volume_timestamp(
                exchange=_EXCHANGE,
                market=_MARKET,
                symbol=symbol,
                timeframe=period,
        ...
        if window.status is DownloadStatus.SKIPPED:
            self._logger.info(
                "Skipping taker-volume download; storage already current",
                extra={
                    "symbol": symbol,
                    "period": period,
                    "start_time": start_time,
        ...
        if combined.height > 0:
            self._persist_taker_volume(combined, symbol=symbol, period=period)
            self._logger.info(
                "Completed symbol taker-volume download",
                extra={
                    "symbol": symbol,
                    "period": period,
                    "start_time": window.start_time,
    ...
    async def download_universe(
        ...
        *,
        ...
        """Download and persist historical taker volume for multiple symbols.
        ...
        """
        self._logger.info(
            "Starting universe taker-volume download",
            extra={
                "exchange": _EXCHANGE,
                "market": _MARKET,
        ...
        self._logger.info(
            "Completed universe taker-volume download",
            extra={
                "symbol_count": len(symbols),
                "period": period,
                "start_time": start_time,
    ...
    async def _execute_task(self, task: TakerVolumeDownloadTask) -> pl.DataFrame:
        """Fetch all taker-volume rows for a single task window.

        Args:
            task: Immutable download window to execute.

        Returns:
            Taker-volume DataFrame for the task range. May be empty.
        ...
        """
        self._logger.debug(
            "Executing taker-volume download task",
            extra={
                "symbol": task.symbol,
                "period": task.period,
        ...
        while cursor <= task.end_time:
            payload = await self._client.get_taker_buy_sell_volume(
                task.symbol,
                task.period,
                start_time=cursor,
                end_time=task.end_time,
            ...
            frames.append(_taker_volume_rows_to_dataframe(rows))
    ...
    def _persist_taker_volume(
        ...
        *,
        ...
        """Persist a taker-volume frame as year partitions.

        Args:
            frame: Combined taker-volume rows for one symbol/period download.
        ...
        for year in years:
            ...
            self._repository.save_taker_volume(
                partition,
                exchange=_EXCHANGE,
                market=_MARKET,
                symbol=symbol,
            ...
            self._logger.debug(
                "Persisted taker-volume year partition",
                extra={
                    "symbol": symbol,
                    "period": period,
                    "year": int(year),
...
def _require_unix_ms(value: object, *, parameter: str) -> UnixTimestampMs:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise ValidationError(
            ...
            error_code="INGESTION-TAKER-VOLUME-006",
...
def _require_period(period: object) -> Timeframe:
    """Validate a Binance taker-volume aggregation period.
    ...
    if not isinstance(period, str):
        raise ValidationError(
            "period must be a string taker-volume interval",
            error_code="INGESTION-TAKER-VOLUME-007",
            details={"parameter": "period", "type": type(period).__name__},
        )
    if period not in TAKER_VOLUME_PERIODS:
        raise ValidationError(
            f"unsupported taker-volume period: {period!r}",
            error_code="INGESTION-TAKER-VOLUME-008",
            details={
                "parameter": "period",
                "value": period,
                "supported": sorted(TAKER_VOLUME_PERIODS),
            },
        )
    return period


def _parse_taker_volume_rows(
    payload: object,
    *,
    symbol: Symbol,
) -> list[tuple[str, int, float, float, float | None]]:
    """Parse a Binance taker-volume payload into typed row tuples.

    Args:
        payload: Decoded JSON body from ``get_taker_buy_sell_volume``.
        symbol: Symbol requested from the exchange, stamped onto each row.

    Returns:
        List of
        ``(symbol, timestamp, buy_volume, sell_volume, buy_sell_ratio)``
        tuples. ``buy_sell_ratio`` is ``None`` when ``sell_volume`` is zero.
    ...
    if not isinstance(payload, Sequence) or isinstance(payload, (str, bytes)):
        raise ValidationError(
            "taker-volume payload must be a sequence",
            error_code="INGESTION-TAKER-VOLUME-009",
...
def _parse_taker_volume_row(
    entry: object,
    *,
    index: int,
    symbol: Symbol,
) -> tuple[str, int, float, float, float | None]:
    """Parse a single Binance taker-volume object into typed fields.

    Args:
        entry: Raw taker-volume object from the exchange payload.
        index: Row index for error context.
        symbol: Symbol stamped onto the row.
    ...
    if not isinstance(entry, Mapping):
        raise ValidationError(
            "taker-volume row must be a mapping",
            error_code="INGESTION-TAKER-VOLUME-010",
            details={"index": index, "type": type(entry).__name__},
        )

    values = cast(Mapping[str, object], entry)
    required = ("timestamp", "buyVol", "sellVol")
    missing = [field for field in required if field not in values]
    if missing:
        raise ValidationError(
            "taker-volume row is missing required fields",
            error_code="INGESTION-TAKER-VOLUME-011",
            details={"index": index, "missing": missing},
        )

    buy_volume = _as_float(values["buyVol"], field="buy_volume", index=index)
    sell_volume = _as_float(values["sellVol"], field="sell_volume", index=index)
    if "buySellRatio" in values:
        buy_sell_ratio: float | None = _as_float(
            values["buySellRatio"],
            field="buy_sell_ratio",
            index=index,
        )
    else:
        buy_sell_ratio = _buy_sell_ratio(buy_volume, sell_volume)
    return (
        symbol,
...
def _buy_sell_ratio(buy_volume: float, sell_volume: float) -> float | None:
    """Compute ``buy_volume / sell_volume``, or ``None`` when sell is zero.
    ...
    Returns:
        Ratio, or ``None`` when ``sell_volume`` is exactly zero.
    """
    if sell_volume == 0.0:
        return None
...
def _taker_volume_rows_to_dataframe(
    rows: Sequence[tuple[str, int, float, float, float | None]],
) -> pl.DataFrame:
    """Convert parsed taker-volume rows into a canonical DataFrame.

    Args:
        rows: Parsed taker-volume tuples.

    Returns:
        Eager Polars DataFrame with the canonical raw taker-volume columns.
    """
    return pl.DataFrame(
        {
            "symbol": [row[0] for row in rows],
            "timestamp": [row[1] for row in rows],
            "buy_volume": [row[2] for row in rows],
            "sell_volume": [row[3] for row in rows],
            "buy_sell_ratio": [row[4] for row in rows],
        },
        schema=_TAKER_VOLUME_SCHEMA,
    )


def _as_int(value: object, *, field: str, index: int) -> int:
    """Coerce a taker-volume field to ``int``.
    ...
    if isinstance(value, bool) or not isinstance(value, (int, str)):
        raise ValidationError(
            f"taker_volume.{field} must be an int-compatible value",
            error_code="INGESTION-TAKER-VOLUME-012",
...
def _as_float(value: object, *, field: str, index: int) -> float:
    """Coerce a taker-volume field to ``float``.
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float, str)):
        raise ValidationError(
            f"taker_volume.{field} must be a float-compatible value",
            error_code="INGESTION-TAKER-VOLUME-013",
            details={"index": index, "field": field, "type": type(value).__name__},
        )
    try:
        return float(value)
