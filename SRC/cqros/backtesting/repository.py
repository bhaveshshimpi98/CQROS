"""CQROS backtesting performance dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    backtesting year partitions produced by the CQROS Backtesting Engine
    performance reconstruction.

Responsibilities:
    - Resolve storage locations for backtesting year partitions via
      ``StorageLayout.backtesting_path``
    - Persist, load, check existence, and delete backtesting partitions
    ...
    - Remain free of performance math, schema validation, and trading logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``BacktestingPartitionRef``, ``BacktestingRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical backtesting
    schema validation belongs to ``BacktestingPipeline`` and
    ``BacktestingVerifier``. This module must not import backtesting engines.
"""
...
class BacktestingPartitionRef:
    """Identity of one discovered backtesting year partition.
...
class BacktestingRepository:
    """Repository facade for canonical backtesting datasets.

    Callers identify partitions by manager, exchange, market, symbol,
    timeframe, and year. Paths are composed privately via
    ``StorageLayout.backtesting_path`` and never returned. Persistence is
    ...
    provided; schema validation is not performed here.
    ...
    def discover_managers(self) -> tuple[str, ...]:
        """Return sorted manager identifiers with at least one backtesting partition.

        Returns:
            Deterministically ordered manager identifiers discovered on disk.
        """
        base = self._backtesting_root()
        if not base.is_dir():
            return ()
    ...
    def discover_symbols(
        ...
        *,
        ...
        """Return sorted symbols with at least one backtesting partition for ``manager``.
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as backtesting parquet partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[BacktestingPartitionRef, ...]:
        """Discover backtesting year partitions matching optional filters.

        Missing backtesting trees are skipped. Only year partitions that exist
        as ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the backtesting tier.
        ...
        items: list[BacktestingPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a backtesting year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.backtesting_path``. Parquet contents
        ...
        Returns:
            ``True`` when the yearly backtesting partition exists; otherwise
            ``False``.
        """
        path = self._layout.backtesting_path(
            manager,
            exchange,
            market,
            symbol,
        ...
        self._logger.debug(
            "Backtesting dataset exists" if present else "Backtesting dataset does not exist",
            extra=_backtesting_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def save(
        ...
        *,
        ...
        """Persist a backtesting year partition.

        Args:
            dataframe: Backtesting frame to store.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
    ...
    def load(
        ...
        *,
        ...
        """Load a backtesting year partition.
        ...
        Returns:
            Eagerly loaded backtesting DataFrame.
        """
        path = self._layout.backtesting_path(
            manager,
            exchange,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a backtesting year partition.
    ...
    def _backtesting_root(self) -> Path:
        """Return the backtesting directory under the storage root."""
        return self._layout.root / STORAGE_DIR_BACKTESTING
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the backtesting directory for manager/exchange/market."""
        return self._backtesting_root() / manager / exchange / market
...
def _backtesting_log_extra(
    *,
    ...
    """Build structured log fields for a backtesting dataset operation."""
