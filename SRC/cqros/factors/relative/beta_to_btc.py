"""CQROS beta-to-BTC research factor.

Purpose:
    Compute rolling OLS beta of an asset versus Bitcoin returns as a pure
    relative-value alpha factor for the Factor Research Engine.

Responsibilities:
    - Expose immutable ``BetaToBTCFactor`` metadata
    - Append a ``beta_to_btc`` column using Polars expressions only
...
__all__ = ["BetaToBTCFactor"]

_ASSET_COLUMN: Final[str] = "asset_return"
_BTC_COLUMN: Final[str] = "btc_return"
_OUTPUT_COLUMN: Final[str] = "beta_to_btc"
_FACTOR_NAME: Final[str] = "beta_to_btc"
_FACTOR_VERSION: Final[str] = "1.0.0"
_FACTOR_CATEGORY: Final[str] = "relative"
_FACTOR_DESCRIPTION: Final[str] = (
    "Rolling beta of asset returns versus BTC returns as population "
    "covariance divided by BTC variance."
)
_ERROR_LOOKBACK: Final[str] = "FACTOR-BETA-TO-BTC-001"
_ERROR_MISSING_COLUMN: Final[str] = "FACTOR-BETA-TO-BTC-002"


@dataclass(frozen=True, slots=True)
class BetaToBTCFactor(BaseFactor):
    """Beta-to-BTC alpha factor from rolling population covariance.

    Computes ``cov(asset_return, btc_return) / var(btc_return)`` over
    ``lookback`` using population moments (``ddof=0``) and appends the
    result as ``beta_to_btc``. Returns null when BTC variance is zero.
    ...
    Attributes:
        name: Stable factor identifier (``beta_to_btc``).
    ...
    def compute(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append beta-to-BTC without mutating ``frame``.
        ...
        Returns:
            A new DataFrame with all original columns plus ``beta_to_btc``.
        ...
        variance = btc.rolling_var(  # pyright: ignore[reportUnknownMemberType]
            window_size=window,
            ddof=0,
        )
        beta_expr = (
