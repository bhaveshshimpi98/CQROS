"""CQROS relative strength versus ETH research factor.

Purpose:
    Compute rolling cumulative-return relative strength of an asset versus
    Ethereum returns as a pure relative-value alpha factor for the Factor
    Research Engine.

Responsibilities:
    - Expose immutable ``RelativeStrengthETHFactor`` metadata
    - Append a ``relative_strength_eth`` column using Polars expressions only
...
__all__ = ["RelativeStrengthETHFactor"]

_ASSET_COLUMN: Final[str] = "asset_return"
_ETH_COLUMN: Final[str] = "eth_return"
_OUTPUT_COLUMN: Final[str] = "relative_strength_eth"
_FACTOR_NAME: Final[str] = "relative_strength_eth"
_FACTOR_VERSION: Final[str] = "1.0.0"
_FACTOR_CATEGORY: Final[str] = "relative"
_FACTOR_DESCRIPTION: Final[str] = (
    "Relative strength as asset cumulative return minus ETH cumulative return "
    "over a lookback window."
)
_ERROR_LOOKBACK: Final[str] = "FACTOR-RELATIVE-STRENGTH-ETH-001"
_ERROR_MISSING_COLUMN: Final[str] = "FACTOR-RELATIVE-STRENGTH-ETH-002"
...
class RelativeStrengthETHFactor(BaseFactor):
    """Relative strength alpha factor versus Ethereum returns.

    Computes rolling compounded cumulative returns
    ``product(1 + r) - 1`` for ``asset_return`` and ``eth_return``, then
    appends their difference as ``relative_strength_eth``. Incomplete
    ...
    Attributes:
        name: Stable factor identifier (``relative_strength_eth``).
    ...
    def compute(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append ETH relative strength without mutating ``frame``.
        ...
        Returns:
            ...
            ``relative_strength_eth``. Incomplete windows are null.
        ...
        eth_cum = _rolling_cumulative_return(pl.col(_ETH_COLUMN), self.lookback)
        strength_expr = asset_cum - eth_cum
