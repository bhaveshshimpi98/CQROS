"""CQROS trend slope research factor.

Purpose:
    Compute the rolling OLS slope of log(close) against a relative time index
    as a pure price alpha factor for the Factor Research Engine.

Responsibilities:
    - Expose immutable ``TrendSlopeFactor`` metadata
    - Append a ``trend_slope`` column using Polars expressions only
...
__all__ = ["TrendSlopeFactor"]

_CLOSE_COLUMN: Final[str] = "close"
_OUTPUT_COLUMN: Final[str] = "trend_slope"
_FACTOR_NAME: Final[str] = "trend_slope"
_FACTOR_VERSION: Final[str] = "1.0.0"
_FACTOR_CATEGORY: Final[str] = "price"
_FACTOR_DESCRIPTION: Final[str] = "Rolling OLS slope of log(close) against a relative time index."
_ERROR_LOOKBACK: Final[str] = "FACTOR-TREND-SLOPE-001"
_ERROR_MISSING_CLOSE: Final[str] = "FACTOR-TREND-SLOPE-002"


@dataclass(frozen=True, slots=True)
class TrendSlopeFactor(BaseFactor):
    """Trend slope alpha factor from rolling OLS on log(close).

    Fits ``log(close) ~ a + b * x`` over each trailing ``lookback`` window
    where ``x`` is the relative index ``0 .. lookback-1``, and appends the
    slope ``b`` as ``trend_slope``. The first ``lookback - 1`` rows are null.
    Missing values are never filled. The input DataFrame is never mutated.

    This is a research alpha factor, not a technical indicator.

    Attributes:
        name: Stable factor identifier (``trend_slope``).
    ...
    def compute(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append trend slope without mutating ``frame``.

        Args:
            frame: Input research DataFrame containing a ``close`` column.

        Returns:
            A new DataFrame with all original columns plus ``trend_slope``.
            The first ``lookback - 1`` rows of ``trend_slope`` are null.
