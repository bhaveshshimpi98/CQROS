"""CQROS price z-score research factor.

Purpose:
    Compute the rolling z-score of close prices as a pure price alpha factor
    for the Factor Research Engine.

Responsibilities:
    - Expose immutable ``PriceZScoreFactor`` metadata
    - Append a ``price_zscore`` column using Polars expressions only
...
__all__ = ["PriceZScoreFactor"]

_CLOSE_COLUMN: Final[str] = "close"
_OUTPUT_COLUMN: Final[str] = "price_zscore"
_FACTOR_NAME: Final[str] = "price_zscore"
_FACTOR_VERSION: Final[str] = "1.0.0"
_FACTOR_CATEGORY: Final[str] = "price"
_FACTOR_DESCRIPTION: Final[str] = (
    "Rolling z-score of close versus its rolling mean and standard deviation."
)
_ERROR_LOOKBACK: Final[str] = "FACTOR-PRICE-ZSCORE-001"
_ERROR_MISSING_CLOSE: Final[str] = "FACTOR-PRICE-ZSCORE-002"


@dataclass(frozen=True, slots=True)
class PriceZScoreFactor(BaseFactor):
    """Price z-score alpha factor from rolling mean and standard deviation.
    ...
    Attributes:
        name: Stable factor identifier (``price_zscore``).
    ...
    def compute(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append price z-score without mutating ``frame``.

        Args:
            frame: Input research DataFrame containing a ``close`` column.

        Returns:
            A new DataFrame with all original columns plus ``price_zscore``.
            Incomplete windows of ``price_zscore`` are null.
        ...
        zscore_expr = rolling_zscore_expr(pl.col(_CLOSE_COLUMN), window_size=self.lookback)
