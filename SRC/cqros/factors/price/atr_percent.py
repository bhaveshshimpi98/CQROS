"""CQROS ATR percent research factor.

Purpose:
    Compute average true range as a fraction of close as a pure price alpha
    factor for the Factor Research Engine.

Responsibilities:
    - Expose immutable ``ATRPercentFactor`` metadata
    - Append an ``atr_percent`` column using Polars expressions only
...
__all__ = ["ATRPercentFactor"]

_HIGH_COLUMN: Final[str] = "high"
_LOW_COLUMN: Final[str] = "low"
_CLOSE_COLUMN: Final[str] = "close"
_OUTPUT_COLUMN: Final[str] = "atr_percent"
_FACTOR_NAME: Final[str] = "atr_percent"
_FACTOR_VERSION: Final[str] = "1.0.0"
_FACTOR_CATEGORY: Final[str] = "price"
_FACTOR_DESCRIPTION: Final[str] = "Average true range as a fraction of close."
_ERROR_LOOKBACK: Final[str] = "FACTOR-ATR-PERCENT-001"
_ERROR_MISSING_COLUMN: Final[str] = "FACTOR-ATR-PERCENT-002"


@dataclass(frozen=True, slots=True)
class ATRPercentFactor(BaseFactor):
    """ATR percent alpha factor as ATR divided by close.

    True range is ``max(high - low, |high - prev_close|, |low - prev_close|)``.
    ATR is the rolling mean of true range over ``lookback``. The factor
    appends ``ATR / close`` as ``atr_percent``. Returns null when close is
    zero. The first ``lookback - 1`` rows are null. Missing values are never
    filled. The input DataFrame is never mutated.

    This is a research alpha factor, not a technical indicator.

    Attributes:
        name: Stable factor identifier (``atr_percent``).
    ...
    def compute(self, frame: pl.DataFrame) -> pl.DataFrame:
        """Append ATR percent without mutating ``frame``.
        ...
        Returns:
            A new DataFrame with all original columns plus ``atr_percent``.
            The first ``lookback - 1`` rows of ``atr_percent`` are null.
        ...
        percent_expr = (
            pl.when(close != 0).then(atr / close).otherwise(None)
        )  # pyright: ignore[reportUnknownMemberType]
        return frame.with_columns(  # pyright: ignore[reportUnknownMemberType]
            percent_expr.cast(pl.Float64).alias(_OUTPUT_COLUMN)
        )
