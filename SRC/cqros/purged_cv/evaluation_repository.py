"""CQROS purged-CV evaluation-result repository.

Purpose:
    Provide a path-free facade for persisting and retrieving Purged-CV
    evaluation observation partitions under ``purged_cv_evaluation``.

Responsibilities:
    - Resolve storage locations via ``StorageLayout.purged_cv_evaluation_path``
...
Dependencies:
    ``polars``, ``cqros.core``, ``cqros.storage``, ``cqros.purged_cv.exceptions``,
    and ``cqros.purged_cv.evaluation_schema``.

Public API:
    ``PurgedCVEvaluationPartitionRef``, ``PurgedCVEvaluationRepository``
"""
...
_ERROR_FRAME_TYPE: Final[str] = "PCV_EVAL_REPO_FRAME_TYPE"
_ERROR_MISSING_COLUMNS: Final[str] = "PCV_EVAL_REPO_MISSING_COLUMNS"
_ERROR_SCHEMA_CAST: Final[str] = "PCV_EVAL_REPO_SCHEMA_CAST"


@dataclass(frozen=True, slots=True)
class PurgedCVEvaluationPartitionRef:
    """Identity of one discovered purged-CV evaluation year partition."""
...
class PurgedCVEvaluationRepository:
    """Repository facade for purged-CV evaluation observation datasets.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[PurgedCVEvaluationPartitionRef, ...]:
        ...
        items: list[PurgedCVEvaluationPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        path = self._layout.purged_cv_evaluation_path(
            manager,
            exchange,
            market,
            timeframe,
    ...
    def save(
        ...
        *,
        ...
        self._logger.info(
            "Saving purged-CV evaluation dataset",
            extra={
                "tier": STORAGE_DIR_PURGED_CV_EVALUATION,
                "manager": manager,
                "exchange": exchange,
                "market": market,
                "timeframe": timeframe,
    ...
    def _evaluation_root(self) -> Path:
        return self._layout.root / STORAGE_DIR_PURGED_CV_EVALUATION
...
def _require_observation_schema(frame: object) -> pl.DataFrame:
    ...
    if not isinstance(frame, pl.DataFrame):
        raise PurgedCVError(
            "purged-CV evaluation frame must be a polars DataFrame",
    ...
    if missing:
        raise PurgedCVError(
            "purged-CV evaluation schema is missing required columns",
    ...
    try:
        ...
        raise PurgedCVError(
            "purged-CV evaluation frame failed schema cast",
