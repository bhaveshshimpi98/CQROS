"""CQROS signal dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical trading
    signal year partitions produced by CQROS signal generation.

Responsibilities:
    - Resolve storage locations for signal year partitions via
      ``StorageLayout.signal_path``
    - Persist, load, check existence, and delete signal partitions
    - Discover symbols, timeframes, and year partitions under the signals
      tier
    ...
    - Remain free of signal generation, schema validation, and trading logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``SignalPartitionRef``, ``SignalRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical Signal
    schema validation belongs to ``SignalPipeline`` and ``SignalVerifier``.
"""
...
class SignalPartitionRef:
    """Identity of one discovered signal year partition.
...
class SignalRepository:
    """Repository facade for canonical trading signal datasets.

    Callers identify partitions by exchange, market, symbol, timeframe, and
    year. Paths are composed privately via ``StorageLayout.signal_path`` and
    ...
    def discover_symbols(
        ...
        *,
        ...
        """Return sorted symbols with at least one signal partition.
        ...
        """
        base = self._signals_root(exchange=exchange, market=market)
        if not base.is_dir():
            return ()
    ...
    def discover_timeframes(
        ...
        *,
        ...
        """
        base = self._signals_root(exchange=exchange, market=market) / symbol
        if not base.is_dir():
            return ()
        return tuple(sorted(path.name for path in base.iterdir() if path.is_dir()))
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as signal parquet partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[SignalPartitionRef, ...]:
        """Discover signal year partitions matching optional filters.

        Missing signal trees are skipped. Only year partitions that exist as
        ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            symbols: Optional symbol allowlist. ``None`` discovers every
                symbol present under the signals tier.
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a signal year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.signal_path``. Parquet contents are
        ...
        Returns:
            ``True`` when the yearly signal partition exists; otherwise
            ``False``.
        """
        path = self._layout.signal_path(exchange, market, symbol, timeframe, year)
        present = self._datastore.exists(path)
        self._logger.debug(
            "Signal dataset exists" if present else "Signal dataset does not exist",
            extra=_signal_log_extra(
                exchange=exchange,
                market=market,
                symbol=symbol,
                timeframe=timeframe,
    ...
    def save(
        ...
        *,
        ...
        """Persist a signal year partition.

        Args:
            dataframe: Signal frame to store.
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
            timeframe: Signal bar interval (for example ``1h``).
    ...
    def load(
        ...
        *,
        ...
        Returns:
            Eagerly loaded signal DataFrame.
        """
        path = self._layout.signal_path(exchange, market, symbol, timeframe, year)
        self._logger.debug(
            "Loading signal dataset",
            extra=_signal_log_extra(
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a signal year partition.
    ...
    def _signals_root(
        ...
        *,
        ...
        """Return the signals directory for exchange/market."""
        return self._layout.root / STORAGE_DIR_SIGNALS / exchange / market
    ...
    def _discover_years(
        ...
        *,
        ...
        base = self._signals_root(exchange=exchange, market=market) / symbol / timeframe
        if not base.is_dir():
            return ()
        years: list[int] = []
...
def _signal_log_extra(
    *,
    ...
    """Build structured log fields for a signal dataset operation."""
