"""CQROS feature dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving engineered
    feature year partitions produced by the Feature Engine.

Responsibilities:
    - Resolve storage locations for feature year partitions via
      ``StorageLayout.feature_path``
    - Persist, load, check existence, and delete feature partitions
    - Discover symbols, timeframes, and year partitions under the features
      tier
    ...
    - Remain free of feature computation, validation, and pipeline logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``FeaturePartitionRef``, ``FeatureRepository``
"""
...
class FeaturePartitionRef:
    """Identity of one discovered feature year partition.

    Attributes:
        symbol: Tradeable symbol.
        timeframe: Feature bar interval.
...
class FeatureRepository:
    """Repository facade for engineered feature datasets.

    Callers identify partitions by exchange, market, symbol, timeframe, and
    year. Paths are composed privately via ``StorageLayout.feature_path`` and
    ...
    Partition layout::
        ...
        features/{exchange}/{market}/{symbol}/{timeframe}/{year}.parquet
    ...
    def discover_symbols(
        ...
        *,
        ...
        """Return sorted symbols with at least one feature partition.
        ...
        """
        base = self._features_root(exchange=exchange, market=market)
        if not base.is_dir():
            return ()
    ...
    def discover_timeframes(
        ...
        *,
        ...
        """
        base = self._features_root(exchange=exchange, market=market) / symbol
        if not base.is_dir():
            return ()
        return tuple(sorted(path.name for path in base.iterdir() if path.is_dir()))
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as feature parquet partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[FeaturePartitionRef, ...]:
        """Discover feature year partitions matching optional filters.

        Missing feature trees are skipped. Only year partitions that exist as
        ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            symbols: Optional symbol allowlist. ``None`` discovers every
                symbol present under the features tier.
        ...
        items: list[FeaturePartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a feature year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.feature_path``. Parquet contents are
        ...
        Returns:
            ``True`` when the yearly feature partition exists; otherwise
            ``False``.
        """
        path = self._layout.feature_path(exchange, market, symbol, timeframe, year)
        present = self._datastore.exists(path)
        self._logger.debug(
            "Feature dataset exists" if present else "Feature dataset does not exist",
            extra=_feature_log_extra(
                exchange=exchange,
                market=market,
                symbol=symbol,
                timeframe=timeframe,
    ...
    def save(
        ...
        *,
        ...
        """Persist a feature year partition.

        Args:
            dataframe: Feature frame to store.
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
            timeframe: Feature bar interval (for example ``1h``).
    ...
    def load(
        ...
        *,
        ...
        Returns:
            Eagerly loaded feature DataFrame.
        """
        path = self._layout.feature_path(exchange, market, symbol, timeframe, year)
        self._logger.debug(
    ...
    def delete(
        ...
        *,
        ...
        """Delete a feature year partition.
    ...
    def _features_root(
        ...
        *,
        ...
        """Return the features directory for exchange/market."""
        return self._layout.root / STORAGE_DIR_FEATURES / exchange / market
    ...
    def _discover_years(
        ...
        *,
        ...
        base = self._features_root(exchange=exchange, market=market) / symbol / timeframe
        if not base.is_dir():
            return ()
        years: list[int] = []
...
def _feature_log_extra(
    *,
    ...
    """Build structured log fields for a feature dataset operation."""
