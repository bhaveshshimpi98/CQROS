"""CQROS portfolio dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    portfolio year partitions produced by CQROS portfolio optimization.

Responsibilities:
    - Resolve storage locations for portfolio year partitions via
      ``StorageLayout.portfolio_path``
    - Persist, load, check existence, and delete portfolio partitions
    - Discover symbols, timeframes, and year partitions under the portfolios
      tier
    ...
    - Remain free of portfolio optimization, schema validation, and trading
      logic
...
Public API:
    ``PortfolioPartitionRef``, ``PortfolioRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical Portfolio
    schema validation belongs to ``PortfolioPipeline`` and
    ``PortfolioVerifier``.
"""
...
class PortfolioPartitionRef:
    """Identity of one discovered portfolio year partition.

    Attributes:
        exchange: Exchange identifier.
        market: Market segment.
        symbol: Tradeable symbol.
        timeframe: Portfolio bar interval.
...
class PortfolioRepository:
    """Repository facade for canonical portfolio datasets.

    Callers identify partitions by exchange, market, symbol, timeframe, and
    year. Paths are composed privately via ``StorageLayout.portfolio_path``
    and never returned. Persistence is delegated entirely to the injected
    ``IDataStore`` (commonly ``ParquetStore``) so atomic writes and alternate
    backends can be substituted without changing this API. Frames are stored
    exactly as provided; schema validation is not performed here.
    ...
    def discover_symbols(
        ...
        *,
        ...
        """Return sorted symbols with at least one portfolio partition.
        ...
        """
        base = self._portfolios_root(exchange=exchange, market=market)
        if not base.is_dir():
            return ()
    ...
    def discover_timeframes(
        ...
        *,
        ...
        """
        base = self._portfolios_root(exchange=exchange, market=market) / symbol
        if not base.is_dir():
            return ()
        return tuple(sorted(path.name for path in base.iterdir() if path.is_dir()))
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as portfolio parquet partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[PortfolioPartitionRef, ...]:
        """Discover portfolio year partitions matching optional filters.

        Missing portfolio trees are skipped. Only year partitions that exist
        as ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            symbols: Optional symbol allowlist. ``None`` discovers every
                symbol present under the portfolios tier.
        ...
        items: list[PortfolioPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a portfolio year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.portfolio_path``. Parquet contents
        ...
        Returns:
            ``True`` when the yearly portfolio partition exists; otherwise
            ``False``.
        """
        path = self._layout.portfolio_path(exchange, market, symbol, timeframe, year)
        present = self._datastore.exists(path)
        self._logger.debug(
            "Portfolio dataset exists" if present else "Portfolio dataset does not exist",
            extra=_portfolio_log_extra(
                exchange=exchange,
                market=market,
                symbol=symbol,
                timeframe=timeframe,
    ...
    def save(
        ...
        *,
        ...
        """Persist a portfolio year partition.

        Args:
            dataframe: Portfolio frame to store.
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
            timeframe: Portfolio bar interval (for example ``1h``).
    ...
    def load(
        ...
        *,
        ...
        """Load a portfolio year partition.
        ...
        Returns:
            Eagerly loaded portfolio DataFrame.
        """
        path = self._layout.portfolio_path(exchange, market, symbol, timeframe, year)
        self._logger.debug(
            "Loading portfolio dataset",
            extra=_portfolio_log_extra(
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a portfolio year partition.
    ...
    def _portfolios_root(
        self,
        *,
        exchange: Exchange,
        market: Market,
    ) -> Path:
        """Return the portfolios directory for exchange/market."""
        return self._layout.root / STORAGE_DIR_PORTFOLIOS / exchange / market
    ...
    def _discover_years(
        ...
        *,
        ...
        base = self._portfolios_root(exchange=exchange, market=market) / symbol / timeframe
        if not base.is_dir():
            return ()
        years: list[int] = []
...
def _portfolio_log_extra(
    *,
    ...
    """Build structured log fields for a portfolio dataset operation."""
