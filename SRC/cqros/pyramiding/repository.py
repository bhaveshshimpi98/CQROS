"""CQROS pyramiding recommendation dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    pyramiding year partitions produced by CQROS Pyramiding Engine evaluation.

Responsibilities:
    - Resolve storage locations for pyramiding year partitions via
      ``StorageLayout.pyramiding_path``
    - Persist, load, check existence, and delete pyramiding partitions
    ...
    - Remain free of pyramiding math, schema validation, and trading logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``PyramidingPartitionRef``, ``PyramidingRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical
    pyramiding schema validation belongs to ``PyramidingPipeline`` and
    ``PyramidingVerifier``. This module must not import pyramiding engines.
    Writes overwrite existing partitions when the datastore permits.
"""
...
class PyramidingPartitionRef:
    """Identity of one discovered pyramiding year partition.
...
class PyramidingRepository:
    """Repository facade for canonical pyramiding datasets.

    Callers identify partitions by manager, exchange, market, symbol,
    timeframe, and year. Paths are composed privately via
    ``StorageLayout.pyramiding_path`` and never returned. Persistence is
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as pyramiding partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[PyramidingPartitionRef, ...]:
        """Discover pyramiding year partitions matching optional filters.

        Missing pyramiding trees are skipped. Only year partitions that exist
        as ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the pyramiding tier.
        ...
        items: list[PyramidingPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether a pyramiding year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.pyramiding_path``. Parquet contents
        ...
        Returns:
            ``True`` when the yearly pyramiding partition exists; otherwise
            ``False``.
        """
        path = self._layout.pyramiding_path(
            manager,
            exchange,
            market,
            symbol,
        ...
        self._logger.debug(
            ("Pyramiding dataset exists" if present else "Pyramiding dataset does not exist"),
            extra=_pyramiding_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def save(
        ...
        *,
        ...
        """Persist a pyramiding year partition, overwriting when present.

        Args:
            dataframe: Pyramiding frame to store.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
    ...
    def load(
        ...
        *,
        ...
        """Load a pyramiding year partition.
        ...
        Returns:
            Eagerly loaded pyramiding DataFrame.
        """
        path = self._layout.pyramiding_path(
            manager,
            exchange,
    ...
    def delete(
        ...
        *,
        ...
        """Delete a pyramiding year partition.
    ...
    def _pyramiding_root(self) -> Path:
        """Return the pyramiding directory under the storage root."""
        return self._layout.root / STORAGE_DIR_PYRAMIDING
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the pyramiding directory for manager/exchange/market."""
        return self._pyramiding_root() / manager / exchange / market
...
def _pyramiding_log_extra(
    *,
    ...
    """Build structured log fields for a pyramiding dataset operation."""
