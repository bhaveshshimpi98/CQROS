"""CQROS executed-trade dataset repository.

Purpose:
    Provide a path-free facade for persisting and retrieving canonical
    executed-trade year partitions produced by CQROS execution simulation.

Responsibilities:
    - Resolve storage locations for execution year partitions via
      ``StorageLayout.execution_path``
    - Persist, load, check existence, and delete execution partitions
    ...
    - Remain free of simulation, schema validation, and trading logic

Dependencies:
    ``polars``, ``cqros.core``, and ``cqros.storage`` layout/interfaces.

Public API:
    ``ExecutionPartitionRef``, ``TradeRepository``

Notes:
    Frames are persisted and loaded exactly as provided. Canonical trade
    schema validation belongs to ``ExecutionPipeline`` and
    ``ExecutionVerifier``. This module must not import fill simulators.
"""
...
class ExecutionPartitionRef:
    """Identity of one discovered executed-trade year partition.
...
class TradeRepository:
    """Repository facade for canonical executed-trade datasets.

    Callers identify partitions by manager, exchange, market, symbol,
    timeframe, and year. Paths are composed privately via
    ``StorageLayout.execution_path`` and never returned. Persistence is
    ...
    def discover_managers(self) -> tuple[str, ...]:
        """Return sorted manager identifiers with at least one trade partition.

        Returns:
            Deterministically ordered manager identifiers discovered on disk.
        """
        base = self._executions_root()
        if not base.is_dir():
            return ()
    ...
    def discover_symbols(
        ...
        *,
        ...
        """Return sorted symbols with at least one trade partition for ``manager``.
    ...
    def list_years(
        ...
        *,
        ...
        """Return sorted calendar years present as trade parquet partitions.
    ...
    def discover_partitions(
        ...
        *,
        ...
        exchange: Exchange = _EXCHANGE,
        market: Market = _MARKET,
    ) -> tuple[ExecutionPartitionRef, ...]:
        """Discover execution year partitions matching optional filters.

        Missing execution trees are skipped. Only year partitions that exist
        as ``{year}.parquet`` files are included. Paths are never returned.

        Args:
            managers: Optional manager allowlist. ``None`` discovers every
                manager present under the executions tier.
        ...
        items: list[ExecutionPartitionRef] = []
    ...
    def exists(
        ...
        *,
        ...
        """Return whether an execution year partition exists.

        Existence is determined solely through ``IDataStore.exists`` on the
        path composed by ``StorageLayout.execution_path``. Parquet contents
        ...
        Returns:
            ``True`` when the yearly execution partition exists; otherwise
            ``False``.
        """
        path = self._layout.execution_path(
            manager,
            exchange,
            market,
            symbol,
        ...
        self._logger.debug(
            "Execution dataset exists" if present else "Execution dataset does not exist",
            extra=_execution_log_extra(
                manager=manager,
                exchange=exchange,
                market=market,
                symbol=symbol,
    ...
    def save(
        ...
        *,
        ...
        """Persist an execution year partition.

        Args:
            dataframe: Trade frame to store.
            manager: Order manager identifier (for example ``simple``).
            exchange: Exchange identifier (for example ``binance``).
            market: Market segment (for example ``usdt_perpetual``).
            symbol: Tradeable symbol (for example ``BTCUSDT``).
    ...
    def load(
        ...
        *,
        ...
        """Load an execution year partition.
        ...
        Returns:
            Eagerly loaded trade DataFrame.
        """
        path = self._layout.execution_path(
            manager,
            exchange,
    ...
    def delete(
        ...
        *,
        ...
        """Delete an execution year partition.
    ...
    def _executions_root(self) -> Path:
        """Return the executions directory under the storage root."""
        return self._layout.root / STORAGE_DIR_EXECUTIONS
    ...
    def _manager_root(
        ...
        *,
        ...
        """Return the executions directory for manager/exchange/market."""
        return self._executions_root() / manager / exchange / market
...
def _execution_log_extra(
    *,
    ...
    """Build structured log fields for an execution dataset operation."""
