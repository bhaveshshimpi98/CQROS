"""CQROS merged label dataset verification.

Purpose:
    Inspect merged label frames and report structural findings without
    cleaning or mutating input data.

Responsibilities:
    - Validate required merged-label columns and expected dtypes
...
Dependencies:
    ``polars``, the Python standard library, ``cqros.labels.schema``,
    ``cqros.labels.verification.exceptions``,
...
_WARN_NUMERIC: Final[str] = "Infinite label values detected."
...
class LabelVerifier(BaseVerifier):
    """Deterministic merged-label verifier that reports findings only.

    Inspects structural quality of a merged label frame against
    ``cqros.labels.schema``. Does not clean rows, fill gaps, sort
    timestamps, mutate values, access storage, or compute labels.
    """
    ...
    def verify(self, frame: pl.DataFrame) -> VerificationReport:
        ...
        Args:
            frame: Input merged label DataFrame. Must not be mutated.
        ...
        Raises:
            LabelValidationError: If any required column is missing or
                column dtypes do not match the merged label schema.
        """
        ...
        nan_rows = self._count_nan_rows(frame, LABEL_COLUMNS)
    ...
    def _validate_required_columns(
        ...
        Raises:
            LabelValidationError: If one or more required columns are missing.
        """
    ...
    def _validate_column_dtypes(self, frame: pl.DataFrame) -> None:
        ...
        Raises:
            LabelValidationError: If one or more column dtypes do not match
                ``COLUMN_DTYPES``.
        """
    ...
    def _count_duplicate_key_rows(self, frame: pl.DataFrame) -> int:
        ...
        Uses keep-first semantics over the canonical merged-label primary
        key.

        Args:
            frame: Input label DataFrame. Must not be mutated.
    ...
    def _count_infinite_rows(self, frame: pl.DataFrame) -> int:
        ...
        Non-floating columns listed in ``LABEL_COLUMNS`` are ignored. NaN is
        ...
        Returns:
            Count of rows with one or more infinite label values.
        """
        if frame.height == 0:
            return 0
        floating_columns = tuple(
            name for name in LABEL_COLUMNS if name in frame.schema and frame.schema[name].is_float()
        )
